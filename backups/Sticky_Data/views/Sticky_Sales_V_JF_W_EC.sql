CREATE VIEW `variant-finance-data-project.Sticky_Data.Sticky_Sales_V_JF_W_EC`
AS WITH IP_Parse AS (
  SELECT 
    JF.*,
    'JF' AS Entity_Name,
    ...
    LOWER(TRIM(
      ...
      SPLIT(JF.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(JF.IP_Address_Lookup, ' ')) - 2)] || ' ' ||
      SPLIT(JF.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(JF
      .IP_Address_Lookup, ' ')) - 1)]
    )) AS last_3_words
  FROM `Sticky_Data.Sticky_JF`  JF
),
...
Country_Lookup_Base AS (
  ...
  LEFT JOIN `Sticky_Data.Sticky_Dim_Campaign` CAM
    ON CONCAT('JF_', ip.Campaign_Id) = CONCAT(CAM.Entity,'_',CAM.Campaign_Id)
),
...
LEFT JOIN Sticky_Data.Sticky_Dim_AFID AF 
  ON CONCAT('JF_', CL.AFID) = CONCAT(AF.ENTITY, '_', AF.AFID)
...
LEFT JOIN Sticky_Data.Sticky_Dim_Order_Status OS
  ON CONCAT('JF_', CL.Order_Status) = CONCAT('JF_', OS.Order_Status)
