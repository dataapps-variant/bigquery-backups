CREATE VIEW `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_FS`
AS WITH combined_data AS (
  -- Union all the FS tables first
  SELECT
...
UNION ALL
  ...
  SELECT
    ...
    prd_offer_id AS Offer_Id,
    cc_first_6 AS BIN
  FROM `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_FS_Incremental`
),
ranked_data AS (
  SELECT *,
    ROW_NUMBER() OVER (
