CREATE VIEW `variant-finance-data-project.Sticky_Data.Sticky_Sales_V_PD_API_W_EC`
AS WITH IP_Parse AS (
  SELECT 
    PD.*,
    'PD' AS Entity_Name,
...
Country_Lookup_Base AS (
  ...
  LEFT JOIN `Sticky_Data.Sticky_Dim_Country_Code` CC_Bill_Name
    ON IP.Bill_Country = CC_Bill_Name.Country_Name
    
  LEFT JOIN `Sticky_Data.Sticky_Dim_Country_Code` CC_Ship_Name
...
SELECT 
  ...
  --Cl.Tax_Country_Code AS Tax_Country_Code,
...
---------------------------------------------------------------------------------------------
  ...
  CASE 
    WHEN CC_Tax.Country_Code = 'US' THEN cast(CL.SALES_TAX_FACTOR as float64)--if null then 0
    ELSE SAFE_MULTIPLY(
           SAFE_DIVIDE(Cast(CL.Order_Total as Float64), 1 + STL.VAT_Rate),
           STL.VAT_Rate
         )
  END
 AS Sales_Tax_Amount_Native_Currency,
  ...
  CASE 
    ...
    ELSE SAFE_DIVIDE(
      CASE 
        ...
        ELSE SAFE_MULTIPLY(PROD.Product_Price, Cast(CL.Quantity as Float64))
      END,
      (1 + STL.VAT_Rate)---if null then 0
    )
  END
  
 AS Plan_Price_Net_of_Tax_Native_Currency,
...
--LEFT JOIN Sticky_Data.Sticky_Dim_Order_Status OS
  --ON CONCAT('PD_', cast(cast(CL.Order_Status as float64) as int64)) = CONCAT('PD_', OS.-Order_Status)
