CREATE VIEW `variant-finance-data-project.Sticky_Data.Sticky_Sales_V_CT_W_EC`
AS WITH IP_Parse AS (
  SELECT 
    CT.*,
    'CT' AS Entity_Name,
    ...
    LOWER(TRIM(
      ...
      SPLIT(CT.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(CT.IP_Address_Lookup, ' ')) - 2)] || ' ' ||
      SPLIT(CT.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(CT
      .IP_Address_Lookup, ' ')) - 1)]
    )) AS last_3_words
  FROM `Sticky_Data.Sticky_CT`  CT
),
...
LEFT JOIN Sticky_Data.Sticky_Dim_AFID AF 
  ON CONCAT('CT_', CL.AFID) = CONCAT(AF.ENTITY, '_', AF.AFID)
...
LEFT JOIN Sticky_Data.Sticky_Dim_Order_Status OS
  ON CONCAT('CT_', CL.Order_Status) = CONCAT('CT_', OS.Order_Status)
