CREATE VIEW `variant-finance-data-project.Sticky_Data.Sticky_Sales_V_AT_W_EC`
AS WITH IP_Parse AS (
  SELECT 
    ATT.*,
    'AT' AS Entity_Name,
    ...
    LOWER(TRIM(
      ...
      SPLIT(ATT.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(ATT.IP_Address_Lookup, ' ')) - 2)] || ' ' ||
      SPLIT(ATT.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(ATT
      .IP_Address_Lookup, ' ')) - 1)]
    )) AS last_3_words
  FROM Sticky_Data.Sticky_AT as ATT
),
...
LEFT JOIN Sticky_Data.Sticky_Dim_AFID AF 
  ON CONCAT('AT_', CL.AFID) = CONCAT(AF.ENTITY, '_', AF.AFID)
...
LEFT JOIN Sticky_Data.Sticky_Dim_Offer OFR 
  ON CONCAT('AT_', CAST(CAST(CL.Offer_Id AS FLOAT64) AS INT64)) = CONCAT(OFR.Entity, '_', OFR.ID)


LEFT JOIN Sticky_Data.Sticky_Dim_Order_Status OS
  ON CONCAT('AT_', CL.Order_Status) = CONCAT('AT_', OS.Order_Status)
