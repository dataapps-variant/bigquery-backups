-- job_id: 1f6d9192-00ba-41d9-9edf-768fc64112c1
-- user: 1041741270489-compute@developer.gserviceaccount.com
-- statement_type: SELECT
-- created: 2026-07-27T13:24:06.770000+00:00
-- started: 2026-07-27T13:24:06.875000+00:00
-- ended: 2026-07-27T13:24:07.962000+00:00
    ...
    SELECT CAST(time_stamp AS TIMESTAMP) AS time_stamp, CAST(customer_id AS INTEGER) AS customer_id, email_address, CAST(order_id AS INTEGER) AS order_id, CAST(order_status AS INTEGER) AS order_status, decline_reason FROM `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_CT_Incremental` WHERE DATETIME(time_stamp) BETWEEN DATETIME_TRUNC(DATETIME_SUB(datetime(current_timestamp, 'America/New_York'), INTERVAL 1 HOUR), HOUR) AND DATETIME_TRUNC(datetime(current_timestamp, 'America/New_York'), HOUR) AND CAST(billing_cycle AS INTEGER) = 0 AND campaign_id IN (SELECT Campaign_Id FROM `variant-finance-data-project.Sticky_Data.Sticky_Dim_Campaign` WHERE App_Name = 'EN')
...
SELECT 'EN' AS app, repeat_failures.*, coalesce(app_ranks.alert_level, 0) AS app_priority, coalesce(app_ranks.subs_count_t30d, 0) AS subs_count_t30d FROM (SELECT max(c) AS total, array_agg(DISTINCT decline_reason) AS decline_reasons FROM (
...
ORDER BY subs_count_t30d DESC) AS app_ranks ON app_ranks.app = 'EN'
