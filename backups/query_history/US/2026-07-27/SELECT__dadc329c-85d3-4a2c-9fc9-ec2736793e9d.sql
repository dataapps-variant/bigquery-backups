-- job_id: dadc329c-85d3-4a2c-9fc9-ec2736793e9d
-- user: 1041741270489-compute@developer.gserviceaccount.com
-- statement_type: SELECT
-- created: 2026-07-27T17:16:26.673000+00:00
-- started: 2026-07-27T17:16:26.801000+00:00
-- ended: 2026-07-27T17:16:27.858000+00:00
    ...
    SELECT CAST(time_stamp AS TIMESTAMP) AS time_stamp, CAST(customer_id AS INTEGER) AS customer_id, email_address, CAST(gateway_id AS INTEGER) AS gateway_id, CAST(order_id AS INTEGER) AS order_id, CAST(order_status AS INTEGER) AS order_status, decline_reason FROM (SELECT COALESCE(CASE WHEN AF.JF_App_Name IS NOT NULL AND TRIM(AF.JF_App_Name) != '' AND UPPER(TRIM(AF.JF_App_Name)) != 'COMBINED' THEN AF.JF_App_Name ELSE NULL END, CAM.App_Name, 'JF') AS app, o.* FROM `variant-finance-data-project.Sticky_Data.test_Sticky_data_API_original_JF_Incremental` AS o
...
SELECT 'CV' AS app, rf.gateway_id, rf.consecutive, coalesce(app_ranks.alert_level, 0) AS app_priority, coalesce(app_ranks.subs_count_t30d, 0) AS subs_count_t30d FROM red_flags AS rf
