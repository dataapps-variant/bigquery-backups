-- job_id: 968e08f4-ad29-4e20-ae23-92b3c610ad7f
-- user: 1041741270489-compute@developer.gserviceaccount.com
-- statement_type: SELECT
-- created: 2026-08-02T18:16:26.774000+00:00
-- started: 2026-08-02T18:16:26.884000+00:00
-- ended: 2026-08-02T18:16:29.213000+00:00


    WITH ct_users AS (
SELECT 'CT' AS app, c.report_date, a.afid_channel, a.Country_Code, sum(c.subs) AS subs FROM (
SELECT DATE(time_stamp) AS report_date, afid, count(*) AS subs FROM (
    SELECT time_stamp, afid, order_id AS subs FROM `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_CT_Incremental`
    WHERE DATE(time_stamp) >= DATE(datetime(current_timestamp, 'America/New_York'))-7 AND billing_cycle = '0' AND order_status <> '7'
    AND campaign_id IN (SELECT Campaign_Id FROM `variant-finance-data-project.Sticky_Data.Sticky_Dim_Campaign` WHERE App_Name = 'CT')
...
en_users AS (
SELECT 'EN' AS app, c.report_date, a.afid_channel, a.Country_Code, sum(c.subs) AS subs FROM (
SELECT DATE(time_stamp) AS report_date, afid, count(*) AS subs FROM (
    SELECT time_stamp, afid, order_id AS subs FROM `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_CT_Incremental`
    WHERE DATE(time_stamp) >= DATE(datetime(current_timestamp, 'America/New_York'))-7 AND billing_cycle = '0' AND order_status <> '7'
    AND campaign_id IN (SELECT Campaign_Id FROM `variant-finance-data-project.Sticky_Data.Sticky_Dim_Campaign` WHERE App_Name = 'EN')
    --FIX FOR STICKY/GOOGLE SYNC OFFSET!AND time_stamp < (SELECT max(_sdc_batched_at) - INTERVAL '4 hour' FROM googleadsv2.campaign_performance_report WHERE date = CAST((SELECT start_dttm FROM ods_stage.stk_api_extract_cdc_stg WHERE entity_name = 'PD') AS DATE))
) AS d
GROUP BY DATE(time_stamp), afid
) AS c
...
spend AS (
  SELECT Date AS report_date, App_name AS app_short_name, Channel_type AS traffic_channel, Channel_code, Country AS country, sum(Amount) AS spend FROM `variant-finance-data-project.Ad_spend_data.Merged_Spend_View_Updated_lagged`
WHERE Date >= current_date-7 AND App_name IN ('CT', 'EN') AND Channel_name = 'Google_Ads'
...
) AS u ON s.report_date = u.report_date AND s.Channel_code = u.afid_channel AND s.app_short_name = u.app AND s.country = u.country_code)
UNION ALL
(SELECT s.report_date, s.app_short_name, s.traffic_channel, s.country, s.spend, u.subs, CASE WHEN u.subs = 0 THEN NULL ELSE s.spend / u.subs END AS cac
...
LEFT JOIN (SELECT app, report_date, 'ALL' AS traffic_channel, country_code, sum(subs) AS subs FROM (
    SELECT app, report_date, 'ALL' AS traffic_channel, country_code, sum(subs) AS subs FROM ct_users GROUP BY report_date, app, country_code
    UNION ALL
    SELECT app, report_date, 'ALL' AS traffic_channel, country_code, sum(subs) AS subs FROM en_users GROUP BY report_date, app, country_code
) AS u GROUP BY report_date, app, country_code) AS u ON s.report_date = u.report_date AND s.app_short_name = u.app AND s.country = u.country_code)
),
calc_squares AS (
...
subs_data AS (
SELECT app, a.Channel_Type AS traffic_channel, a.Country_Code, sum(c.subs) AS subs_count_t30d FROM (
SELECT Date_of_Sale AS report_date, app, afid, count(*) AS subs FROM (
    SELECT Date_of_Sale, App_Name AS app, AFID AS afid, order_id FROM `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_V_Merged_TBL`
    WHERE Date_of_Sale >= DATE(datetime(current_timestamp, 'America/New_York'))-30 AND billing_cycle = '0' AND Final_Order_Status <> 7 AND Trial_Type <> 'SS' AND AFID <> '' AND App_Name IN ('CT', 'EN')
) AS d
GROUP BY Date_of_Sale, app, afid
) AS c
LEFT JOIN `variant-finance-data-project.Sticky_Data.Sticky_Dim_AFID` AS a ON c.afid = a.AFID
WHERE a.Channel_Name = 'Google_Ads'
GROUP BY c.report_date, c.app, a.Channel_Type, a.country_code
),
t30d_subs AS (
