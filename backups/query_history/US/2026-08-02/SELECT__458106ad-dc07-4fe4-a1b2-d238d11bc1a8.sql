-- job_id: 458106ad-dc07-4fe4-a1b2-d238d11bc1a8
-- user: 1041741270489-compute@developer.gserviceaccount.com
-- statement_type: SELECT
-- created: 2026-08-02T12:15:23.401000+00:00
-- started: 2026-08-02T12:15:23.504000+00:00
-- ended: 2026-08-02T12:15:25.180000+00:00


    WITH users AS (
SELECT 'PD' AS app, c.report_date, a.afid_channel, a.Country_Code, sum(c.subs) AS subs FROM (
SELECT DATE(time_stamp) AS report_date, afid, count(*) AS subs FROM (
    SELECT time_stamp, afid, order_id AS subs FROM `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_PD_Incremental`
    WHERE DATE(time_stamp) >= DATE(datetime(current_timestamp, 'America/New_York'))-7 AND billing_cycle = '0' AND order_status <> '7'
    AND campaign_id IN (SELECT Campaign_Id FROM `variant-finance-data-project.Sticky_Data.Sticky_Dim_Campaign` WHERE App_Name = 'PD')
...
WHERE Date >= current_date-7 AND App_name = 'PD' AND Channel_name = 'Google_Ads'
...
subs_data AS (
SELECT 'PD' AS app, a.Channel_Type AS traffic_channel, a.Country_Code, sum(c.subs) AS subs_count_t30d FROM (
SELECT Date_of_Sale AS report_date, afid, count(*) AS subs FROM (
    ...
    WHERE Date_of_Sale >= DATE(datetime(current_timestamp, 'America/New_York'))-30 AND billing_cycle = '0' AND Final_Order_Status <> 7 AND Trial_Type <> 'SS' AND AFID <> '' AND App_Name = 'PD'
) AS d
