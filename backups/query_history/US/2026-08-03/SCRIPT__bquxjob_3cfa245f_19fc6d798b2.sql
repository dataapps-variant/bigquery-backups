-- job_id: bquxjob_3cfa245f_19fc6d798b2
-- user: dataapps@variant.net
-- statement_type: SCRIPT
-- created: 2026-08-03T08:57:26.716000+00:00
-- started: 2026-08-03T08:57:26.794000+00:00
-- ended: 2026-08-03T08:57:28.194000+00:00
...
--
-- Run AFTER 4b.1. Splits the winning model's error into the part that is
-- the model's responsibility and the part that is product churn.
--
-- NO DECLARE HERE. GoogleSQL allows variable declarations only at the
-- very top of a script, and 4b.1 already used that slot. The cutoff is
-- computed as a CTE instead, so this block runs standalone OR after 4b.1.
--
...
WITH
cut AS (
  SELECT APPROX_QUANTILES(acquisition_date, 100)[OFFSET(80)] AS eval_cutoff
  FROM `variant-finance-data-project.Cohort_Hunter.v_ltv_training`
),
seen_products AS (
  SELECT DISTINCT t.product_name_final_merged
  FROM `variant-finance-data-project.Cohort_Hunter.v_ltv_training` t, cut c
  WHERE t.acquisition_date <= c.eval_cutoff
),
holdout AS (
  SELECT
    w.entity_name, w.app_name, w.product_name_final_merged,
    w.trial_type, w.is_nt,
    CAST(w.afid_channel AS STRING) AS afid_channel,
    w.spend_country_code_afid, w.payment, w.gateway_id,
    w.dow_bc0,
    CAST(w.month_bc0 AS STRING)    AS month_bc0,
    w.retry_bc0, w.acquisition_date, w.cac,
    w.bc4_net_ltv                  AS actual_net_ltv
  FROM `variant-finance-data-project.Cohort_Hunter.User_Feature_Wide` w, cut c
  WHERE w.is_bc4_mature = TRUE
    AND NOT w.is_organic
    AND w.bc4_net_ltv IS NOT NULL
    AND w.cac IS NOT NULL
    AND w.acquisition_date > c.eval_cutoff
),
scored AS (
  SELECT
    product_name_final_merged,
    actual_net_ltv,
    -- MODEL B version. For Model A: swap the model name to
    -- ltv_bc4_linear_v1 and use predicted_bc4_net_ltv without "- cac".
    predicted_cum_net_arpu_thru_bc4 - cac AS pred_net_ltv
  FROM ML.PREDICT(
    MODEL `variant-finance-data-project.Cohort_Hunter.arpu_bc4_linear_v1`,
    (SELECT * FROM holdout)
...
--       -> product identity was not carrying much signal anyway.
--          The 8.88% is a non-issue; report the headline number as-is.
--   MAE much worse on unseen
--       -> expected. Report the SEEN figure as the model's real
