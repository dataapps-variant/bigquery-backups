-- job_id: scheduled_query_6aefae43-0000-21ab-89ea-d4f547f40c40
-- user: dataapps@variant.net
-- statement_type: CREATE_TABLE_AS_SELECT
-- created: 2026-08-03T08:00:04.765000+00:00
-- started: 2026-08-03T08:00:05.038000+00:00
-- ended: 2026-08-03T08:00:06.387000+00:00

CREATE OR REPLACE TABLE `variant-finance-data-project.R100.CT_new_trailing100` AS
WITH 


-- Step 1B: Find products from other apps that map to CT
...
base_data AS (
    -- Step 1A: First data source - Direct CT data from Sticky table
    SELECT 
    ...
    -- Filter for last 100 days from current date and App_Name = "CT"
    ...
    -- Step 1B: Additional data from other apps that map to CT
    SELECT 
        -- Day Charged (past 100 days) - Date_of_Sale filtered for last 100 days
    ...
    SELECT 
        ...
        -- APP SHORT NAME - Rename CT-JP and CT-Non-JP to CT before lookup
        CASE 
            WHEN App_name IN ('CT-JP', 'CT-Non-JP') THEN 'CT'
            ELSE App_name
        END AS APP_SHORT_NAME,
    ...
    -- Filter for last 100 days from current date and App_name in CT-JP or CT-Non-JP
    WHERE Date >= DATE_SUB(CURRENT_DATE(), INTERVAL 100 DAY)
        AND Date <= CURRENT_DATE()
        AND App_name IN ('CT-JP', 'CT-Non-JP')
...
-- Filter to keep only records where APP_SHORT_NAME = "CT" after lookup
WHERE COALESCE(upd.Updated_App_Name, base.APP_SHORT_NAME) = 'CT'
