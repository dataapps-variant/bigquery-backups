-- job_id: script_job_c41bdd480f9acd38ba7f6f7d71659150_3
-- user: dataapps@variant.net
-- statement_type: CREATE_TABLE_AS_SELECT
-- created: 2026-08-03T08:15:02.858000+00:00
-- started: 2026-08-03T08:15:03.126000+00:00
-- ended: 2026-08-03T08:15:04.363000+00:00
...
CREATE OR REPLACE TABLE `variant-finance-data-project.Icarus_Cohort.IC_AFID_List` AS
  WITH base AS (
    ...
    COUNT(DISTINCT CASE WHEN FORMAT_DATE('%Y-%m', Date_of_Sale) = '2026-08' THEN Updated_Cust_ID END) AS `Aug_2026`
    FROM
      `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_V_Merged_TBL`
    WHERE
      Date_of_Sale >= '2025-04-01'
      AND Date_of_Sale < '2026-08-04'
      AND (
  ...
  SELECT
    *,
    CASE WHEN (`Jun_2026` + `Jul_2026` + `Aug_2026`) > 50 THEN 'Yes' ELSE 'No' END AS Last_3_Months_Over_50
  FROM base
