-- job_id: scheduled_query_6aefae54-0000-21ab-89ea-d4f547f40c40
-- user: dataapps@variant.net
-- statement_type: CREATE_TABLE_AS_SELECT
-- created: 2026-08-03T08:00:04.132000+00:00
-- started: 2026-08-03T08:00:04.338000+00:00
-- ended: 2026-08-03T08:00:05.682000+00:00

CREATE OR REPLACE TABLE `variant-finance-data-project.R100.CL_new_trailing100` AS
WITH 
...
-- Step 1B: Find products from other apps that map to CL
cross_app_products AS (
    SELECT DISTINCT
        App_Name,
        Product_Name
    FROM `variant-finance-data-project.Sticky_Data.R100_BQ_Updated_Product_Name`
    WHERE Updated_App_Name = 'CL'
        AND App_Name != 'CL'
),

base_data AS (
    -- Step 1A: First data source - Direct CL data from Sticky table
    SELECT 
    ...
    -- Filter for last 100 days from current date and App_Name = "CL"
    ...
    -- Step 1B: Additional data from other apps that map to CL
    SELECT 
        -- Day Charged (past 100 days) - Date_of_Sale filtered for last 100 days
...
-- Filter to keep only records where APP_SHORT_NAME = "CL" after lookup
WHERE COALESCE(upd.Updated_App_Name, base.APP_SHORT_NAME) = 'CL'
