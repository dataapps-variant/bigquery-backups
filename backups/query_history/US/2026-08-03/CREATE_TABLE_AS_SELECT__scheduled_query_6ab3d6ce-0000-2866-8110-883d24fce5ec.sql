-- job_id: scheduled_query_6ab3d6ce-0000-2866-8110-883d24fce5ec
-- user: dataapps@variant.net
-- statement_type: CREATE_TABLE_AS_SELECT
-- created: 2026-08-03T09:00:12.112000+00:00
-- started: 2026-08-03T09:00:12.473000+00:00
-- ended: 2026-08-03T09:03:49.416000+00:00

-- =============================================================================
-- RETENTION ANALYSIS V2.5: BC0 through BC12 with Rolling Cohorts
-- =============================================================================
--
-- CHANGES FROM V2.4 TO V2.5:
--   1. Added avg_metrics CTE with 3-day rolling averages at 3-day intervals
--      (anchor: 2025-04-09, show dates where DATE_DIFF % 3 = 0)
--   2. New columns: Cohort_Size_Avg, Approval_Rate_Avg, Attempt_Rate_Avg,
--      Net_Retention_Rate_Avg
--
-- =============================================================================

CREATE OR REPLACE TABLE `variant-finance-data-project.ICARUS_Multi.Retention_Approval_Analysis` AS
...
-- Only need plan_map join for Regular_BC_period
...
-- =============================================================================
-- SEGMENTS: Unique Product + Country segments with modified App_Name
-- =============================================================================
segments AS (
  SELECT DISTINCT
...
analysis_grid AS (
  SELECT
    ...
    CASE
      WHEN bc.bc = 0 THEN 0                          -- Non-NT anchor (NT BC0 excluded below)
      WHEN bc.bc = 1 AND s.Trial_Type = 'NT' THEN 1  -- NT anchor
      ELSE bc.bc - 1                                  -- Retention steps (both plan types)
    END AS base_bc_updated,

    -- Target (current cycle) Billing_Cycle_Updated value — no shift for NT
...
-- BC0: all statuses; BC1+: pre-filtered to clean approved
-- Rank by Date_of_Sale DESC for 7K cap
-- =============================================================================
...
-- =============================================================================
-- STEP B: Apply 7K cap, compute BC_end_date
-- =============================================================================
cohort_bounds AS (
  SELECT
    report_date,
...
target_orders AS (
  ...
  LEFT JOIN (
    SELECT
      ...
      -- Has at least one approved status (2, 6, or 8)
      MAX(CASE WHEN Final_Order_Status IN (2, 6, 8) THEN 1 ELSE 0 END) AS has_approved_status,

      -- MAX of Is_Refund and Is_Chargeback — outer query checks values
      MAX(Is_Refund) AS Is_Refund,
      MAX(Is_Chargeback) AS Is_Chargeback,

      -- Total transaction count (all rows = retry attempts)
      COUNT(*) AS attempted_transaction_count,

      -- Approved transaction count (rows with approved status)
...
-- =============================================================================
-- STEP F: Aggregate metrics per grid row
-- =============================================================================
...
-- =============================================================================
-- STEP G: Compute cumulative retention rates using window functions
--         (V2.4: NULL cascade removed — straight cumulative product)
-- =============================================================================
retention_calc AS (
  SELECT
    *,
    SAFE_DIVIDE(Attempted_Users, Cohort_Size) AS gross_survival,
    -- Anchor BC uses Approved_Users as denominator; all other BCs use Cohort_Size
    CASE
...
cumulative_retention AS (
  SELECT
    *,
    ...
    -- Cumulative Attempt Rate (straight product — no cascade)
    SAFE_DIVIDE(Attempted_Users, Cohort_Size) AS Attempt_Rate,

    -- Cumulative Net Retention Rate (straight product — no cascade)
    EXP(SUM(LN(CASE WHEN net_survival > 0 THEN net_survival ELSE NULL END)) OVER (
      PARTITION BY report_date, Product_Name_Final, Country_Segment
...
avg_metrics AS (
  SELECT
    ...
    -- Cohort_Size_Avg: simple average of last 3 days
    CASE
      WHEN MOD(DATE_DIFF(cr.report_date, DATE('2025-04-09'), DAY), 3) = 0
      THEN AVG(cr.Cohort_Size) OVER (
        PARTITION BY cr.Product_Name_Final, cr.App_Name, cr.Billing_Cycle
        ORDER BY cr.report_date
        ROWS BETWEEN 2 PRECEDING AND CURRENT ROW
      )
    END AS Cohort_Size_Avg,

    -- Approval_Rate_Avg: weighted average by Cohort_Size
    CASE
      WHEN MOD(DATE_DIFF(cr.report_date, DATE('2025-04-09'), DAY), 3) = 0
      THEN SAFE_DIVIDE(
    ...
    -- Attempt_Rate_Avg: weighted average by Cohort_Size
    CASE
      WHEN MOD(DATE_DIFF(cr.report_date, DATE('2025-04-09'), DAY), 3) = 0
      THEN SAFE_DIVIDE(
    ...
    -- Net_Retention_Rate_Avg: weighted average by Cohort_Size
    CASE
      WHEN MOD(DATE_DIFF(cr.report_date, DATE('2025-04-09'), DAY), 3) = 0
      THEN SAFE_DIVIDE(
...
SELECT
  ...
  cr.Entity_Name,
  cr.App_Name,                -- Now 'CT-JP' / 'CT-Non-JP' for CT products
  cr.Trial_Type,
  cr.Trial_Period,
  ...
  -- Churn Rate (per-BC, uses Approved_Users at anchor, Cohort_Size otherwise)
  CASE
  ...
  -- ARPU Retention Rates from Final_Table (joined via suffixed plan name, no country key)
  ...
  -- 3-Day Rolling Averages (populated every 3rd day from 2025-04-09)
  cr.Cohort_Size_Avg,
...
-- ARPU lookup: country suffix baked into Plan_Name, no separate country join
LEFT JOIN (
  SELECT
    Reporting_Date,
    Plan_Name,
