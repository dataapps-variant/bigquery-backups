-- job_id: scheduled_query_6a9d3952-0000-2852-8e73-089e082668b0
-- user: dataapps@variant.net
-- statement_type: CREATE_TABLE_AS_SELECT
-- created: 2026-08-03T10:30:03.140000+00:00
-- started: 2026-08-03T10:30:03.455000+00:00
-- ended: 2026-08-03T10:31:01.513000+00:00

-- =====================================================
-- CWC CRYSTAL BALL TABLE CREATION SCRIPT
-- AFID Level Bifurcation Added (INTEGER type)
-- NULL Cascade Logic from BC0 (BC1 for NT Plans)
-- Single Sale Removed from All Calculations
-- Crystal Ball Methodology Added: Day_0, SOT_Ratio, CB_User, CB_Value
-- Updated: Table name changed to CWC.CWC_Crystal_Ball
-- Updated: Rebill_users and Rebill_value removed
-- Updated: Net_LTV_Discounted column added
-- Updated: SOT table join uses Channel instead of AFID
-- =====================================================

CREATE OR REPLACE TABLE `variant-finance-data-project.CWC.CWC_Crystal_Ball` AS
...
-- =====================================================
-- DAY_0 METRICS CALCULATION (NEW - CRYSTAL BALL)
...
-- =====================================================
-- SOT_RATIO LOOKUP (NEW - CRYSTAL BALL)
-- UPDATED: Join on Channel instead of AFID
-- =====================================================
sot_ratio_lookup AS (
  SELECT 
    bedc.report_date,
  ...
  FROM bc_end_date_calculation bedc
  LEFT JOIN `variant-finance-data-project.CWC.CWC_15K_SOT_Ratio` sot
    ...
    AND bedc.AFID = sot.Channel  -- CHANGED: AFID to Channel
...
-- =====================================================
-- CORE METRICS ASSEMBLY (UPDATED - CRYSTAL BALL)
-- REMOVED: Rebill_users and Rebill_value calculations
-- =====================================================
core_metrics AS (
  SELECT 
    bedc.*,
    ...
    -- Day_0 metrics (on-time only) - CRYSTAL BALL
    CASE 
    ...
    -- SOT_Ratio - CRYSTAL BALL
    sot.final_sot_ratio as clean_sot_ratio,
    
    -- Crystal Ball metrics (CB_User and CB_Value) - CRYSTAL BALL
    COALESCE(
      SAFE_DIVIDE(
        CASE 
          WHEN (bedc.Trial_Type = 'NT' AND bedc.billing_cycle IN (0, 1)) OR bedc.billing_cycle = 0 
...
-- =====================================================
-- RATIOS AND BASIC CALCULATIONS (UPDATED - CRYSTAL BALL)
...
calculated_ratios AS (
  SELECT 
    ...
    -- Base churn rate calculation (USES CB_User - CRYSTAL BALL)
    CASE 
  ...
  LEFT JOIN `variant-finance-data-project.CWC.Refund_Table` rt
...
-- =====================================================
-- RETENTION RATE CALCULATION (STEP-BY-STEP) (UPDATED - CRYSTAL BALL)
    ...
    CASE 
      ...
      WHEN rs0.billing_cycle = 1 AND rs0.Trial_Type != 'NT' THEN 
        CASE 
          ...
          -- Check if current BC has CB_User < minimum_rebill_users → set to 0 (CRYSTAL BALL)
...
-- =====================================================
-- FINAL CALCULATIONS (UPDATED - CRYSTAL BALL)
...
final_calculations AS (
  SELECT 
    ...
    -- Refund_ratio
    rs4.base_refund_ratio as final_refund_ratio,
    ...
    -- Net_ARPU_Discounted (USES CB_Value / CB_User - CRYSTAL BALL)
    CASE 
      WHEN rs4.final_retention_rate IS NULL THEN NULL
      WHEN rs4.final_retention_rate = 0 THEN 0.0
      ELSE COALESCE(
        SAFE_DIVIDE(rs4.clean_cb_value + COALESCE(rs4.clean_ss_value, 0), NULLIF(rs4.clean_cb_user, 0)) * (rs4.final_retention_rate - rs4.base_refund_ratio),
        0.0
      )
    END as final_net_arpu_discounted,
    
    -- Net_LTV_Discounted (NEW)
    CASE 
      WHEN rs4.final_retention_rate IS NULL THEN NULL
      WHEN rs4.final_retention_rate = 0 THEN 0.0
      ELSE COALESCE(
...
-- =====================================================
-- FINAL OUTPUT - With Crystal Ball Columns
-- REMOVED: Rebill_users and Rebill_value
-- NEW: Net_LTV_Discounted
-- =====================================================
SELECT 
  ...
  -- User & Revenue Metrics (WITH CRYSTAL BALL COLUMNS)
  ...
  CASE 
    WHEN fop.billing_cycle = 4 AND fop.final_net_arpu_discounted > 0 THEN
      SUM(fop.final_net_arpu_discounted) OVER (
        PARTITION BY fop.report_date, fop.Product_Name_Final, fop.Country_Code, fop.AFID
      )
    ELSE NULL
  END as Net_BC4_ARPU,
...
-- =====================================================
-- SCRIPT COMPLETE - CWC CRYSTAL BALL TABLE
-- 
-- FINAL CHANGES APPLIED:
-- ✅ Removed rebill_metrics CTE
-- ✅ Removed Rebill_users and Rebill_value from core_metrics
-- ✅ Removed Rebill_users and Rebill_value from output
-- ✅ Fixed SOT table join: AFID = Channel
-- ✅ Added Net_LTV_Discounted = Net_ARPU_Discounted - Recent_CAC
-- ✅ Added Active_Inactive column from Active_Plans_6M lookup
-- 
-- OUTPUT TABLE: variant-finance-data-project.CWC.CWC_Crystal_Ball
-- =====================================================
