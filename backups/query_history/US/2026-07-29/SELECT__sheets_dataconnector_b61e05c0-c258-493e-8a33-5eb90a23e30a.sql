-- job_id: sheets_dataconnector_b61e05c0-c258-493e-8a33-5eb90a23e30a
-- user: dev@variant.net
-- statement_type: SELECT
-- created: 2026-07-29T14:48:13.743000+00:00
-- started: 2026-07-29T14:48:13.794000+00:00
-- ended: 2026-07-29T14:48:14.024000+00:00
...
SELECT * FROM `variant-finance-data-project.ICARUS_Multi.Final_Table_Formatted`
  ...
  WHERE ((`Reporting_Date` = '2026-7-28') AND ((UPPER(`Cohort`) NOT IN (UPPER('7K_30D'))) OR (UPPER(`Cohort`) IS NULL)) AND ((UPPER(`Table_Type`) NOT IN (UPPER('Crystal Ball'))) OR (UPPER(`Table_Type`) IS NULL)) AND ((UPPER(`Active_Inactive`) NOT IN (UPPER('Active'))) OR (UPPER(`Active_Inactive`) IS NULL)) AND (UPPER(`Metric_Name`) IN (UPPER('06. Refund Rate'))))
  LIMIT 100000
