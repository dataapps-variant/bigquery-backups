-- job_id: scheduled_query_6a88dfe8-0000-2548-9a5c-30fd380f88f8
-- user: dataapps@variant.net
-- statement_type: SCRIPT
-- created: 2026-07-29T08:45:03.289000+00:00
-- started: 2026-07-29T08:45:03.349000+00:00
-- ended: 2026-07-29T08:46:06.650000+00:00
...
IF days_to_process > 0 THEN
  ...
  -- =====================================================
  INSERT INTO `variant-finance-data-project.Icarus_Cohort.IC_7K_30D_Crystal_Ball`
...
master_combinations AS (
  ...
  WHERE rd.report_date >= apc.First_Date_of_Sale     -- ✅ Filter by plan's own start date
),
...
sot_ratio_lookup AS (
  ...
  LEFT JOIN `variant-finance-data-project.Icarus_Cohort.IC_7K_SOT_Ratio` sot
    ...
    AND bedc.AFID = sot.Channel                       -- ✅ CHANGED TO Channel
...
-- =====================================================
-- T30D NEW USERS CALCULATION (NEW)
-- =====================================================
...
calculated_ratios AS (
  SELECT 
    ...
    -- Base churn rate calculation (UPDATED: Uses CB_User instead of Rebill_users)
    CASE 
...
-- CB_User < minimum_rebill_users sets retention to 0 at non-trigger BCs
-- =====================================================
retention_step_0 AS (
  SELECT 
    cr.*,
    ...
    CASE 
      ...
      WHEN rs0.billing_cycle = 1 AND rs0.Trial_Type != 'NT' THEN 
        CASE 
          ...
          -- Check if current BC has CB_User < minimum_rebill_users → set to 0
...
-- UPDATED: Net_ARPU_Discounted now uses CB_Value/CB_User
-- =====================================================
final_calculations AS (
  SELECT 
    ...
    -- ARPU_Discounted (NEW - MAKE SURE THIS IS HERE)
    CASE 
      WHEN rs12.final_retention_rate IS NULL THEN NULL
    ...
    -- Net_ARPU_Discounted (UPDATED: Now uses CB_Value/CB_User instead of Rebill_value/Rebill_users)
    CASE 
      WHEN rs12.final_retention_rate IS NULL THEN NULL
      WHEN rs12.final_retention_rate = 0 THEN 0.0
      ELSE COALESCE(
...
-- =====================================================
-- FINAL OUTPUT - Crystal Ball Version with AFID
-- =====================================================
SELECT 
  ...
  fop.final_arpu_discounted as ARPU_Discounted,            -- ✅ NEW LINE ADDED
...
-- =====================================================
-- SCRIPT COMPLETE - IC_7K_30D_CRYSTAL_BALL TABLE
-- Table: variant-finance-data-project.Icarus_Cohort.IC_7K_30D_Crystal_Ball
-- 
-- CONFIGURATION:
-- - Cohort Size: 7,000 users
-- - Max Billing Cycles: 12 (BC0 through BC12)
-- - Retry Engine Period: 1 day
-- - Minimum Rebill Count: 25
-- - Limiting Days: 30
-- - Recent CAC Days: 7
-- 
-- CRYSTAL BALL FEATURES:
-- ✅ AFID dimension maintained (Product × Country × AFID level)
-- ✅ Day_0_user and Day_0_values (on-time rebills)
-- ✅ SOT_Ratio lookup from CWC.CWC_15K_SOT_Ratio
-- ✅ CB_User and CB_Value (Crystal Ball projections)
-- ✅ T30D_New_Users metric
-- ✅ Removed Rebill_users and Rebill_value columns
-- ✅ Churn rate uses CB_User instead of Rebill_users
-- ✅ Retention logic uses CB_User thresholds
-- ✅ Net_ARPU_Discounted uses CB_Value/CB_User
-- ✅ Removed Net_BC4_ARPU windowed calculation
-- ✅ Configuration updated: minimum_rebill_users=25, retry_engine_period=1
-- =====================================================
  -- =====================================================
  -- ▲▲▲ END OF YOUR QUERY ▲▲▲
  -- =====================================================
