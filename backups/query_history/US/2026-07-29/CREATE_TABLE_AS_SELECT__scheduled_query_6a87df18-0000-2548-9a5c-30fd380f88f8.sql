-- job_id: scheduled_query_6a87df18-0000-2548-9a5c-30fd380f88f8
-- user: dataapps@variant.net
-- statement_type: CREATE_TABLE_AS_SELECT
-- created: 2026-07-29T08:00:15.002000+00:00
-- started: 2026-07-29T08:00:15.340000+00:00
-- ended: 2026-07-29T08:00:16.722000+00:00

CREATE OR REPLACE TABLE `variant-finance-data-project.R100.FS_new_trailing100` AS
WITH 
...
-- Step 1B: Find products from other apps that map to FS
cross_app_products AS (
    SELECT DISTINCT
        App_Name,
        Product_Name
    FROM `variant-finance-data-project.Sticky_Data.R100_BQ_Updated_Product_Name`
    WHERE Updated_App_Name = 'FS'
        AND App_Name != 'FS'
),

base_data AS (
    -- Step 1A: First data source - Direct FS data from Sticky table
    SELECT 
    ...
    -- Filter for last 100 days from current date and App_Name = "FS"
    ...
    -- Step 1B: Additional data from other apps that map to FS
    SELECT 
        -- Day Charged (past 100 days) - Date_of_Sale filtered for last 100 days
...
-- Filter to keep only records where APP_SHORT_NAME = "FS" after lookup
WHERE COALESCE(upd.Updated_App_Name, base.APP_SHORT_NAME) = 'FS'
