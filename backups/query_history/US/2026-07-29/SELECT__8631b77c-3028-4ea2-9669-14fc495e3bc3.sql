-- job_id: 8631b77c-3028-4ea2-9669-14fc495e3bc3
-- user: 1041741270489-compute@developer.gserviceaccount.com
-- statement_type: SELECT
-- created: 2026-07-29T12:17:20.011000+00:00
-- started: 2026-07-29T12:17:20.207000+00:00
-- ended: 2026-07-29T12:17:23.946000+00:00


    WITH order_data AS (
    SELECT report_date, afid, t_type, decline_reason, count(*) AS count FROM(
    SELECT * FROM (
    SELECT COALESCE(CASE WHEN AF.JF_App_Name IS NOT NULL AND TRIM(AF.JF_App_Name) != '' AND UPPER(TRIM(AF.JF_App_Name)) != 'COMBINED' THEN AF.JF_App_Name ELSE NULL END, CAM.App_Name, 'JF') AS app, o.Order_Id AS order_id, Bill_Email AS email_address, Date_of_Sale AS report_date, o.AFID AS afid, CASE WHEN Billing_Cycle = '0' THEN 'CIT' ELSE 'MIT' END AS t_type, regexp_replace(o.Decline_Reason, r'(Unable to authorize payment for payment method ([0-9]*); Transaction Id: ([0-9]*)\.)|(Transaction ID = ([A-Z0-9]*))|(REFID:([0-9]*))', '') AS decline_reason FROM `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_JF` AS o
...
ORDER BY subs_count_t30d DESC) AS app_ranks ON app_ranks.app = 'RL'
...
WHERE Final_Order_Status <> 7 AND Billing_Cycle = '0' AND App_Name IN ('RL') AND Trial_Type <> 'SS' AND AFID <> '' AND Date_of_Sale >= current_date - 30 GROUP BY AFID) AS d) AS z ON d.afid = z.afid
