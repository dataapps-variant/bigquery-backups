-- job_id: sheets_dataconnector_41ad1848-98ef-48d9-b9a0-6a7e4f614d67
-- user: dev@variant.net
-- statement_type: SELECT
-- created: 2026-07-29T14:48:13.866000+00:00
-- started: 2026-07-29T14:48:13.927000+00:00
-- ended: 2026-07-29T14:48:14.130000+00:00
...
SELECT * FROM `variant-finance-data-project.ICARUS_Multi.Final_Table_Formatted`
  ...
  WHERE ((`Reporting_Date` = '2026-7-28') AND ((UPPER(`Cohort`) NOT IN (UPPER('7K_30D'))) OR (UPPER(`Cohort`) IS NULL)) AND ((UPPER(`Table_Type`) NOT IN (UPPER('Crystal Ball'))) OR (UPPER(`Table_Type`) IS NULL)) AND ((UPPER(`Active_Inactive`) NOT IN (UPPER('Active'))) OR (UPPER(`Active_Inactive`) IS NULL)) AND (UPPER(`Metric_Name`) IN (UPPER('05. Gross Retention Rate'))))
  LIMIT 100000
