-- job_id: e1278487-c00b-46af-ae6a-322e1c80f37a
-- user: 1041741270489-compute@developer.gserviceaccount.com
-- statement_type: SELECT
-- created: 2026-07-29T12:20:00.984000+00:00
-- started: 2026-07-29T12:20:01.248000+00:00
-- ended: 2026-07-29T12:20:03.724000+00:00


    WITH order_data AS (
    SELECT report_date, afid, t_type, decline_reason, count(*) AS count FROM(
    SELECT * FROM (
    SELECT sjo.Order_Id AS order_id, Bill_Email AS email_address, Date_of_Sale AS report_date, AFID AS afid, CASE WHEN Billing_Cycle = '0' THEN 'CIT' ELSE 'MIT' END AS t_type, regexp_replace(sjo.Decline_Reason, r'(Unable to authorize payment for payment method ([0-9]*); Transaction Id: ([0-9]*)\.)|(Transaction ID = ([A-Z0-9]*))|(REFID:([0-9]*))', '') AS decline_reason FROM `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_CT` AS sjo
...
WHERE Final_Order_Status <> 7 AND Billing_Cycle = '0' AND App_Name IN ('EN') AND Trial_Type <> 'SS' AND AFID <> '' AND Date_of_Sale >= current_date - 30 GROUP BY AFID) AS d) AS z ON d.afid = z.afid
