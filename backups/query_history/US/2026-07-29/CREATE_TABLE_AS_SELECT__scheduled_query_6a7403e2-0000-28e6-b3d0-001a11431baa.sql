-- job_id: scheduled_query_6a7403e2-0000-28e6-b3d0-001a11431baa
-- user: dataapps@variant.net
-- statement_type: CREATE_TABLE_AS_SELECT
-- created: 2026-07-29T09:00:15.252000+00:00
-- started: 2026-07-29T09:00:15.515000+00:00
-- ended: 2026-07-29T09:04:30.852000+00:00


CREATE OR REPLACE TABLE `variant-finance-data-project.Icarus_Spend_Country_AFID.SCA_7K_SOT_Ratio_App_Level` AS
...
-- =====================================================
-- APP × SPEND_COUNTRY_CODE_AFID LIST (from SCA_Plan_List)
-- Compute App_Name with country suffix to match output format
-- =====================================================
app_sca_list AS (
  SELECT DISTINCT
    CASE 
  ...
  FROM `variant-finance-data-project.Icarus_Spend_Country_AFID.SCA_Plan_List`
...
master_combinations AS (
  SELECT 
    ...
    asl.App_Name,
    asl.Country_Code,
    asl.Spend_Country_Code_AFID,
    bcr.billing_cycle,
    cfg.cohort_size
  FROM report_dates rd
  CROSS JOIN app_sca_list asl
  CROSS JOIN billing_cycle_range bcr
  CROSS JOIN config cfg
),

-- =====================================================
-- SUBSCRIPTION COHORT: Top 7000 users per app+SCA+BC
-- Match SCA source: base.Spend_Country_Code_AFID = mc.Spend_Country_Code_AFID
-- =====================================================
subscription_cohort AS (
  SELECT 
    ...
    base.Delay_days_SOT,
    ROW_NUMBER() OVER (
      PARTITION BY mc.report_date, mc.App_Name, mc.Country_Code, mc.Spend_Country_Code_AFID, mc.billing_cycle
...
top_cohort AS (
  SELECT 
    ...
    billing_cycle, Updated_Cust_ID, Delay_days_SOT
...
SELECT 
  ...
  COALESCE(
    SAFE_DIVIDE(COALESCE(m.sot_users, 0), NULLIF(COALESCE(m.subscription_users, 0), 0)),
    0
  ) AS SOT_Ratio
...
ORDER BY mc.report_date DESC, mc.App_Name, mc.Country_Code, mc.Spend_Country_Code_AFID, mc.billing_cycle;
