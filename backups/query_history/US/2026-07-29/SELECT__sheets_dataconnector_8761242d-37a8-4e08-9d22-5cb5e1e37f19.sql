-- job_id: sheets_dataconnector_8761242d-37a8-4e08-9d22-5cb5e1e37f19
-- user: dev@variant.net
-- statement_type: SELECT
-- created: 2026-07-29T14:48:13.510000+00:00
-- started: 2026-07-29T14:48:13.591000+00:00
-- ended: 2026-07-29T14:48:13.809000+00:00
...
SELECT * FROM `variant-finance-data-project.ICARUS_Multi.Final_Table_Formatted`
  ...
  WHERE ((`Reporting_Date` = '2026-7-28') AND ((UPPER(`Cohort`) NOT IN (UPPER('7K'))) OR (UPPER(`Cohort`) IS NULL)) AND ((UPPER(`Table_Type`) NOT IN (UPPER('Crystal Ball'))) OR (UPPER(`Table_Type`) IS NULL)) AND ((UPPER(`Active_Inactive`) NOT IN (UPPER('Active'))) OR (UPPER(`Active_Inactive`) IS NULL)) AND (UPPER(`Metric_Name`) IN (UPPER('05. Gross Retention Rate'))))
  LIMIT 100000
