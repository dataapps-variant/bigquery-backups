-- job_id: job_s0nxfqlGgS0EXiRZXsKK2yD2IuqL
-- user: dataapps@variant.net
-- statement_type: CREATE_TABLE_AS_SELECT
-- created: 2026-07-29T13:48:20.058000+00:00
-- started: 2026-07-29T13:48:20.446000+00:00
-- ended: 2026-07-29T13:48:24.203000+00:00

CREATE OR REPLACE TABLE `variant-finance-data-project.Extension_Scenario_Retention.Dim_Table_Extension_Retention_Scenario` AS
...
cohort_priority AS (
  SELECT Plan_Name, Cohort, Active_Inactive, Billing_Cycle, retention_rate,
    ROW_NUMBER() OVER (
...
last_icarus_bc AS (
  SELECT Plan_Name,
    ...
    ARRAY_AGG(retention_rate ORDER BY Billing_Cycle DESC LIMIT 1)[OFFSET(0)] AS last_retention
  FROM best_icarus
  GROUP BY Plan_Name
),
...
combined AS (
  SELECT
    i.Input_ID, i.Entity, i.Retention_Curve,
  ...
  LEFT JOIN best_icarus ic
    ON ic.Plan_Name = i.Retention_Curve AND ic.Billing_Cycle = b.bc
...
final_calc AS (
  SELECT
    Entity, Retention_Curve, Billing_Cycle, Input_ID,
    icarus_retention, icarus_max_bc, icarus_last_retention, extension_churn, icarus_source,
    CASE
...
SELECT
  Entity,
  Retention_Curve AS Curve_Name,
  Billing_Cycle AS bc_level,
  ROUND(final_retention, 4) AS retention_pct,
  Source,
