-- job_id: sheets_dataconnector_7178ccff-9323-472f-a19a-a35034a17e47
-- user: dev@variant.net
-- statement_type: SELECT
-- created: 2026-07-30T11:24:38.457000+00:00
-- started: 2026-07-30T11:24:38.520000+00:00
-- ended: 2026-07-30T11:24:38.693000+00:00
...
SELECT * FROM `variant-finance-data-project.ICARUS_Multi.Final_Table_Formatted`
  ...
  WHERE ((`Reporting_Date` = '2026-7-29') AND ((UPPER(`Cohort`) NOT IN (UPPER('7K_30D'))) OR (UPPER(`Cohort`) IS NULL)) AND ((UPPER(`Active_Inactive`) NOT IN (UPPER('Active'))) OR (UPPER(`Active_Inactive`) IS NULL)) AND ((UPPER(`Table_Type`) NOT IN (UPPER('Crystal Ball'))) OR (UPPER(`Table_Type`) IS NULL)) AND (UPPER(`Metric_Name`) IN (UPPER('05. Gross Retention Rate'))))
  LIMIT 100000
