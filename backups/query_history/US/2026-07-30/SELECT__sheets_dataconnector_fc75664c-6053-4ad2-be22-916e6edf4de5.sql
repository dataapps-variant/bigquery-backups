-- job_id: sheets_dataconnector_fc75664c-6053-4ad2-be22-916e6edf4de5
-- user: dev@variant.net
-- statement_type: SELECT
-- created: 2026-07-30T14:02:22.565000+00:00
-- started: 2026-07-30T14:02:22.622000+00:00
-- ended: 2026-07-30T14:02:22.723000+00:00
...
SELECT * FROM `variant-finance-data-project.ICARUS_Multi.Final_Table_Formatted`
  ...
  WHERE ((`Reporting_Date` = '2026-7-29') AND ((UPPER(`Cohort`) NOT IN (UPPER('7K'))) OR (UPPER(`Cohort`) IS NULL)) AND ((UPPER(`Table_Type`) NOT IN (UPPER('Crystal Ball'))) OR (UPPER(`Table_Type`) IS NULL)) AND ((UPPER(`Active_Inactive`) NOT IN (UPPER('Inactive'))) OR (UPPER(`Active_Inactive`) IS NULL)) AND (UPPER(`Metric_Name`) IN (UPPER('06. Refund Rate'))))
  LIMIT 100000
