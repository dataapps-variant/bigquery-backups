-- job_id: scheduled_query_6a888ba2-0000-274e-a79b-14223bc5fb3e
-- user: dataapps@variant.net
-- statement_type: SCRIPT
-- created: 2026-07-30T08:45:05.697000+00:00
-- started: 2026-07-30T08:45:05.738000+00:00
-- ended: 2026-07-30T08:46:23.468000+00:00
...
plan_list_ranked AS (
  SELECT 
    ...
    Trial_Price,
    Regular_Price,
    First_Date_of_Sale,    -- ADDED: pull from IC_Plan_List_AFID
    ROW_NUMBER() OVER (
...
plan_list_first_record AS (
  SELECT 
    ...
    Currency,
   Regular_Price,
    First_Date_of_Sale    -- ADDED: carry through
...
aggregated_plan_list AS (
  SELECT 
    ...
    fr.Regular_Price,
    tp.Trial_Price,
    fr.First_Date_of_Sale    -- ADDED: expose to downstream CTEs
...
enriched_master AS (
  SELECT 
    ...
    apl.Trial_Price,
    apl.Regular_Price,
    apl.First_Date_of_Sale    -- ADDED: needed for filter below
  ...
  WHERE mc.report_date >= apl.First_Date_of_Sale    -- ADDED: drop pre-launch dates
  ...
  SELECT rs11.*,
    CASE WHEN rs11.step_11_retention IS NOT NULL THEN rs11.step_11_retention
      WHEN rs11.billing_cycle = 12 THEN 
        CASE WHEN LAG(rs11.step_11_retention) OVER (PARTITION BY rs11.report_date, rs11.Product_Name_Final, rs11.Country_Code, rs11.AFID_Suffix ORDER BY rs11.billing_cycle) IS NULL THEN NULL
          WHEN rs11.clean_cb_user < rs11.minimum_rebill_users THEN 0.0
          ELSE LAG(rs11.step_11_retention) OVER (PARTITION BY rs11.report_date, rs11.Product_Name_Final, rs11.Country_Code, rs11.AFID_Suffix ORDER BY rs11.billing_cycle) * (1 - rs11.base_churn_rate) END
...
final_calculations AS (
  SELECT rs12.*,
    ...
    CASE WHEN rs12.final_retention_rate IS NULL THEN NULL WHEN rs12.final_retention_rate = 0 THEN 0.0
      ELSE COALESCE(SAFE_DIVIDE(rs12.clean_cb_value, NULLIF(rs12.clean_cb_user, 0)) * rs12.final_retention_rate, 0.0)
        + COALESCE(SAFE_DIVIDE(rs12.clean_ss_value, NULLIF(rs12.clean_subscription_users, 0)), 0) END as final_arpu_discounted,
    CASE WHEN rs12.final_retention_rate IS NULL THEN NULL WHEN rs12.final_retention_rate = 0 THEN 0.0
      ELSE COALESCE(SAFE_DIVIDE(rs12.clean_cb_value, NULLIF(rs12.clean_cb_user, 0)) * (rs12.final_retention_rate - rs12.base_refund_ratio), 0.0)
        + COALESCE(SAFE_DIVIDE(rs12.clean_ss_value, NULLIF(rs12.clean_subscription_users, 0)), 0) END as final_net_arpu_discounted
