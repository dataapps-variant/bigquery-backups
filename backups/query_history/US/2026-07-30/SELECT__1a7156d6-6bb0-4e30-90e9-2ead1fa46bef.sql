-- job_id: 1a7156d6-6bb0-4e30-90e9-2ead1fa46bef
-- user: 1041741270489-compute@developer.gserviceaccount.com
-- statement_type: SELECT
-- created: 2026-07-30T20:18:54.408000+00:00
-- started: 2026-07-30T20:18:54.526000+00:00
-- ended: 2026-07-30T20:18:55.722000+00:00


    WITH past_orders AS (
  SELECT 'CT - US' AS app, DATE_TRUNC(DATETIME(time_stamp), HOUR) AS date_trunc, DATE(DATE_TRUNC(DATETIME(time_stamp), HOUR)) AS date_value, EXTRACT(HOUR FROM DATE_TRUNC(DATETIME(time_stamp), HOUR)) AS num_hour, COUNT(1) AS count FROM `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_CT_Incremental` AS d
  ...
  UNION ALL
  SELECT 'CT - JP' AS app, DATE_TRUNC(DATETIME(time_stamp), HOUR) AS date_trunc, DATE(DATE_TRUNC(DATETIME(time_stamp), HOUR)) AS date_value, EXTRACT(HOUR FROM DATE_TRUNC(DATETIME(time_stamp), HOUR)) AS num_hour, COUNT(1) AS count FROM `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_CT_Incremental` AS d
  LEFT JOIN `variant-finance-data-project.Sticky_Data.Sticky_Dim_AFID` AS a ON d.AFID = a.AFID
  WHERE DATETIME(time_stamp) >= DATETIME_TRUNC(DATETIME_SUB(datetime(current_timestamp, 'America/New_York'), INTERVAL 720 HOUR), HOUR) AND EXTRACT(HOUR FROM DATE_TRUNC(DATETIME(time_stamp), HOUR)) = EXTRACT(HOUR FROM DATETIME_SUB(datetime(current_timestamp, 'America/New_York'), INTERVAL 1 HOUR))AND campaign_id IN (SELECT Campaign_Id FROM `variant-finance-data-project.Sticky_Data.Sticky_Dim_Campaign` WHERE App_Name = 'CT') AND COALESCE(a.Country_Code, d.billing_country) = 'JP'
  GROUP BY DATE_TRUNC(DATETIME(time_stamp), HOUR), DATE(DATE_TRUNC(DATETIME(time_stamp), HOUR)), EXTRACT(HOUR FROM DATE_TRUNC(DATETIME(time_stamp), HOUR))
  UNION ALL
  SELECT 'EN' AS app, DATE_TRUNC(DATETIME(time_stamp), HOUR) AS date_trunc, DATE(DATE_TRUNC(DATETIME(time_stamp), HOUR)) AS date_value, EXTRACT(HOUR FROM DATE_TRUNC(DATETIME(time_stamp), HOUR)) AS num_hour, COUNT(1) AS count FROM `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_CT_Incremental` 
