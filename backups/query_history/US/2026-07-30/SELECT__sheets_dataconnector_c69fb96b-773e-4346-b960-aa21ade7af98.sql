-- job_id: sheets_dataconnector_c69fb96b-773e-4346-b960-aa21ade7af98
-- user: dev@variant.net
-- statement_type: SELECT
-- created: 2026-07-30T14:06:39.630000+00:00
-- started: 2026-07-30T14:06:39.706000+00:00
-- ended: 2026-07-30T14:06:39.804000+00:00
...
SELECT * FROM `variant-finance-data-project.ICARUS_Multi.Final_Table_Formatted`
  ...
  WHERE ((`Reporting_Date` = '2026-7-29') AND ((UPPER(`Cohort`) NOT IN (UPPER('7K'))) OR (UPPER(`Cohort`) IS NULL)) AND ((UPPER(`Table_Type`) NOT IN (UPPER('Crystal Ball'))) OR (UPPER(`Table_Type`) IS NULL)) AND ((UPPER(`Active_Inactive`) NOT IN (UPPER('Active'))) OR (UPPER(`Active_Inactive`) IS NULL)) AND (UPPER(`Metric_Name`) IN (UPPER('05. Gross Retention Rate'))))
  LIMIT 100000
