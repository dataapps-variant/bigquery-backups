-- job_id: job_w4xfALXMHtqXuHzJKQcXs2PrdUDp
-- user: dataapps@variant.net
-- statement_type: None
-- created: 2026-07-30T12:20:50.493000+00:00
-- started: 2026-07-30T12:20:50.725000+00:00
-- ended: 2026-07-30T12:20:50.725000+00:00
...
-- Step 1: Unpivot ALL Icarus retention data (untouched)
icarus_long AS (
  SELECT
...
-- Step 2: Priority waterfall per BC (untouched)
cohort_priority AS (
  SELECT
...
best_icarus AS (
  SELECT
    Plan_Name, Entity_Name,
    CONCAT(Active_Inactive, ' ', Cohort) AS icarus_source,
    bc_level, retention_rate
  FROM cohort_priority WHERE rn = 1
),

-- Step 3: Get all unique plans from Icarus
all_icarus_plans AS (
  SELECT DISTINCT Plan_Name, Entity_Name
  FROM best_icarus
),

-- Step 4: Last available BC per plan from Icarus
last_icarus_bc AS (
  SELECT Plan_Name,
...
-- Step 5: Lookup Extension Scenario from R100_Inputs_Full
-- Match Icarus Plan_Name to R100 Retention_Curve column
-- Pick one extension per Retention_Curve
...
-- Step 6: Unpivot dim_table extension churn data
extension_parsed AS (
  SELECT
    Curve_Name,
    bc_level,
    retention_pct AS churn_rate
  FROM `variant-finance-data-project.Extension_Retentions_Scenarios.Extension_Retentions_Scenarios`
),

-- Step 7: Generate BC range 0-36
bc_range AS (
...
combined AS (
  ...
  LEFT JOIN extension_parsed ext_churn
    ON ext_churn.Curve_Name = em.Extension_Scenarios_Retentions
    AND ext_churn.bc_level = b.bc
),

-- Step 9: Calculate retention
final_calc AS (
  SELECT
    Entity_Name,
    Plan_Name,
    bc_level,
    CASE
      -- Icarus has data: use it directly
      WHEN icarus_retention IS NOT NULL THEN icarus_retention

      -- Beyond Icarus + has extension mapping: calculate using churn
