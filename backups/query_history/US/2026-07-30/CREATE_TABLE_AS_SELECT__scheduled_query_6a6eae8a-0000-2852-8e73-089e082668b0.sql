-- job_id: scheduled_query_6a6eae8a-0000-2852-8e73-089e082668b0
-- user: dataapps@variant.net
-- statement_type: CREATE_TABLE_AS_SELECT
-- created: 2026-07-30T08:00:04.125000+00:00
-- started: 2026-07-30T08:00:04.489000+00:00
-- ended: 2026-07-30T08:00:05.851000+00:00

CREATE OR REPLACE TABLE `variant-finance-data-project.R100.JF_new_trailing100` AS
WITH 


-- Step 1B: Find products from other apps that map to JF
cross_app_products AS (
    SELECT DISTINCT
        App_Name,
        Product_Name
    FROM `variant-finance-data-project.Sticky_Data.R100_BQ_Updated_Product_Name`
    WHERE Updated_App_Name = 'JF'
        AND App_Name != 'JF'
),

base_data AS (
    -- Step 1A: First data source - Direct JF data from Sticky table
    SELECT 
    ...
    -- Filter for last 100 days from current date and App_Name = "JF"
    ...
    -- Step 1B: Additional data from other apps that map to JF
    SELECT 
        -- Day Charged (past 100 days) - Date_of_Sale filtered for last 100 days
...
-- Filter to keep only records where APP_SHORT_NAME = "JF" after lookup
WHERE COALESCE(upd.Updated_App_Name, base.APP_SHORT_NAME) = 'JF'
