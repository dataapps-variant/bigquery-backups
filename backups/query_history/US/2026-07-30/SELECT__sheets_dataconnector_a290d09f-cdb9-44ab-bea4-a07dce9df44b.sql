-- job_id: sheets_dataconnector_a290d09f-cdb9-44ab-bea4-a07dce9df44b
-- user: dev@variant.net
-- statement_type: SELECT
-- created: 2026-07-30T11:24:38.378000+00:00
-- started: 2026-07-30T11:24:38.465000+00:00
-- ended: 2026-07-30T11:24:38.675000+00:00
...
SELECT * FROM `variant-finance-data-project.ICARUS_Multi.Final_Table_Formatted`
  ...
  WHERE ((`Reporting_Date` = '2026-7-29') AND ((UPPER(`Cohort`) NOT IN (UPPER('7K'))) OR (UPPER(`Cohort`) IS NULL)) AND ((UPPER(`Active_Inactive`) NOT IN (UPPER('Inactive'))) OR (UPPER(`Active_Inactive`) IS NULL)) AND ((UPPER(`Table_Type`) NOT IN (UPPER('Crystal Ball'))) OR (UPPER(`Table_Type`) IS NULL)) AND (UPPER(`Metric_Name`) IN (UPPER('05. Gross Retention Rate'))))
  LIMIT 100000
