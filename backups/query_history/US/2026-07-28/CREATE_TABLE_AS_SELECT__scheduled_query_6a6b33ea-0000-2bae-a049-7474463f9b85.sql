-- job_id: scheduled_query_6a6b33ea-0000-2bae-a049-7474463f9b85
-- user: dataapps@variant.net
-- statement_type: CREATE_TABLE_AS_SELECT
-- created: 2026-07-28T08:00:19.981000+00:00
-- started: 2026-07-28T08:00:20.248000+00:00
-- ended: 2026-07-28T08:00:21.666000+00:00

CREATE OR REPLACE TABLE `variant-finance-data-project.R100.PD_new_trailing100` AS
WITH 


-- Step 1B: Find products from other apps that map to PD
cross_app_products AS (
    SELECT DISTINCT
        App_Name,
        Product_Name
    FROM `variant-finance-data-project.Sticky_Data.R100_BQ_Updated_Product_Name`
    WHERE Updated_App_Name = 'PD'
        AND App_Name != 'PD'
),

base_data AS (
    -- Step 1A: First data source - Direct PD data from Sticky table
    SELECT 
    ...
    -- Filter for last 100 days from current date and App_Name = "PD"
    ...
    -- Step 1B: Additional data from other apps that map to PD
    SELECT 
        -- Day Charged (past 100 days) - Date_of_Sale filtered for last 100 days
...
-- Filter to keep only records where APP_SHORT_NAME = "PD" after lookup
WHERE COALESCE(upd.Updated_App_Name, base.APP_SHORT_NAME) = 'PD'
