-- job_id: scheduled_query_6a73bd17-0000-21ab-89ea-d4f547f40c40
-- user: dataapps@variant.net
-- statement_type: CREATE_TABLE_AS_SELECT
-- created: 2026-07-28T10:20:07.982000+00:00
-- started: 2026-07-28T10:20:08.340000+00:00
-- ended: 2026-07-28T10:27:40.359000+00:00

-- =====================================================
-- REFUND TABLE CREATION SCRIPT - CWC VERSION
-- Refund Analysis for Subscription Business Metrics
-- Table: CWC.Refund_Table
-- MODIFICATION: Added AFID dimension and changed BC range to 0-4
-- MODIFICATION 2: Added Plan_List aggregation logic and AFID from Plan_List
-- =====================================================

CREATE OR REPLACE TABLE `variant-finance-data-project.CWC.Refund_Table` AS
...
eligible_transactions AS (
  SELECT 
    ...
    base.Date_of_Sale,
    ROW_NUMBER() OVER (
      PARTITION BY bl.report_date, bl.Product_Name_Final, bl.AFID, bl.billing_cycle, bl.Country_Code
...
cohort_analysis AS (
  ...
  GROUP BY report_date, Product_Name_Final, AFID, billing_cycle, Country_Code
),
...
refund_metrics AS (
  ...
  GROUP BY fr.report_date, fr.Product_Name_Final, fr.AFID, fr.billing_cycle, fr.Country_Code, fr.Trial_Type
),
...
ORDER BY fc.report_date DESC, fc.Product_Name_Final, fc.AFID, fc.Country_Code, fc.billing_cycle;

-- =====================================================
-- SCRIPT COMPLETE - CWC VERSION WITH AFID DIMENSION
-- Table: variant-finance-data-project.CWC.Refund_Table
-- 
-- CONFIGURATION:
-- - Cohort Size: 15,000 users
-- - Max Billing Cycles: 4 (BC0 through BC4)
-- - Retry Engine Period: 30 days
-- - Minimum User Count: 100
-- - Default Regular BC Period: 30 days
-- - Refund Ratio Threshold: 0.20 (20%)
-- - Minimum Refund Count: 30
-- 
-- FEATURES:
-- ✅ AFID dimension added (Product × Country × AFID level)
-- ✅ Plan_List aggregation logic to handle duplicate trial types
-- ✅ AFID sourced from aggregated Plan_List
-- ✅ Country_Code separation (JP/Non-JP for CT products)
-- ✅ Proper cohort selection with country and AFID filtering
-- ✅ Refund calculation with date windows
-- ✅ Exchange rate conversion to USD
-- ✅ Special handling for NT products at BC0
-- ✅ Data quality filter for refund ratios
-- 
-- CHANGES IN THIS VERSION:
-- - Added 4 CTEs for Plan_List aggregation (plan_list_ranked, plan_list_first_record, 
--   plan_list_trial_price_avg, aggregated_plan_list)
-- - Changed VPU.Plan_List to CWC.Plan_List
-- - Updated product_list to use aggregated_plan_list
-- - Updated afid_list to use aggregated_plan_list (instead of transaction table)
-- - Updated enriched_master to join to aggregated_plan_list
-- =====================================================
