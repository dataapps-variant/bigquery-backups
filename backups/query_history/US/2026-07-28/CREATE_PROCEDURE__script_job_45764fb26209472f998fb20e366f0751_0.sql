-- job_id: script_job_45764fb26209472f998fb20e366f0751_0
-- user: dataapps@variant.net
-- statement_type: CREATE_PROCEDURE
-- created: 2026-07-28T18:12:38.910000+00:00
-- started: 2026-07-28T18:12:39.067000+00:00
-- ended: 2026-07-28T18:12:39.224000+00:00

CREATE OR REPLACE PROCEDURE `Icarus_Spend_Country_AFID.proc_SCA_7K_Crystal_Ball`()
BEGIN
-- =====================================================
...
IF days_to_process > 0 THEN
  ...
  -- =====================================================
  INSERT INTO `variant-finance-data-project.Icarus_Spend_Country_AFID.SCA_7K_Crystal_Ball`
...
master_combinations AS (
  SELECT 
    ...
    bcr.billing_cycle, cfg.cohort_size, cfg.limiting_days, cfg.minimum_rebill_users,
    cfg.retry_engine_period, cfg.default_regular_bc_period
...
core_metrics AS (
  ...
  LEFT JOIN day_0_metrics d0m ON bedc.report_date=d0m.report_date AND bedc.Product_Name_Final=d0m.Product_Name_Final AND bedc.Country_Code=d0m.Country_Code AND bedc.Spend_Country_Code_AFID=d0m.Spend_Country_Code_AFID AND bedc.billing_cycle=d0m.billing_cycle
  LEFT JOIN sot_ratio_lookup sot ON bedc.report_date=sot.report_date AND bedc.Product_Name_Final=sot.Product_Name_Final AND bedc.Country_Code=sot.Country_Code AND bedc.Spend_Country_Code_AFID=sot.Spend_Country_Code_AFID AND bedc.billing_cycle=sot.billing_cycle
  ...
  LEFT JOIN t30d_new_users_calc t30d ON bedc.report_date=t30d.report_date AND bedc.Product_Name_Final=t30d.Product_Name_Final AND bedc.Country_Code=t30d.Country_Code AND bedc.Spend_Country_Code_AFID=t30d.Spend_Country_Code_AFID
),
...
retention_step_0 AS (
  SELECT cr.*,
    CASE WHEN cr.billing_cycle = 0 AND cr.Trial_Type != 'NT' AND (cr.clean_subscription_users = 0 OR cr.clean_cb_user < cr.minimum_rebill_users) THEN NULL
