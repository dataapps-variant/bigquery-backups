-- job_id: sheets_dataconnector_28e0512d-4037-47f1-ae67-364adebfe73b
-- user: dev@variant.net
-- statement_type: SELECT
-- created: 2026-07-28T11:40:11.682000+00:00
-- started: 2026-07-28T11:40:11.779000+00:00
-- ended: 2026-07-28T11:40:11.913000+00:00
...
SELECT * FROM `variant-finance-data-project.ICARUS_Multi.Final_Table_Formatted`
  ...
  WHERE ((`Reporting_Date` = '2026-7-27') AND ((UPPER(`Cohort`) NOT IN (UPPER('7K_30D'))) OR (UPPER(`Cohort`) IS NULL)) AND ((UPPER(`Active_Inactive`) NOT IN (UPPER('Inactive'))) OR (UPPER(`Active_Inactive`) IS NULL)) AND ((UPPER(`Table_Type`) NOT IN (UPPER('Crystal Ball'))) OR (UPPER(`Table_Type`) IS NULL)) AND (UPPER(`Metric_Name`) IN (UPPER('06. Refund Rate'))))
  LIMIT 50000
