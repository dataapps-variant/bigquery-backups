-- job_id: scheduled_query_6a6a0bd4-0000-2191-a113-fc4116926309
-- user: dataapps@variant.net
-- statement_type: CREATE_TABLE_AS_SELECT
-- created: 2026-07-28T08:00:13.325000+00:00
-- started: 2026-07-28T08:00:13.615000+00:00
-- ended: 2026-07-28T08:00:15.052000+00:00

CREATE OR REPLACE TABLE `variant-finance-data-project.R100.MB_new_trailing100` AS
WITH 
...
-- Step 1B: Find products from other apps that map to MB
cross_app_products AS (
    SELECT DISTINCT
        App_Name,
        Product_Name
    FROM `variant-finance-data-project.Sticky_Data.R100_BQ_Updated_Product_Name`
    WHERE Updated_App_Name = 'MB'
        AND App_Name != 'MB'
),

base_data AS (
    -- Step 1A: First data source - Direct MB data from Sticky table
    SELECT 
    ...
    -- Filter for last 100 days from current date and App_Name = "MB"
    ...
    -- Step 1B: Additional data from other apps that map to MB
    SELECT 
        -- Day Charged (past 100 days) - Date_of_Sale filtered for last 100 days
...
-- Filter to keep only records where APP_SHORT_NAME = "MB" after lookup
WHERE COALESCE(upd.Updated_App_Name, base.APP_SHORT_NAME) = 'MB'
AND base.Day_Charged < CURRENT_DATE()
