-- job_id: sheets_dataconnector_dd1cc722-7eee-445d-aa27-ba4997306d9e
-- user: dev@variant.net
-- statement_type: SELECT
-- created: 2026-07-28T11:38:17.501000+00:00
-- started: 2026-07-28T11:38:17.562000+00:00
-- ended: 2026-07-28T11:38:17.662000+00:00
...
SELECT * FROM `variant-finance-data-project.ICARUS_Multi.Final_Table_Formatted`
  ...
  WHERE ((`Reporting_Date` = '2026-7-27') AND ((UPPER(`Cohort`) NOT IN (UPPER('7K_30D'))) OR (UPPER(`Cohort`) IS NULL)) AND ((UPPER(`Active_Inactive`) NOT IN (UPPER('Inactive'))) OR (UPPER(`Active_Inactive`) IS NULL)) AND ((UPPER(`Table_Type`) NOT IN (UPPER('Crystal Ball'))) OR (UPPER(`Table_Type`) IS NULL)) AND (UPPER(`Metric_Name`) IN (UPPER('05. Gross Retention Rate'))))
  LIMIT 100000
