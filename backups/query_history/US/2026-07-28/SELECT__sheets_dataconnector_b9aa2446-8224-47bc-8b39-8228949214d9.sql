-- job_id: sheets_dataconnector_b9aa2446-8224-47bc-8b39-8228949214d9
-- user: dev@variant.net
-- statement_type: SELECT
-- created: 2026-07-28T10:25:35.891000+00:00
-- started: 2026-07-28T10:25:35.985000+00:00
-- ended: 2026-07-28T10:25:36.193000+00:00
...
SELECT * FROM `variant-finance-data-project.ICARUS_Multi.Final_Table_Formatted`
  ...
  WHERE ((`Reporting_Date` = '2026-7-27') AND ((UPPER(`Cohort`) NOT IN (UPPER('7K'))) OR (UPPER(`Cohort`) IS NULL)) AND ((UPPER(`Active_Inactive`) NOT IN (UPPER('Active'))) OR (UPPER(`Active_Inactive`) IS NULL)) AND ((UPPER(`Table_Type`) NOT IN (UPPER('Crystal Ball'))) OR (UPPER(`Table_Type`) IS NULL)) AND (UPPER(`Metric_Name`) IN (UPPER('05. Gross Retention Rate'))))
  LIMIT 100000
