-- job_id: scheduled_query_6a899358-0000-2535-a1a3-089e0831bf14
-- user: dataapps@variant.net
-- statement_type: CREATE_TABLE_AS_SELECT
-- created: 2026-07-31T08:00:18.165000+00:00
-- started: 2026-07-31T08:00:18.435000+00:00
-- ended: 2026-07-31T08:00:20.297000+00:00

CREATE OR REPLACE TABLE `variant-finance-data-project.R100.AT_new_trailing100` AS
WITH 
...
-- Step 1B: Find products from other apps that map to AT
cross_app_products AS (
    SELECT DISTINCT
        App_Name,
        Product_Name
    FROM `variant-finance-data-project.Sticky_Data.R100_BQ_Updated_Product_Name`
    WHERE Updated_App_Name = 'AT'
        AND App_Name != 'AT'
),

base_data AS (
    -- Step 1A: First data source - Direct AT data from Sticky table
    SELECT 
    ...
    -- Filter for last 100 days from current date and App_Name = "AT"
    ...
    -- Step 1B: Additional data from other apps that map to AT
    SELECT 
        -- Day Charged (past 100 days) - Date_of_Sale filtered for last 100 days
...
-- Filter to keep only records where APP_SHORT_NAME = "AT" after lookup
WHERE COALESCE(upd.Updated_App_Name, base.APP_SHORT_NAME) = 'AT'
 AND (base.SPENT_AMOUNT IS NULL OR base.SPENT_AMOUNT > 0.01)
