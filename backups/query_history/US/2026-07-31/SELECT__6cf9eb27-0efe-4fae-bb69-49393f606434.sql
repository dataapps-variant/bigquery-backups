-- job_id: 6cf9eb27-0efe-4fae-bb69-49393f606434
-- user: 1041741270489-compute@developer.gserviceaccount.com
-- statement_type: SELECT
-- created: 2026-07-31T20:15:40.924000+00:00
-- started: 2026-07-31T20:15:41.033000+00:00
-- ended: 2026-07-31T20:15:42.830000+00:00


    WITH users AS (
SELECT c.app, c.report_date, a.afid_channel, a.Country_Code, sum(c.subs) AS subs FROM (
SELECT app, DATE(time_stamp) AS report_date, afid, count(*) AS subs FROM (
    SELECT COALESCE(CASE WHEN AF.JF_App_Name IS NOT NULL AND TRIM(AF.JF_App_Name) != '' AND UPPER(TRIM(AF.JF_App_Name)) != 'COMBINED' THEN AF.JF_App_Name ELSE NULL END, CAM.App_Name, 'JF') AS app, time_stamp, o.afid, order_id AS subs FROM `variant-finance-data-project.Sticky_Data.test_Sticky_data_API_original_JF_Incremental` AS o
...
LEFT JOIN Sticky_Data.Sticky_Dim_Campaign AS CAM ON o.Campaign_Id = CAM.Campaign_Id AND CAM.Entity = 'JF'
    ...
    AND o.campaign_id IN (SELECT Campaign_Id FROM `variant-finance-data-project.Sticky_Data.Sticky_Dim_Campaign` WHERE Entity = 'JF')
...
WHERE Date >= current_date-7 AND App_name IN ('JF', 'CL', 'CV', 'RT', 'RL') AND Channel_name = 'Google_Ads'
...
LEFT JOIN (
SELECT app, report_date, 'ALL' AS traffic_channel, country_code, sum(subs) AS subs FROM users GROUP BY report_date, app, country_code
) AS u ON s.report_date = u.report_date AND s.app_short_name = u.app AND s.country = u.country_code)
),
calc_squares AS (
...
SELECT Date_of_Sale AS report_date, app, afid, count(*) AS subs FROM (
    ...
    WHERE Date_of_Sale >= DATE(datetime(current_timestamp, 'America/New_York'))-30 AND billing_cycle = '0' AND Final_Order_Status <> 7 AND Trial_Type <> 'SS' AND AFID <> '' AND App_Name IN ('JF', 'CL', 'CV', 'RT', 'RL')
) AS d
