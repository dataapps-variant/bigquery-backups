-- job_id: scheduled_query_6a843fe0-0000-28e6-b3d0-001a11431baa
-- user: dataapps@variant.net
-- statement_type: CREATE_TABLE_AS_SELECT
-- created: 2026-07-31T08:00:12.197000+00:00
-- started: 2026-07-31T08:00:12.440000+00:00
-- ended: 2026-07-31T08:00:13.932000+00:00

CREATE OR REPLACE TABLE `variant-finance-data-project.R100.CV_new_trailing100` AS
WITH 
...
-- Step 1B: Find products from other apps that map to CV
cross_app_products AS (
    SELECT DISTINCT
        App_Name,
        Product_Name
    FROM `variant-finance-data-project.Sticky_Data.R100_BQ_Updated_Product_Name`
    WHERE Updated_App_Name = 'CV'
        AND App_Name != 'CV'
),

base_data AS (
    -- Step 1A: First data source - Direct CV data from Sticky table
    SELECT 
    ...
    -- Filter for last 100 days from current date and App_Name = "CV"
    ...
    -- Step 1B: Additional data from other apps that map to CV
    SELECT 
        -- Day Charged (past 100 days) - Date_of_Sale filtered for last 100 days
...
-- Filter to keep only records where APP_SHORT_NAME = "CV" after lookup
WHERE COALESCE(upd.Updated_App_Name, base.APP_SHORT_NAME) = 'CV'
