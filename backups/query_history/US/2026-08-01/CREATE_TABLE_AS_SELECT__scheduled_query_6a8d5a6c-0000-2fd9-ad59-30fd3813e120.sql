-- job_id: scheduled_query_6a8d5a6c-0000-2fd9-ad59-30fd3813e120
-- user: dataapps@variant.net
-- statement_type: CREATE_TABLE_AS_SELECT
-- created: 2026-08-01T09:00:03.871000+00:00
-- started: 2026-08-01T09:00:04.234000+00:00
-- ended: 2026-08-01T09:02:49.533000+00:00
...
--
-- PERFORMANCE NOTE (V2.6 optimisation):
--   Payment is NOT added to the segments/analysis_grid cross join.
--   Instead, it is attached as a label in base_customers_deduped via
--   anchor_payments_deduped and carried through from that point onward.
--   This avoids multiplying the grid by the number of distinct Payment types.
--
-- =============================================================================
...
-- =============================================================================
-- SEGMENTS: Unique Product + Country segments (NO Payment — same as V2.5)
-- Payment is introduced later via anchor_payments_deduped to avoid grid explosion
-- =============================================================================
segments AS (
  SELECT DISTINCT
...
-- NO Payment here — grid stays same size as V2.5
-- =============================================================================
analysis_grid AS (
  SELECT
    rd.report_date,
...
-- V2.6: Join to anchor_payments_deduped to LABEL each customer with their
--        anchor Payment. Grid does NOT have Payment — it's introduced here.
-- =============================================================================
base_customers_deduped AS (
  SELECT
    ...
    ag.Country_Segment,
    ap.Payment,                  -- V2.6: Payment from anchor lookup
    ag.Billing_Cycle,
    ag.BC_start_date,
    ag.Limiting_Days,
    ag.base_bc_updated,
  ...
  -- V2.6: Attach anchor Payment label to each customer
...
SELECT
  ...
  -- ARPU Retention Rates (product-level — no Payment in join)
