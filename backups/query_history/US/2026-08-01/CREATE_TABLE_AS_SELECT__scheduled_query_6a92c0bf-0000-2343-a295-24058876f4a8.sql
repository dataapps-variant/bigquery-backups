-- job_id: scheduled_query_6a92c0bf-0000-2343-a295-24058876f4a8
-- user: dataapps@variant.net
-- statement_type: CREATE_TABLE_AS_SELECT
-- created: 2026-08-01T08:00:17.027000+00:00
-- started: 2026-08-01T08:00:17.288000+00:00
-- ended: 2026-08-01T08:00:18.822000+00:00

CREATE OR REPLACE TABLE `variant-finance-data-project.R100.DT_new_trailing100` AS
WITH 
...
-- Step 1B: Find products from other apps that map to DT
cross_app_products AS (
    SELECT DISTINCT
        App_Name,
        Product_Name
    FROM `variant-finance-data-project.Sticky_Data.R100_BQ_Updated_Product_Name`
    WHERE Updated_App_Name = 'DT'
        AND App_Name != 'DT'
),

base_data AS (
    -- Step 1A: First data source - Direct DT data from Sticky table
    SELECT 
    ...
    -- Filter for last 100 days from current date and App_Name = "DT"
    ...
    -- Step 1B: Additional data from other apps that map to DT
    SELECT 
        -- Day Charged (past 100 days) - Date_of_Sale filtered for last 100 days
        ...
        -- ORDER COUNT - Count of Order_Id grouped by all previous columns
        COUNT(sticky.Order_ID) AS ORDER_COUNT
...
-- Filter to keep only records where APP_SHORT_NAME = "DT" after lookup
WHERE COALESCE(upd.Updated_App_Name, base.APP_SHORT_NAME) = 'DT'
