-- job_id: scheduled_query_6a8cc4de-0000-2fd9-ad59-30fd3813e120
-- user: dataapps@variant.net
-- statement_type: CREATE_TABLE_AS_SELECT
-- created: 2026-08-01T08:15:02.652000+00:00
-- started: 2026-08-01T08:15:02.947000+00:00
-- ended: 2026-08-01T08:15:05.817000+00:00

CREATE OR REPLACE TABLE `variant-finance-data-project.Daedalus.Daedalus_Traffic_Channel` AS
...
trailing_30d AS (
  -- Calculate 30-day trailing metrics by traffic channel using window functions
  SELECT 
    date,
    App_Name,
    Traffic_Channel,
    ...
    Daily_Spend,
    Daily_New_Users,
    -- T30D Spend: average of daily spend over 30 days
    SUM(Daily_Spend) OVER (
      PARTITION BY App_Name, Traffic_Channel 
      ORDER BY UNIX_DATE(date)
      RANGE BETWEEN 29 PRECEDING AND CURRENT ROW
    ) / 30 AS T30D_Spend,
    -- T30D Users: average of daily new users over 30 days
    SUM(Daily_New_Users) OVER (
      PARTITION BY App_Name, Traffic_Channel 
      ORDER BY UNIX_DATE(date)
      RANGE BETWEEN 29 PRECEDING AND CURRENT ROW
    ) / 30 AS T30D_Users,
    -- T30D CAC: total spend / total users over 30 days
    CASE 
