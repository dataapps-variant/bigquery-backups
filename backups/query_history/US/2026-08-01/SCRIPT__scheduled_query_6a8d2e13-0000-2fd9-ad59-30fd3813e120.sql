-- job_id: scheduled_query_6a8d2e13-0000-2fd9-ad59-30fd3813e120
-- user: dataapps@variant.net
-- statement_type: SCRIPT
-- created: 2026-08-01T08:45:05.275000+00:00
-- started: 2026-08-01T08:45:05.334000+00:00
-- ended: 2026-08-01T08:45:56.036000+00:00
...
IF days_to_process > 0 THEN
  ...
  -- =====================================================
  INSERT INTO `variant-finance-data-project.Icarus_Spend_Country_AFID.SCA_7K_30D_Crystal_Ball`
...
master_combinations AS (
  SELECT 
    rd.report_date,
    spc.Product_Name_Final,
    spc.Country_Code,
    spc.Spend_Country_Code_AFID,
    bcr.billing_cycle,
    cfg.cohort_size,
    cfg.limiting_days,
...
initial_cohort_selection AS (
  SELECT 
    report_date, Product_Name_Final, Country_Code, Spend_Country_Code_AFID,
...
bc_end_date_calculation AS (
  SELECT 
    bl.*,
    ca.oldest_transaction_date, ca.total_transactions, ca.unique_customers,
    CASE 
...
final_cohort AS (
  SELECT 
    ics.report_date, ics.Product_Name_Final, ics.Country_Code, ics.Spend_Country_Code_AFID,
    ics.billing_cycle, ics.calculated_denominator_bc,
    ics.Updated_Cust_ID, ics.Order_Price_Net_of_Tax_USD,
  ...
  INNER JOIN bc_end_date_calculation bedc
    ...
    AND ics.Spend_Country_Code_AFID = bedc.Spend_Country_Code_AFID
  ...
  SELECT 
    bedc.report_date, bedc.Product_Name_Final, bedc.Country_Code, bedc.Spend_Country_Code_AFID,
    bedc.billing_cycle, bedc.calculated_denominator_bc,
    base.Updated_Cust_ID, base.Order_Price_Net_of_Tax_USD,
...
exchange_rate_lookup AS (
  SELECT 
    ...
    CASE WHEN bedc.Currency = 'USD' THEN 1.0
         ELSE COALESCE(AVG(er.Value_USD), 1.0) END as final_exchange_rate
  FROM bc_end_date_calculation bedc
...
subscription_metrics AS (
  SELECT 
    fc.report_date, fc.Product_Name_Final, fc.Country_Code, fc.Spend_Country_Code_AFID,
...
day_0_metrics AS (
  SELECT 
    fc_distinct.report_date, fc_distinct.Product_Name_Final, fc_distinct.Country_Code,
  ...
  FROM (
    SELECT DISTINCT report_date, Product_Name_Final, Country_Code, Spend_Country_Code_AFID,
           billing_cycle, calculated_denominator_bc, Updated_Cust_ID 
    FROM final_cohort
  ) fc_distinct
  INNER JOIN bc_end_date_calculation bedc
    ...
    AND fc_distinct.Spend_Country_Code_AFID = bedc.Spend_Country_Code_AFID
...
sot_ratio_lookup AS (
  ...
  FROM bc_end_date_calculation bedc
  LEFT JOIN `Icarus_Spend_Country_AFID.SCA_7K_SOT_Ratio` sot
    ...
    AND bedc.Spend_Country_Code_AFID = sot.Spend_Country_Code_AFID
...
t30d_new_users_calc AS (
  SELECT 
    bedc.report_date, bedc.Product_Name_Final, bedc.Country_Code,
    bedc.Spend_Country_Code_AFID, bedc.Trial_Type,
    COUNT(DISTINCT base.Updated_Cust_ID) as final_t30d_new_users
  FROM (
    SELECT DISTINCT report_date, Product_Name_Final, Country_Code, Spend_Country_Code_AFID, Trial_Type
...
core_metrics AS (
  SELECT 
    ...
    CASE WHEN (bedc.Trial_Type = 'NT' AND bedc.billing_cycle IN (0,1)) OR bedc.billing_cycle = 0 
         ...
         ELSE COALESCE(d0m.final_day_0_user, 0) END as clean_day_0_user,
    CASE WHEN (bedc.Trial_Type = 'NT' AND bedc.billing_cycle IN (0,1)) OR bedc.billing_cycle = 0 
         THEN COALESCE(sm.final_subscription_value, 0)
         ELSE COALESCE(d0m.final_day_0_values, 0) END as clean_day_0_values,
    sot.final_sot_ratio as clean_sot_ratio,
    COALESCE(SAFE_DIVIDE(
      CASE WHEN (bedc.Trial_Type = 'NT' AND bedc.billing_cycle IN (0,1)) OR bedc.billing_cycle = 0 
           THEN COALESCE(sm.final_subscription_users, 0)
           ELSE COALESCE(d0m.final_day_0_user, 0) END,
      NULLIF(sot.final_sot_ratio, 0)), 0) as clean_cb_user,
    COALESCE(SAFE_DIVIDE(
      CASE WHEN (bedc.Trial_Type = 'NT' AND bedc.billing_cycle IN (0,1)) OR bedc.billing_cycle = 0 
           THEN COALESCE(sm.final_subscription_value, 0)
           ELSE COALESCE(d0m.final_day_0_values, 0) END,
  ...
  LEFT JOIN exchange_rate_lookup erl
    ...
    AND bedc.Country_Code=erl.Country_Code AND bedc.Spend_Country_Code_AFID=erl.Spend_Country_Code_AFID
  ...
  LEFT JOIN subscription_metrics sm
    ...
    AND bedc.Country_Code=sm.Country_Code AND bedc.Spend_Country_Code_AFID=sm.Spend_Country_Code_AFID
  ...
  LEFT JOIN single_sale_metrics ssm
    ...
    AND bedc.Country_Code=ssm.Country_Code AND bedc.Spend_Country_Code_AFID=ssm.Spend_Country_Code_AFID
  ...
  LEFT JOIN day_0_metrics d0m
    ...
    AND bedc.Country_Code=d0m.Country_Code AND bedc.Spend_Country_Code_AFID=d0m.Spend_Country_Code_AFID
  ...
  LEFT JOIN sot_ratio_lookup sot
    ...
    AND bedc.Country_Code=sot.Country_Code AND bedc.Spend_Country_Code_AFID=sot.Spend_Country_Code_AFID
    AND bedc.billing_cycle=sot.billing_cycle
  LEFT JOIN recent_spend_metrics rsm
    ON bedc.report_date=rsm.report_date AND bedc.Product_Name_Final=rsm.Product_Name_Final
    AND bedc.Country_Code=rsm.Country_Code AND bedc.Spend_Country_Code_AFID=rsm.Spend_Country_Code_AFID
  ...
  LEFT JOIN recent_users_metrics rum
    ...
    AND bedc.Country_Code=rum.Country_Code AND bedc.Spend_Country_Code_AFID=rum.Spend_Country_Code_AFID
  ...
  LEFT JOIN t30d_new_users_calc t30d
    ...
    AND bedc.Country_Code=t30d.Country_Code AND bedc.Spend_Country_Code_AFID=t30d.Spend_Country_Code_AFID
),
...
calculated_ratios AS (
  SELECT 
    ...
    COALESCE(
      CASE WHEN (cm.Trial_Type = 'NT' AND cm.billing_cycle = 1) OR cm.billing_cycle = 0 
  ...
  LEFT JOIN `Icarus_Spend_Country_AFID.SCA_Refund_Table` rt
    ...
    AND cm.Spend_Country_Code_AFID = rt.Spend_Country_Code_AFID
...
final_calculations AS (
  SELECT 
    ...
    CASE WHEN rs12.final_retention_rate IS NULL THEN NULL
...
final_output_prep AS (
  SELECT 
    ...
    CASE WHEN fc.Country_Code IS NOT NULL AND fc.Country_Code != '' 
         ...
         ELSE fc.App_Name END as computed_app_name,
  ...
  ) ap ON fc.Product_Name_Final = ap.Product_Name_Final
)
