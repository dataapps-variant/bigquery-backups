-- job_id: scheduled_query_6aaa141d-0000-2fa5-bcfc-240588731210
-- user: dataapps@variant.net
-- statement_type: SCRIPT
-- created: 2026-08-01T08:30:02.996000+00:00
-- started: 2026-08-01T08:30:03.045000+00:00
-- ended: 2026-08-01T08:30:24.745000+00:00
...
IF days_to_process > 0 THEN
  ...
  -- =====================================================
  INSERT INTO `variant-finance-data-project.Icarus_Cohort.IC_Refund_Table_AFID`
...
product_list AS (
  SELECT DISTINCT Product_Name_Final, Currency
...
enriched_master AS (
  ...
  WHERE mc.report_date >= apl.First_Date_of_Sale    -- ADDED (Edit 5)
),
-- =====================================================
...
-- Grain: Report_date + Product_Name_Final + AFID_Suffix + Country_Code + Billing_Cycle
-- =====================================================
aggregated_output AS (
  ...
  GROUP BY cd.report_date, cd.Product_Name_Final, cd.AFID_Suffix, cd.billing_cycle, cd.Country_Code
)
...
ORDER BY ao.report_date DESC, ao.Product_Name_Final, ao.AFID_Suffix, ao.Country_Code, ao.billing_cycle;

-- =====================================================
-- SCRIPT COMPLETE - REFUND TABLE V3 AFID_SUFFIX ONLY
--
...
-- - No AFID_CHANNEL join condition on base tables
-- - No COALESCE(AFID_CHANNEL, 80) anywhere
--
-- BASE TABLE JOINS match on:
-- - Product_Name_Final = Product_Name_Final_Merged
-- - AFID_Suffix = base.AFID (NULL-safe)
-- - Country_Code logic (JP / Non-JP / all)
-- - Date and billing cycle conditions
-- =====================================================
