...
excluded_users AS (
  SELECT DISTINCT
    user_id
  FROM `variant-finance-data-project.Sticky_Data.split-trial-experiment-resume-user-list-excluded`
  WHERE user_id IS NOT NULL
),
...
initial_trial AS (
  ...
  WHERE s.App_Name = 'JF'
    ...
    AND s.Bill_Email NOT IN (SELECT user_id FROM excluded_users)
  GROUP BY r.Category
),
...
users_with_bc1 AS (
  ...
  WHERE s.Billing_Cycle_Updated = 1
...
bc1_dates AS (
  SELECT
    s.Updated_Cust_ID,
    s.Date_of_Sale AS BC1_Date_of_Sale,
    s.Final_Order_Status,
    s.Is_Recurring
  FROM `variant-finance-data-project.Sticky_Data.Sticky_data_API_Original_V_W_EC_Merged_TBL` s
  WHERE s.Billing_Cycle_Updated > 0
    AND s.Date_of_Sale >= '2026-03-30'
    AND s.Bill_Email NOT IN (SELECT user_id FROM excluded_users)
),
...
-- BC1 Revenue (sum before dividing)
bc1_revenue AS (
  SELECT
    ic.Category,
    SUM(s.Order_Price_Net_of_Tax_USD) AS BC1_Revenue
  FROM initial_converted_base ic
  JOIN `variant-finance-data-project.Sticky_Data.Sticky_data_API_Original_V_W_EC_Merged_TBL` s
...
-- Net_BC1_ARPU (BC1 Revenue / Initial Trial Conversions)
net_bc1_arpu AS (
  SELECT
    br.Category,
    SAFE_DIVIDE(br.BC1_Revenue, it.Initial_Trial_Conversions) AS Net_BC1_ARPU
  FROM bc1_revenue br
  JOIN initial_trial it
    ON br.Category = it.Category
)
...
SELECT
  ...
  -- ADD before Net_BC1_ARPU:
  COALESCE(rev.BC1_Revenue, 0)                                                                        AS BC1_Revenue,
...
LEFT JOIN bc1_revenue                  rev   ON r.Category = rev.Category
