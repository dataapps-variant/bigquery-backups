CREATE PROCEDURE `variant-finance-data-project`.VPU_Merged.proc_15K_Crystal_Ball_30()
BEGIN

-- =====================================================
-- 15K CRYSTAL BALL TABLE CREATION SCRIPT - 30D - UPDATED
-- Cohort Analysis with Crystal Ball Projections
-- 
-- UPDATES IN THIS VERSION:
-- 1. Plan_List source: VPU.Plan_List → VPU_Merged.Plan_List
-- 2. Added Plan_List aggregation logic (4 CTEs)
-- 3. Product column: Product_Name_Final → Product_Name_Final_Merged
-- 4. SOT_Ratio source: VPU.15K_SOT_Ratio → VPU_Merged.15k_SOT_Ratio
-- 5. Refund source: Already uses VPU_Merged.Refund_Table
...
-- 
-- OUTPUT: 47 columns (Crystal Ball methodology with VPU_Merged sources)
-- =====================================================

CREATE OR REPLACE TABLE `variant-finance-data-project.VPU_Merged.15K_Crystal_Ball_30` AS
...
master_combinations AS (
  SELECT 
    ...
    cfg.cohort_size,
    cfg.limiting_days,
    cfg.minimum_rebill_users,  -- CHANGED
...
-- =====================================================
-- SOT_RATIO LOOKUP
-- UPDATED: VPU_Merged.15k_SOT_Ratio
-- =====================================================
sot_ratio_lookup AS (
  SELECT 
    bedc.report_date,
  ...
  FROM bc_end_date_calculation bedc
  LEFT JOIN `variant-finance-data-project.VPU_Merged.15K_SOT_Ratio` sot  -- CHANGED
...
-- =====================================================
-- SCRIPT COMPLETE - 15K CRYSTAL BALL TABLE - UPDATED WITH NT BC0 NULL LOGIC
-- Table: variant-finance-data-project.VPU_Merged.15K_30D_Crystal_Ball
-- 
