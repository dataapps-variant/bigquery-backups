CREATE PROCEDURE `variant-finance-data-project`.VPU_Merged.proc_15K_Main_Table_30()
BEGIN
...
-- 
-- Updated: Rebill_users < 25 logic added
-- Updated: Refund_ratio = 0 when Retention_rate = 0
-- Updated: minimum_rebill_users as configurable variable
-- =====================================================

CREATE OR REPLACE TABLE `variant-finance-data-project.VPU_Merged.15K_Main_Table_30` AS
...
raw_eligible_transactions AS (
  ...
  INNER JOIN `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_V_Merged_TBL` base
    ON bl.Product_Name_Final = base.Product_Name_Final_Merged  -- CHANGED: Was Product_Name_Final
...
final_cohort AS (
  ...
  INNER JOIN `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_V_Merged_TBL` base
    ON bedc.Product_Name_Final = base.Product_Name_Final_Merged  -- CHANGED: Was Product_Name_Final
...
rebill_metrics AS (
  ...
  LEFT JOIN `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_V_Merged_TBL` rebill
    ...
    AND fc_distinct.Product_Name_Final = rebill.Product_Name_Final_Merged  -- CHANGED: Was Product_Name_Final
...
single_sales_metrics AS (
  ...
  LEFT JOIN `variant-finance-data-project.Sticky_Data.Sales_SS_original_API_Merged_TBL` ss
    ON bedc.Product_Name_Final = ss.Product_Name_Final_Main_Merged  -- CHANGED: Was Product_Name_Final_Main
...
ad_spend_metrics AS (
  ...
  LEFT JOIN `variant-finance-data-project.Ad_spend_data.Merged_Spend_Split_TBL` ads
    ON bedc.Product_Name_Final = ads.Product_Name_Final_Merged  -- CHANGED: Was Product_Name_Final
...
core_metrics AS (
  SELECT 
    ...
    COALESCE(asd.final_allocated_spend, 0) as clean_allocated_spend
...
LEFT JOIN recent_users_metrics rum
  ...
  LEFT JOIN allocated_spend_daily asd
    ON bedc.report_date = asd.report_date 
    AND bedc.Product_Name_Final = asd.Product_Name_Final 
    AND bedc.Country_Code = asd.Country_Code
    AND bedc.billing_cycle = asd.billing_cycle
),
-- =====================================================
...
SELECT 
  ...
  fop.clean_allocated_spend as Allocated_Spend_Total,
...
-- =====================================================
-- 
-- MODIFICATION 1: PLAN_LIST AGGREGATION (6 changes)
-- 1. Added plan_list_ranked CTE
-- 2. Added plan_list_first_record CTE  
-- 3. Added plan_list_trial_price_avg CTE
-- 4. Added aggregated_plan_list CTE
-- 5. Updated product_list to use aggregated_plan_list
-- 6. Updated enriched_master join with NULL-safe Country_Code matching
--
-- MODIFICATION 2: COLUMN NAME UPDATES (8 changes)
...
--
-- All changed from Product_Name_Final to Product_Name_Final_Merged
-- (except Plan_List which keeps Product_Name_Final)
-- 
-- =====================================================
