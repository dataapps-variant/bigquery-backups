...
-- =====================================================
-- STEP 7: RETENTION RATE CASCADING
-- =====================================================
retention_step_0 AS (
  SELECT 
    wc.*,
...
final_calculations AS (
  SELECT 
    ...
    rs4.Churn_rate,
    rs4.Retention_rate,
    rs4.Wt_Avg_Refund_ratio,
    rs4.Avg_Refund_ratio,
    rs4.Retention_rate - rs4.Wt_Avg_Refund_ratio AS NET_Retention_rate,
    
    -- CAC metrics
    COALESCE(SAFE_DIVIDE(rs4.Spend_amount, NULLIF(rs4.Subscription_users, 0)), 0) AS Cohort_CAC,
    COALESCE(SAFE_DIVIDE(rs4.Recent_Spend, NULLIF(rs4.Recent_Users, 0)), 0) AS Recent_CAC,
    COALESCE(SAFE_DIVIDE(rs4.Recent_Spend_MP, NULLIF(rs4.Recent_Users_MP, 0)), 0) AS Recent_CAC_MP,
    
    -- ARPU metrics
    COALESCE(SAFE_DIVIDE(rs4.Single_Sale_Value + rs4.Rebill_value, NULLIF(rs4.Rebill_users, 0)), 0) * rs4.Retention_rate AS ARPU_Discounted,
    COALESCE(SAFE_DIVIDE(rs4.Single_Sale_Value + rs4.Rebill_value, NULLIF(rs4.Rebill_users, 0)), 0) * (rs4.Retention_rate - rs4.Wt_Avg_Refund_ratio) AS Net_ARPU_Discounted
...
SELECT 
  ...
  fc.Net_ARPU_Discounted - fc.Recent_CAC_MP AS Net_LTV_Discounted
...
-- 1. Output table: VPU → VPU_Merged (line 14)
-- 2. Source table (15K_Main_Table_MP): VPU → VPU_Merged (line 51)
--
-- MODIFICATION 2: COLUMN NAME UPDATES (3 changes)
-- 3. Sticky Data: Product_Name_Final → Product_Name_Final_Merged (line 111)
-- 4. Ad Spend: App_name → App_Name (line 69)
-- 5. Ad Spend: App_name → App_Name (line 94)
--
-- TOTAL CHANGES: 5
-- 
-- KEY FEATURES PRESERVED:
-- - CT-JP/CT-Non-JP mutually exclusive logic
-- - Recent_CAC_MP calculation with (User, Product) pairs
-- - Weighted average refund ratio
-- - Step-by-step retention calculation
-- 
