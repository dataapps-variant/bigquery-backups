...
app_afid_base_aggregated AS (
  SELECT 
    ...
    -- Refund weighted average components (by Rebill_value, NULL treated as 0%)
...
-- =====================================================
-- CTE 2: LAST SPEND DATE PER APP + COUNTRY + AFID
...
-- =====================================================
-- CTE 3: SPEND DATE WINDOWS (7-day window from last_spend_date)
-- =====================================================
...
-- Rebill_users < 25 threshold at BC1-BC12 only (Fix #5)
-- NULL cascade: if previous BC is NULL or Rebill_users < 25 (Fix #6)
...
-- Formulas aligned with CB pattern (Rebill substituted for CB)
-- =====================================================
final_calculations AS (
  SELECT 
    ...
    -- ARPU_Discounted = (Rebill_value/Rebill_users) × Retention + SS/Subscription_users
    CASE 
      WHEN rs12.Retention_rate IS NULL THEN NULL
      ELSE COALESCE(
    ...
    -- Net_ARPU_Discounted = (Rebill_value/Rebill_users) × (Retention - Refund) + SS/Subscription_users
    CASE 
      WHEN rs12.Retention_rate IS NULL THEN NULL
      ELSE COALESCE(
...
-- =====================================================
-- SCRIPT COMPLETE - 7K MAIN TABLE APP (300D) × AFID LEVEL
-- Table: variant-finance-data-project.ICARUS_Multi.7K_Main_Table_App_AFID_level
--
-- GRAIN: Report_date × App_Name × Country_Code × AFID × Billing_Cycle
--
-- SOURCE TABLE:
-- variant-finance-data-project.Icarus_Cohort.IC_7K_Main_Table
--
-- NO SOT LAYER — Uses Rebill_users/Rebill_value directly
--
-- SOURCE TABLE JOINS:
-- Spend: COALESCE(ads.Channel_code, 80) = AFID
-- Sticky: COALESCE(base.AFID_CHANNEL, 80) = AFID
--
-- CB-ALIGNED FORMULAS (Rebill substituted for CB):
-- Churn = 1 - Rebill_users / Subscription_users
-- Threshold = Rebill_users < 25
-- Refund weight = Rebill_value
-- ARPU = (Rebill_value/Rebill_users) × Retention + SS/Sub
-- Net_ARPU = (Rebill_value/Rebill_users) × (Ret-Ref) + SS/Sub
--
-- ALL 7 FIXES APPLIED:
-- 1. Refund denominator: SUM(COALESCE(Refund_ratio,0) × Rebill_value) / SUM(Rebill_value)
-- 2. No Spend_amount in output
-- 3. No CAC (only Recent_CAC)
-- 4. BC0 = 1.0 ALWAYS (no threshold check)
-- 5. Rebill_users < 25 threshold at BC1-BC12 only → NULL
...
-- Base: Subscription_users, Subscription_value, Rebill_users, Rebill_value, SS_Users, SS_Value
