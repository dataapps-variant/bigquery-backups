CREATE OR REPLACE VIEW `variant-finance-data-project.Facbook_ad_spend_data.Facebook_final_spend_CT` AS
...
WITH base_data AS (
  ...
  SELECT 
    ...
    fb.AccountName,
    fb.Spend
  FROM `variant-finance-data-project.Facbook_ad_spend_data.Facebook_Ads_CT_DailyAdInsights` fb
),
...
url_lookup AS (
  ...
  FROM base_data base
  LEFT JOIN `variant-finance-data-project.Facbook_ad_spend_data.Facebook_Ads_CT_URL_map` url_map
...
country_spend_ranked AS (
  ...
  FROM `variant-finance-data-project.Facbook_ad_spend_data.Facebook_Ads_CT_Country_Code_map` country_map
),
...
SELECT 
  ...
  -- App_name: Use mapped value or fallback to 'CT'
  COALESCE(final.mapped_app_name, 'CT') as App_name,
