EXECUTE IMMEDIATE (
  SELECT FORMAT("""
    CREATE OR REPLACE TABLE `variant-finance-data-project.demo_practise.en_rolling_actual_data` AS
    WITH start_date AS (
    ...
    date_rows AS (
      ...
      UNNEST(GENERATE_DATE_ARRAY(
        DATE_SUB(min_date, INTERVAL 1 DAY),
  ...
  FROM UNNEST(GENERATE_DATE_ARRAY(
    DATE_SUB(CURRENT_DATE(), INTERVAL 101 DAY),
    CURRENT_DATE(),
    INTERVAL 1 DAY
  )) AS col_date),
