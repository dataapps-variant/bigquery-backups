...
base AS (
  SELECT
    ...
    END AS Entity,
    CASE
      WHEN s.App_Name = 'CT' AND s.Spend_Country_Code_AFID = 'JP' THEN COALESCE(p.Updated_Concat, s.Product_Name_Final) || '-JP'
      WHEN s.App_Name = 'CT' THEN COALESCE(p.Updated_Concat, s.Product_Name_Final) || '-Non-JP'
      ELSE COALESCE(p.Updated_Concat, s.Product_Name_Final)
    END AS Plan_Name,
