...
-- cohort qualification uses ALL trials, SS included (threshold applied before exclusion)
repeat_pairs AS (
...
chain_orders AS (
  SELECT o.*, k.report_app, k.spend_country_code_afid
  FROM orders o
...
revenue_by_cycle AS (
  SELECT report_app, bc AS n, SUM(revenue) AS revenue_usd
  FROM chain_orders
...
units AS (
  SELECT c.report_app, ANY_VALUE(c.spend_country_code_afid) AS spend_country_code_afid, n,
...
metrics AS (
  SELECT
    ...
    EXP(SUM(LN(NULLIF(SAFE_DIVIDE(u.rebill_units, u.subscription_units), 0)))
        OVER (PARTITION BY u.report_app ORDER BY u.n))    AS retention_rate
  FROM units u
  LEFT JOIN revenue_by_cycle r
    ON r.report_app = u.report_app AND r.n = u.n
...
SELECT
  ...
  ROUND(SUM(COALESCE(retention_rate * arpu_usd, 0))
        OVER (PARTITION BY report_app ORDER BY n), 2) AS cumulative_arpu_usd
FROM metrics
ORDER BY App_Name, billing_cycle;
