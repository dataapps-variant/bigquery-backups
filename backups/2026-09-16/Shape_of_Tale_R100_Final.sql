...
cohort_totals AS (
  SELECT
    t.App_Name,
    CASE
      WHEN t.App_Name = 'CT-JP'     THEN COALESCE(p.Updated_Concat, t.Original_Product) || '-JP'
      WHEN t.App_Name = 'CT-Non-JP' THEN COALESCE(p.Updated_Concat, t.Original_Product) || '-Non-JP'
      ELSE COALESCE(p.Updated_Concat, t.Original_Product)
...
delay_distribution AS (
  SELECT
    d.App_Name,
    CASE
      WHEN d.App_Name = 'CT-JP'     THEN COALESCE(p.Updated_Concat, d.Original_Product) || '-JP'
      WHEN d.App_Name = 'CT-Non-JP' THEN COALESCE(p.Updated_Concat, d.Original_Product) || '-Non-JP'
      ELSE COALESCE(p.Updated_Concat, d.Original_Product)
    END AS Product_Name_Final,
    d.Trial_Type,
    d.BC,
    d.SOT_Days,
