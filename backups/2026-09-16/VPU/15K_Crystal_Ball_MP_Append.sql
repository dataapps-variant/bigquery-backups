-- =====================================================
-- 15K CRYSTAL BALL TABLE INCREMENTAL APPEND SCRIPT
-- Cohort Analysis with Crystal Ball Projections
-- 
-- APPEND CONFIGURATION:
-- - lookback_days: 20 (configurable for data refresh)
-- - Fallback date: DATE('2025-01-01')
-- - Deletes last 20 days before inserting new data
-- - Processes only new dates incrementally
-- 
-- KEY FEATURES (UNCHANGED):
-- 1. Retry_engine_period = 1 day (vs 30 in Main Table)
-- 2. Day_0_user and Day_0_values (on-time rebills only)
-- 3. SOT_Ratio from 15K_SOT_Ratio table
-- 4. CB_User and CB_Value (Crystal Ball projections)
-- 5. All formulas use CB metrics instead of Rebill metrics
-- 
-- OUTPUT: 55 columns (52 from Main - 2 Rebill + 5 new)
-- =====================================================

-- =====================================================
-- STEP 1: DELETE EXISTING DATA FOR REFRESH WINDOW
-- =====================================================
DECLARE lookback_days INT64 DEFAULT 20;

DELETE FROM `variant-finance-data-project.VPU.15K_Crystal_Ball_MP`
WHERE Report_date >= DATE_SUB(CURRENT_DATE(), INTERVAL lookback_days DAY);

-- =====================================================
-- STEP 2: INSERT NEW/UPDATED DATA
-- =====================================================
INSERT INTO `variant-finance-data-project.VPU.15K_Crystal_Ball_MP`
...
config AS (
  SELECT 
    GREATEST(
      COALESCE(
        (SELECT MAX(Report_date) FROM `variant-finance-data-project.VPU.15K_Crystal_Ball_MP`),
        DATE('2025-01-01')
      ),
      DATE_SUB(CURRENT_DATE(), INTERVAL 20 DAY)
    ) AS new_start_date,
    DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY) AS new_end_date,
...
report_dates AS (
  ...
  FROM UNNEST(GENERATE_DATE_ARRAY(
    (SELECT new_end_date FROM config),
    (SELECT new_start_date FROM config),
    INTERVAL 1 DAY
  )) AS report_date
),
product_list AS (
...
day_0_metrics AS (
  ...
  LEFT JOIN `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_V_Merged_TBL` rebill
    ...
    AND (rebill.Delay_days_SOT <= 0 OR rebill.Delay_days_SOT IS NULL)  -- NEW: Day 0 filter
    AND (
...
-- UPDATED: Uses CB_User instead of Rebill_users
-- =====================================================
calculated_ratios AS (
  SELECT 
    cm.*,
    -- Churn rate calculation (UPDATED: uses CB_User)
    CASE 
...
-- Uses Churn_rate which now uses CB_User
-- =====================================================
retention_step_0 AS (
  SELECT 
    cr.*,
...
-- =====================================================
-- FINAL CALCULATIONS
-- UPDATED: Uses CB_Value and CB_User
-- =====================================================
  ...
  SELECT 
    ...
    CASE 
      ...
      ELSE 
        ...
        COALESCE(SAFE_DIVIDE(rs4.clean_single_sale_value, NULLIF(rs4.clean_subscription_users, 0)), 0)
    END as final_arpu,
    -- ARPU Discounted: UPDATED - uses CB_Value / CB_User
    COALESCE(
    ...
    -- Net ARPU Discounted: UPDATED - uses CB_Value / CB_User
    COALESCE(
      SAFE_DIVIDE(rs4.clean_cb_value, NULLIF(rs4.clean_cb_user, 0)) * (rs4.final_retention_rate - rs4.final_refund_ratio),
      0.0
...
-- UPDATED: Uses CB_Value for BASE calculation
-- =====================================================
weighted_avg_prep AS (
  SELECT 
    ...
    -- LTV calculations (uses Recent_CAC)
    (CASE 
    ...
    (COALESCE(
      SAFE_DIVIDE(fc.clean_cb_value, NULLIF(fc.clean_cb_user, 0)) * (fc.final_retention_rate - fc.final_refund_ratio),
      0.0
...
-- UPDATED: Uses CB_Value for BASE and WA_RATIO
-- =====================================================
weighted_avg_final AS (
  SELECT 
    ...
    -- BASE: Sum of CB_Value for the same Key (UPDATED)
    SUM(wap.clean_cb_value) OVER (
      PARTITION BY 
        wap.computed_app_name,
        wap.report_date, 
        wap.billing_cycle
    ) as final_base,
    -- WA_RATIO: CB_Value / BASE (UPDATED)
    SAFE_DIVIDE(
      wap.clean_cb_value,
      NULLIF(SUM(wap.clean_cb_value) OVER (
        PARTITION BY 
          wap.computed_app_name,
          wap.report_date, 
          wap.billing_cycle
...
-- REMOVED: Rebill_users, Rebill_value
-- ADDED: Day_0_user, Day_0_values, SOT_Ratio, CB_User, CB_Value
-- =====================================================
SELECT 
  -- Primary Dimensions
  ...
  -- REMOVED: Rebill_users, Rebill_value
  waf.clean_day_0_user as Day_0_user,  -- NEW
  waf.clean_day_0_values as Day_0_values,  -- NEW
  waf.clean_sot_ratio as SOT_Ratio,  -- NEW
  waf.clean_cb_user as CB_User,  -- NEW
  waf.clean_cb_value as CB_Value,  -- NEW
...
-- =====================================================
-- SCRIPT COMPLETE - 15K CRYSTAL BALL TABLE APPEND
-- Table: variant-finance-data-project.VPU.15K_Crystal_Ball_MP
-- 
-- APPEND SUMMARY:
-- - Deletes last 20 days of data
-- - Inserts new data starting from MAX(Report_date) or 2025-01-01
-- - Maintains all Crystal Ball methodology
-- 
-- OUTPUT: 55 columns
-- - Removed: Rebill_users, Rebill_value (2)
-- - Added: Day_0_user, Day_0_values, SOT_Ratio, CB_User, CB_Value (5)
-- 
-- KEY FEATURES (UNCHANGED):
-- 1. retry_engine_period = 1 day (vs 30 in Main Table)
-- 2. Day_0 metrics filter on Delay_days_SOT <= 0 or NULL
-- 3. SOT_Ratio from 15K_SOT_Ratio table
-- 4. CB_User = Day_0_user / SOT_Ratio
-- 5. CB_Value = Day_0_values / SOT_Ratio
-- 6. All formulas use CB metrics instead of Rebill metrics
-- 
-- CRYSTAL BALL CONCEPT:
-- Projects total rebills by accounting for delayed starters
-- Example: 10K rebills on time / 0.80 SOT ratio = 12.5K projected total
-- =====================================================
