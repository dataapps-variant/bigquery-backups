CREATE VIEW `variant-finance-data-project.Sticky_Data.Sticky_Sales_V_PD_W_EC`
AS WITH IP_Parse AS (
  SELECT 
    PD.*,
    'PD' AS Entity_Name,
  ...
  FROM `Sticky_Data.Sticky_PDFinal` PD
),
...
---------------------------------------------------------------------------------------------
  ...
  CASE 
    WHEN CC_Tax.Country_Code = 'US' THEN CL.SALES_TAX_FACTOR--if null then 0
    ELSE SAFE_MULTIPLY(
...
LEFT JOIN Sticky_Data.Sticky_Dim_Order_Status OS
  ON CONCAT('PD_', CL.Order_Status) = CONCAT('PD_', OS.Order_Status)
