CREATE OR REPLACE VIEW `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_AT` AS
WITH combined_data AS (
  ...
  SELECT
    ...
    SUBSTR(time_stamp, 12, 8) AS Time_Stamp,
...
UNION ALL
  ...
  SELECT
    FORMAT_TIMESTAMP('%Y-%m-%d %H:%M:%S', TIMESTAMP(synced_at)) AS last_updated,
    order_id AS Order_Id,
    billing_first_name AS Bill_First,
    billing_last_name AS Bill_Last,
    billing_street_address AS Bill_Address1,
    ...
    prd_offer_id AS Offer_Id,
    cc_first_6 AS BIN
  FROM `variant-finance-data-project.Sticky_Data.hold_orders_AT`
),
ranked_data AS (
  SELECT *,
    ROW_NUMBER() OVER (
