CREATE PROCEDURE `variant-finance-data-project`.VPU_Merged.proc_Entity_Level_Main_MP()
BEGIN
...
-- 3. NEW: Rebill < 25 at ANY BC → Retention_rate = NULL → All metrics = NULL
...
retention_step_0 AS (
  SELECT 
    wc.*,
    CASE 
      -- BC0: NULL if Subscription_users = 0 OR Rebill_users < minimum_rebill_users
      WHEN wc.Billing_Cycle = 0 
        AND (wc.Subscription_users = 0 
             OR wc.Rebill_users < (SELECT minimum_rebill_users FROM config)) 
        THEN NULL
...
--
-- MODIFICATION 3: NULL CASCADE (NEW)
-- 6. Added minimum_rebill_users = 25 to config
-- 7. Rewrote retention_step_0 through retention_step_4 with NULL injection and cascade
-- 8. Rewrote final_calculations: all metrics NULL when Retention_rate IS NULL
-- 9. Updated final SELECT: Net_LTV_Discounted NULL when Retention_rate IS NULL
--
-- TOTAL CHANGES: 5 + NULL cascade
-- 
