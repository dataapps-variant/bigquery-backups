-- =====================================================
-- IC_7K_30D_Main_Table_AFID — INCREMENTAL APPEND
-- Converted from proc_IC_7K_30D_Main_Table_AFID (full rebuild).
--
...
--   3. CREATE OR REPLACE TABLE  ->  INSERT INTO (explicit 41-column list).
...
CREATE OR REPLACE PROCEDURE `variant-finance-data-project.Icarus_Cohort.proc_IC_7K_30D_Main_Table_AFID_Append`()
BEGIN
  ...
  WHERE Report_date >= DATE_SUB(
    (SELECT MAX(Report_date) FROM `variant-finance-data-project.Icarus_Cohort.IC_7K_30D_Main_Table_AFID`),
  ...
  IF days_to_process > 0 THEN
    ...
    -- =====================================================
    -- INSERT NEW DATA (41 columns)
    -- =====================================================
    INSERT INTO `variant-finance-data-project.Icarus_Cohort.IC_7K_30D_Main_Table_AFID` (
      Report_date, Product_Name_Final, Billing_Cycle, Cohort_Size,
      Limiting_days, Minimum_User_count, Retry_engine_Period, Entity_Name,
      ...
      SS_Users, SS_Value, Rebill_users, Rebill_value,
      Recent_Spend, Recent_Users, T7D_Spend, T7D_Users,
      Churn_rate, Refund_ratio, Recent_CAC, Retention_rate,
      NET_Retention_rate, ARPU_Discounted, Net_ARPU_Discounted, Net_LTV_Discounted,
      Active_Inactive
    )
...
core_metrics AS (
  ...
  LEFT JOIN rebill_metrics rm
    ...
    AND (
      (bedc.Country_Code IS NULL AND rm.Country_Code IS NULL)
      OR bedc.Country_Code = rm.Country_Code
    )
    AND (
      (bedc.AFID_Suffix IS NULL AND rm.AFID_Suffix IS NULL)
      OR bedc.AFID_Suffix = rm.AFID_Suffix
    )
