CREATE OR REPLACE PROCEDURE `variant-finance-data-project.Icarus_Cohort.proc_Final_Table_App_AFID_24BC`()
BEGIN


-- =====================================================
-- FINAL TABLE - APP × AFID_SUFFIX LEVEL
-- Merge of 4 App×AFID_Suffix tables + daily metrics
--
-- GRAIN: Report_date × App_Name × Country_Code × AFID_Suffix × Billing_Cycle × Cohort × Table
-- OUTPUT: 28 columns
-- =====================================================

CREATE OR REPLACE TABLE `variant-finance-data-project.Icarus_Cohort.Final_Table_App_AFID_24BC` AS
...
unioned_data AS (
  ...
  FROM `variant-finance-data-project.Icarus_Cohort.IC_7K_30D_Crystal_Ball_App_AFID_24BC`
  ...
  FROM `variant-finance-data-project.Icarus_Cohort.IC_7K_Crystal_Ball_App_AFID_24BC`
  ...
  FROM `variant-finance-data-project.Icarus_Cohort.IC_7K_30D_Main_Table_App_AFID_24BC`
  ...
  FROM `variant-finance-data-project.Icarus_Cohort.IC_7K_Main_Table_App_AFID_24BC`
),
...
-- =====================================================
-- SCRIPT COMPLETE - Final_Table_App_AFID_Suffix_Level
-- Table: variant-finance-data-project.Icarus_Cohort.Final_Table_App_AFID_Suffix_Level
--
-- GRAIN: Report_date × App_Name × Country_Code × AFID_Suffix × Billing_Cycle × Cohort × Table
--
-- SOURCE TABLES (4):
-- 1. IC_7K_30D_Crystal_Ball_App_AFID (CB_User → Rebills_raw)
-- 2. IC_7K_Crystal_Ball_App_AFID (CB_User → Rebills_raw)
-- 3. IC_7K_30D_Main_Table_App_AFID (Rebill_users → Rebills_raw)
-- 4. IC_7K_Main_Table_App_AFID (Rebill_users → Rebills_raw)
--
-- DAILY METRICS SOURCE TABLES:
-- Spend: Merged_Spend_Split_TBL (App_Name + Final_URL_suffix_AFID, NULL-safe)
-- Users: Sticky_data_API_original_V_Merged_TBL (LEFT(App_Name,2) + base.AFID, NULL-safe)
-- SS Users: Same table (RIGHT(Product_name_Final,2)='SS', BC0)
-- T7D SS: Same table (7-day trailing avg of daily SS distinct counts)
--
-- NULL CASCADE:
-- BC0: Never triggers
-- BC1+: Rebills_raw < 25 → cascade forward
-- 8 metrics cascaded, BC4_CAC_Ceiling excluded
--
-- OUTPUT: 28 columns
-- =====================================================
