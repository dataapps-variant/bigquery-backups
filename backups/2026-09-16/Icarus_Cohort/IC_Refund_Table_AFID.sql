...
-- Optimization of proc_IC_Refund_Table_AFID. Same structure as the
...
--     from IC_Refund_Table, which filters on Currency)
...
CREATE OR REPLACE PROCEDURE `variant-finance-data-project.Icarus_Cohort.proc_IC_Refund_Table_AFID`()
BEGIN
  ...
  -- the base table without one, unlike IC_Refund_Table.
  ------------------------------------------------------------------
  ...
  ------------------------------------------------------------------
  CREATE OR REPLACE TABLE `variant-finance-data-project.Icarus_Cohort.IC_Refund_Table_AFID_staging`
  PARTITION BY Report_date
  CLUSTER BY Product_Name_Final, AFID_Suffix, Country_Code, Billing_Cycle
  AS
  WITH
  ...
  ------------------------------------------------------------------
  DROP TABLE IF EXISTS `variant-finance-data-project.Icarus_Cohort.IC_Refund_Table_AFID`;

  ALTER TABLE `variant-finance-data-project.Icarus_Cohort.IC_Refund_Table_AFID_staging`
    RENAME TO IC_Refund_Table_AFID;
...
--    IC_Refund_Table filters base-table joins on Currency; this variant
...
--
-- CREATE OR REPLACE TABLE `variant-finance-data-project.Icarus_Cohort.IC_Refund_Table_AFID_backup`
-- AS SELECT * FROM `variant-finance-data-project.Icarus_Cohort.IC_Refund_Table_AFID`;
--
...
--   bq show --format=prettyjson variant-finance-data-project:Icarus_Cohort.IC_Refund_Table_AFID
--
...
--   WHERE routine_name = 'proc_IC_Refund_Table_AFID';
--
-- STEP 5 — Sanity check
...
-- FROM      `variant-finance-data-project.Icarus_Cohort.IC_Refund_Table_AFID`        n
-- FULL JOIN `variant-finance-data-project.Icarus_Cohort.IC_Refund_Table_AFID_backup` o
...
-- Keep IC_Refund_Table_AFID_backup for a few cycles before dropping it.
--
--
-- =====================================================
-- IF A RUN EVER FAILS
...
--
-- CREATE OR REPLACE TABLE `variant-finance-data-project.Icarus_Cohort.IC_Refund_Table_AFID`
-- PARTITION BY Report_date
-- CLUSTER BY Product_Name_Final, AFID_Suffix, Country_Code, Billing_Cycle
-- AS SELECT * FROM `variant-finance-data-project.Icarus_Cohort.IC_Refund_Table_AFID_backup`;
--
--
-- =====================================================
-- DOWNSTREAM QUERIES
