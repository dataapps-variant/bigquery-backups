-- =====================================================
-- IC_7K_Crystal_Ball_AFID — INCREMENTAL APPEND
-- Converted from proc_IC_7K_Crystal_Ball_AFID (full rebuild).
--
-- WHAT CHANGED (4 things, nothing else):
--   1. DECLARE block + DELETE-last-N-days + window calculation.
--   2. The two date lines in the config CTE now read the append window.
--      NO CROSSOVER: report_start_date IS the earlier bound here.
--   3. CREATE OR REPLACE TABLE  ->  INSERT INTO (explicit 45-column list).
--   4. Final ORDER BY dropped.
--
-- *** READ THE ROLLING-WINDOW NOTE AT THE BOTTOM BEFORE SCHEDULING. ***
-- The rebuild used a rolling 100-day window, so this table has only ever
-- held the last 100 days. Appending changes that.
-- =====================================================

CREATE OR REPLACE PROCEDURE `variant-finance-data-project.Icarus_Cohort.proc_IC_7K_Crystal_Ball_AFID_Append`()
BEGIN

  -- APPEND CONTROL
  DECLARE lookback_days       INT64 DEFAULT 7;
  -- Cold start: reproduce the rebuild's 100-day window (101 back, +1 = 100 days).
  DECLARE fallback_date       DATE  DEFAULT DATE_SUB(CURRENT_DATE(), INTERVAL 101 DAY);
  ...
  WHERE Report_date >= DATE_SUB(
    (SELECT MAX(Report_date) FROM `variant-finance-data-project.Icarus_Cohort.IC_7K_Crystal_Ball_AFID`),
  ...
  SET new_end_date    = DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY);  -- see TODAY note at bottom
  ...
  IF days_to_process > 0 THEN
    ...
    -- =====================================================
    -- INSERT NEW DATA (45 columns)
    -- =====================================================
    INSERT INTO `variant-finance-data-project.Icarus_Cohort.IC_7K_Crystal_Ball_AFID` (
      Report_date, Product_Name_Final, Billing_Cycle, Cohort_Size,
      Limiting_days, Minimum_Rebill_Count, Retry_engine_Period, Entity_Name,
      App_Name, Trial_Type, Trial_Period, Currency,
      Trial_Price, Regular_Price, Regular_BC_period, Exchange_rate,
      Country_Code, AFID_Suffix, BC_start_date, BC_end_date,
      Denominator_BC, cohort_source, Subscription_users, Subscription_value,
      Day_0_user, Day_0_values, SOT_Ratio, CB_User,
      CB_Value, SS_Users, SS_Value, Recent_Spend,
      Recent_Users, T7D_Spend, T7D_Users, Churn_rate,
      Refund_ratio, Recent_CAC, Retention_rate, NET_Retention_rate,
      ARPU_Discounted, Net_ARPU_Discounted, Net_LTV_Discounted, T30D_New_Users,
      Active_Inactive
    )
...
config AS (
  SELECT 
    new_start_date AS report_start_date,   -- APPEND: was DATE_SUB(CURRENT_DATE(), INTERVAL 100 DAY)  (EARLIER bound)
    new_end_date   AS report_end_date,     -- APPEND: was CURRENT_DATE()                              (LATER bound)
    7000 AS cohort_size,
...
raw_eligible_transactions AS (
  SELECT 
    bl.report_date, bl.Product_Name_Final, bl.Country_Code, bl.AFID_Suffix,
...
initial_cohort_selection AS (
  SELECT report_date, Product_Name_Final, Country_Code, AFID_Suffix, billing_cycle,
    calculated_bc_start_date, calculated_denominator_bc, Updated_Cust_ID, Date_of_Sale, Order_Price_Net_of_Tax_USD
  FROM raw_eligible_transactions 
  WHERE transaction_rank <= (SELECT cohort_size FROM config)
),

cohort_analysis AS (
  SELECT report_date, Product_Name_Final, Country_Code, AFID_Suffix, billing_cycle, calculated_bc_start_date,
    MIN(Date_of_Sale) as oldest_transaction_date, COUNT(*) as total_transactions, COUNT(DISTINCT Updated_Cust_ID) as unique_customers
...
bc_end_date_calculation AS (
  SELECT 
    bl.*, ca.oldest_transaction_date, ca.total_transactions, ca.unique_customers,
    CASE 
  ...
  LEFT JOIN cohort_analysis ca
    ...
    AND ((bl.Country_Code IS NULL AND ca.Country_Code IS NULL) OR bl.Country_Code = ca.Country_Code)
    AND ((bl.AFID_Suffix IS NULL AND ca.AFID_Suffix IS NULL) OR bl.AFID_Suffix = ca.AFID_Suffix)
...
-- =====================================================
final_cohort AS (
  SELECT ics.report_date, ics.Product_Name_Final, ics.Country_Code, ics.AFID_Suffix,
    ics.billing_cycle, ics.calculated_denominator_bc, ics.Updated_Cust_ID, ics.Order_Price_Net_of_Tax_USD,
  ...
  INNER JOIN bc_end_date_calculation bedc
    ...
    AND ((ics.Country_Code IS NULL AND bedc.Country_Code IS NULL) OR ics.Country_Code = bedc.Country_Code)
    AND ((ics.AFID_Suffix IS NULL AND bedc.AFID_Suffix IS NULL) OR ics.AFID_Suffix = bedc.AFID_Suffix)
    AND ics.billing_cycle = bedc.billing_cycle
  WHERE bedc.cohort_method = 'use_selected_cohort'
  
  UNION ALL
  
  SELECT bedc.report_date, bedc.Product_Name_Final, bedc.Country_Code, bedc.AFID_Suffix,
    bedc.billing_cycle, bedc.calculated_denominator_bc, base.Updated_Cust_ID, base.Order_Price_Net_of_Tax_USD,
  ...
  INNER JOIN `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_V_Merged_TBL` base
    ...
    AND ((bedc.AFID_Suffix IS NULL AND base.AFID IS NULL) OR bedc.AFID_Suffix = base.AFID)
...
exchange_rate_lookup AS (
  SELECT bedc.report_date, bedc.Product_Name_Final, bedc.Country_Code, bedc.AFID_Suffix, bedc.billing_cycle, bedc.Currency,
...
subscription_metrics AS (
  SELECT fc.report_date, fc.Product_Name_Final, fc.Country_Code, fc.AFID_Suffix, fc.billing_cycle, fc.final_cohort_source,
    COUNT(fc.Updated_Cust_ID) as final_subscription_users, SUM(fc.Order_Price_Net_of_Tax_USD) as final_subscription_value
...
-- =====================================================
-- DAY_0 METRICS (On-time rebills only)
-- =====================================================
day_0_metrics AS (
  SELECT fc_distinct.report_date, fc_distinct.Product_Name_Final, fc_distinct.Country_Code, fc_distinct.AFID_Suffix,
    fc_distinct.billing_cycle, fc_distinct.calculated_denominator_bc,
    COUNT(rebill.Updated_Cust_ID) as final_day_0_user, SUM(rebill.Order_Price_Net_of_Tax_USD) as final_day_0_values
  FROM (
    SELECT DISTINCT report_date, Product_Name_Final, Country_Code, AFID_Suffix, billing_cycle, calculated_denominator_bc, Updated_Cust_ID 
    FROM final_cohort
  ) fc_distinct
  INNER JOIN bc_end_date_calculation bedc
    ON fc_distinct.report_date = bedc.report_date AND fc_distinct.Product_Name_Final = bedc.Product_Name_Final
    AND ((fc_distinct.Country_Code IS NULL AND bedc.Country_Code IS NULL) OR fc_distinct.Country_Code = bedc.Country_Code)
    AND ((fc_distinct.AFID_Suffix IS NULL AND bedc.AFID_Suffix IS NULL) OR fc_distinct.AFID_Suffix = bedc.AFID_Suffix)
  ...
  LEFT JOIN `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_V_Merged_TBL` rebill
    ...
    AND ((fc_distinct.AFID_Suffix IS NULL AND rebill.AFID IS NULL) OR fc_distinct.AFID_Suffix = rebill.AFID)
...
-- =====================================================
-- SOT_RATIO LOOKUP (from IC_7K_SOT_Ratio_AFID)
-- =====================================================
sot_ratio_lookup AS (
  SELECT bedc.report_date, bedc.Product_Name_Final, bedc.Country_Code, bedc.AFID_Suffix, bedc.billing_cycle,
    COALESCE(sot.SOT_Ratio, 0) as final_sot_ratio
  FROM bc_end_date_calculation bedc
  LEFT JOIN `variant-finance-data-project.Icarus_Cohort.IC_7K_SOT_Ratio_AFID` sot
    ON bedc.report_date = sot.Report_date AND bedc.Product_Name_Final = sot.Product_Name_Final
    AND ((bedc.Country_Code IS NULL AND sot.Country_Code IS NULL) OR bedc.Country_Code = sot.Country_Code)
    AND ((bedc.AFID_Suffix IS NULL AND sot.AFID_Suffix IS NULL) OR bedc.AFID_Suffix = sot.AFID_Suffix)
...
single_sale_metrics AS (
  SELECT bedc.report_date, bedc.Product_Name_Final, bedc.Country_Code, bedc.AFID_Suffix, bedc.billing_cycle,
    COUNT(DISTINCT ss.Updated_Cust_ID) as final_ss_users, SUM(ss.Order_Price_Net_of_Tax_Allocated_USD) as final_ss_value
  FROM bc_end_date_calculation bedc
  LEFT JOIN `variant-finance-data-project.Sticky_Data.Sales_SS_original_API_Merged_TBL` ss
    ...
    AND ((bedc.AFID_Suffix IS NULL AND ss.AFID IS NULL) OR bedc.AFID_Suffix = ss.AFID)
...
ad_spend_metrics AS (
  SELECT bedc.report_date, bedc.Product_Name_Final, bedc.Country_Code, bedc.AFID_Suffix, bedc.billing_cycle,
    SUM(CASE 
      WHEN bedc.Country_Code IS NULL OR bedc.Country_Code = '' THEN ads.allocated_spend
  ...
  LEFT JOIN `variant-finance-data-project.Ad_spend_data.Merged_Spend_Split_TBL` ads
    ...
    AND ((bedc.AFID_Suffix IS NULL AND ads.Final_URL_suffix_AFID IS NULL) OR bedc.AFID_Suffix = ads.Final_URL_suffix_AFID)
...
t30d_new_users_calc AS (
  SELECT bedc.report_date, bedc.Product_Name_Final, bedc.Country_Code, bedc.AFID_Suffix, bedc.Trial_Type,
  ...
  LEFT JOIN `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_V_Merged_TBL` base
    ...
    AND base.Billing_Cycle_Updated = CASE WHEN bedc.Trial_Type = 'NT' THEN 1 ELSE 0 END
    AND (
      (bedc.Country_Code = 'JP' AND base.Spend_Country_Code_AFID = 'JP')
      OR (bedc.Country_Code = 'Non-JP' AND (base.Spend_Country_Code_AFID != 'JP' OR base.Spend_Country_Code_AFID IS NULL))
      OR (bedc.Country_Code IS NULL OR bedc.Country_Code = '')
...
core_metrics AS (
  SELECT 
    ...
    CASE WHEN (bedc.Trial_Type = 'NT' AND bedc.billing_cycle IN (0, 1)) OR bedc.billing_cycle = 0 
      THEN COALESCE(sm.final_subscription_users, 0) ELSE COALESCE(d0m.final_day_0_user, 0) END as clean_day_0_user,
    CASE WHEN (bedc.Trial_Type = 'NT' AND bedc.billing_cycle IN (0, 1)) OR bedc.billing_cycle = 0 
      THEN COALESCE(sm.final_subscription_value, 0) ELSE COALESCE(d0m.final_day_0_values, 0) END as clean_day_0_values,
    sot.final_sot_ratio as clean_sot_ratio,
    COALESCE(SAFE_DIVIDE(
      CASE WHEN (bedc.Trial_Type = 'NT' AND bedc.billing_cycle IN (0, 1)) OR bedc.billing_cycle = 0 
        THEN COALESCE(sm.final_subscription_users, 0) ELSE COALESCE(d0m.final_day_0_user, 0) END,
      NULLIF(sot.final_sot_ratio, 0)), 0) as clean_cb_user,
    COALESCE(SAFE_DIVIDE(
      CASE WHEN (bedc.Trial_Type = 'NT' AND bedc.billing_cycle IN (0, 1)) OR bedc.billing_cycle = 0 
        THEN COALESCE(sm.final_subscription_value, 0) ELSE COALESCE(d0m.final_day_0_values, 0) END,
      NULLIF(sot.final_sot_ratio, 0)), 0) as clean_cb_value,
  ...
  LEFT JOIN exchange_rate_lookup erl
    ON bedc.report_date = erl.report_date AND bedc.Product_Name_Final = erl.Product_Name_Final
    AND ((bedc.Country_Code IS NULL AND erl.Country_Code IS NULL) OR bedc.Country_Code = erl.Country_Code)
    AND ((bedc.AFID_Suffix IS NULL AND erl.AFID_Suffix IS NULL) OR bedc.AFID_Suffix = erl.AFID_Suffix)
    AND bedc.billing_cycle = erl.billing_cycle
  LEFT JOIN subscription_metrics sm
    ON bedc.report_date = sm.report_date AND bedc.Product_Name_Final = sm.Product_Name_Final
    AND ((bedc.Country_Code IS NULL AND sm.Country_Code IS NULL) OR bedc.Country_Code = sm.Country_Code)
    AND ((bedc.AFID_Suffix IS NULL AND sm.AFID_Suffix IS NULL) OR bedc.AFID_Suffix = sm.AFID_Suffix)
    AND bedc.billing_cycle = sm.billing_cycle
  LEFT JOIN single_sale_metrics ssm
    ON bedc.report_date = ssm.report_date AND bedc.Product_Name_Final = ssm.Product_Name_Final
    AND ((bedc.Country_Code IS NULL AND ssm.Country_Code IS NULL) OR bedc.Country_Code = ssm.Country_Code)
    AND ((bedc.AFID_Suffix IS NULL AND ssm.AFID_Suffix IS NULL) OR bedc.AFID_Suffix = ssm.AFID_Suffix)
    AND bedc.billing_cycle = ssm.billing_cycle
  LEFT JOIN day_0_metrics d0m
    ON bedc.report_date = d0m.report_date AND bedc.Product_Name_Final = d0m.Product_Name_Final
    AND ((bedc.Country_Code IS NULL AND d0m.Country_Code IS NULL) OR bedc.Country_Code = d0m.Country_Code)
    AND ((bedc.AFID_Suffix IS NULL AND d0m.AFID_Suffix IS NULL) OR bedc.AFID_Suffix = d0m.AFID_Suffix)
  ...
  LEFT JOIN recent_spend_metrics rsm
    ON bedc.report_date = rsm.report_date AND bedc.Product_Name_Final = rsm.Product_Name_Final
    AND ((bedc.Country_Code IS NULL AND rsm.Country_Code IS NULL) OR bedc.Country_Code = rsm.Country_Code)
    AND ((bedc.AFID_Suffix IS NULL AND rsm.AFID_Suffix IS NULL) OR bedc.AFID_Suffix = rsm.AFID_Suffix)
    AND bedc.billing_cycle = rsm.billing_cycle
  LEFT JOIN recent_users_metrics rum
    ON bedc.report_date = rum.report_date AND bedc.Product_Name_Final = rum.Product_Name_Final
    AND ((bedc.Country_Code IS NULL AND rum.Country_Code IS NULL) OR bedc.Country_Code = rum.Country_Code)
    AND ((bedc.AFID_Suffix IS NULL AND rum.AFID_Suffix IS NULL) OR bedc.AFID_Suffix = rum.AFID_Suffix)
    AND bedc.billing_cycle = rum.billing_cycle
  LEFT JOIN t30d_new_users_calc t30d
    ON bedc.report_date = t30d.report_date AND bedc.Product_Name_Final = t30d.Product_Name_Final
    AND ((bedc.Country_Code IS NULL AND t30d.Country_Code IS NULL) OR bedc.Country_Code = t30d.Country_Code)
    AND ((bedc.AFID_Suffix IS NULL AND t30d.AFID_Suffix IS NULL) OR bedc.AFID_Suffix = t30d.AFID_Suffix)
),
...
calculated_ratios AS (
  SELECT cm.*,
    CASE WHEN cm.billing_cycle = 0 THEN 0.00 WHEN cm.Trial_Type = 'NT' AND cm.billing_cycle = 1 THEN 0.00
      ELSE 1 - COALESCE(SAFE_DIVIDE(cm.clean_cb_user, NULLIF(cm.clean_subscription_users, 0)), 0) END as base_churn_rate,
    ...
    COALESCE(CASE WHEN (cm.Trial_Type = 'NT' AND cm.billing_cycle = 1) OR cm.billing_cycle = 0 
      THEN SAFE_DIVIDE(cm.clean_recent_spend, NULLIF(cm.clean_recent_users, 0)) ELSE 0.0 END, 0.0) as base_recent_cac
  ...
  LEFT JOIN `variant-finance-data-project.Icarus_Cohort.IC_Refund_Table_AFID` rt
    ...
    AND ((cm.Country_Code IS NULL AND rt.Country_Code IS NULL) OR cm.Country_Code = rt.Country_Code)
    AND ((cm.AFID_Suffix IS NULL AND rt.AFID_Suffix IS NULL) OR cm.AFID_Suffix = rt.AFID_Suffix)
    AND cm.billing_cycle = rt.Billing_Cycle
),

-- =====================================================
-- RETENTION RATE CALCULATION (13 STEPS)
  ...
  SELECT rs0.*,
    CASE WHEN rs0.step_0_retention IS NOT NULL THEN rs0.step_0_retention
      WHEN rs0.billing_cycle = 1 AND rs0.Trial_Type != 'NT' THEN 
        CASE WHEN LAG(rs0.step_0_retention) OVER (PARTITION BY rs0.report_date, rs0.Product_Name_Final, rs0.Country_Code, rs0.AFID_Suffix ORDER BY rs0.billing_cycle) IS NULL THEN NULL
  ...
  SELECT rs1.*,
    CASE WHEN rs1.step_1_retention IS NOT NULL THEN rs1.step_1_retention
      WHEN rs1.billing_cycle = 2 THEN 
        CASE WHEN LAG(rs1.step_1_retention) OVER (PARTITION BY rs1.report_date, rs1.Product_Name_Final, rs1.Country_Code, rs1.AFID_Suffix ORDER BY rs1.billing_cycle) IS NULL THEN NULL
          WHEN rs1.clean_cb_user < rs1.minimum_rebill_users THEN 0.0
          ELSE LAG(rs1.step_1_retention) OVER (PARTITION BY rs1.report_date, rs1.Product_Name_Final, rs1.Country_Code, rs1.AFID_Suffix ORDER BY rs1.billing_cycle) * (1 - rs1.base_churn_rate) END
  ...
  SELECT rs2.*,
    CASE WHEN rs2.step_2_retention IS NOT NULL THEN rs2.step_2_retention
      WHEN rs2.billing_cycle = 3 THEN 
        CASE WHEN LAG(rs2.step_2_retention) OVER (PARTITION BY rs2.report_date, rs2.Product_Name_Final, rs2.Country_Code, rs2.AFID_Suffix ORDER BY rs2.billing_cycle) IS NULL THEN NULL
          WHEN rs2.clean_cb_user < rs2.minimum_rebill_users THEN 0.0
          ELSE LAG(rs2.step_2_retention) OVER (PARTITION BY rs2.report_date, rs2.Product_Name_Final, rs2.Country_Code, rs2.AFID_Suffix ORDER BY rs2.billing_cycle) * (1 - rs2.base_churn_rate) END
  ...
  SELECT rs3.*,
    CASE WHEN rs3.step_3_retention IS NOT NULL THEN rs3.step_3_retention
      WHEN rs3.billing_cycle = 4 THEN 
        CASE WHEN LAG(rs3.step_3_retention) OVER (PARTITION BY rs3.report_date, rs3.Product_Name_Final, rs3.Country_Code, rs3.AFID_Suffix ORDER BY rs3.billing_cycle) IS NULL THEN NULL
          WHEN rs3.clean_cb_user < rs3.minimum_rebill_users THEN 0.0
          ELSE LAG(rs3.step_3_retention) OVER (PARTITION BY rs3.report_date, rs3.Product_Name_Final, rs3.Country_Code, rs3.AFID_Suffix ORDER BY rs3.billing_cycle) * (1 - rs3.base_churn_rate) END
      ELSE rs3.step_3_retention END as step_4_retention 
  ...
  SELECT rs4.*,
    CASE WHEN rs4.step_4_retention IS NOT NULL THEN rs4.step_4_retention
      WHEN rs4.billing_cycle = 5 THEN 
        CASE WHEN LAG(rs4.step_4_retention) OVER (PARTITION BY rs4.report_date, rs4.Product_Name_Final, rs4.Country_Code, rs4.AFID_Suffix ORDER BY rs4.billing_cycle) IS NULL THEN NULL
          WHEN rs4.clean_cb_user < rs4.minimum_rebill_users THEN 0.0
          ELSE LAG(rs4.step_4_retention) OVER (PARTITION BY rs4.report_date, rs4.Product_Name_Final, rs4.Country_Code, rs4.AFID_Suffix ORDER BY rs4.billing_cycle) * (1 - rs4.base_churn_rate) END
  ...
  SELECT rs5.*,
    CASE WHEN rs5.step_5_retention IS NOT NULL THEN rs5.step_5_retention
      WHEN rs5.billing_cycle = 6 THEN 
        CASE WHEN LAG(rs5.step_5_retention) OVER (PARTITION BY rs5.report_date, rs5.Product_Name_Final, rs5.Country_Code, rs5.AFID_Suffix ORDER BY rs5.billing_cycle) IS NULL THEN NULL
          WHEN rs5.clean_cb_user < rs5.minimum_rebill_users THEN 0.0
          ELSE LAG(rs5.step_5_retention) OVER (PARTITION BY rs5.report_date, rs5.Product_Name_Final, rs5.Country_Code, rs5.AFID_Suffix ORDER BY rs5.billing_cycle) * (1 - rs5.base_churn_rate) END
  ...
  SELECT rs6.*,
    CASE WHEN rs6.step_6_retention IS NOT NULL THEN rs6.step_6_retention
      WHEN rs6.billing_cycle = 7 THEN 
        CASE WHEN LAG(rs6.step_6_retention) OVER (PARTITION BY rs6.report_date, rs6.Product_Name_Final, rs6.Country_Code, rs6.AFID_Suffix ORDER BY rs6.billing_cycle) IS NULL THEN NULL
          WHEN rs6.clean_cb_user < rs6.minimum_rebill_users THEN 0.0
          ELSE LAG(rs6.step_6_retention) OVER (PARTITION BY rs6.report_date, rs6.Product_Name_Final, rs6.Country_Code, rs6.AFID_Suffix ORDER BY rs6.billing_cycle) * (1 - rs6.base_churn_rate) END
  ...
  SELECT rs7.*,
    CASE WHEN rs7.step_7_retention IS NOT NULL THEN rs7.step_7_retention
      WHEN rs7.billing_cycle = 8 THEN 
        CASE WHEN LAG(rs7.step_7_retention) OVER (PARTITION BY rs7.report_date, rs7.Product_Name_Final, rs7.Country_Code, rs7.AFID_Suffix ORDER BY rs7.billing_cycle) IS NULL THEN NULL
          WHEN rs7.clean_cb_user < rs7.minimum_rebill_users THEN 0.0
          ELSE LAG(rs7.step_7_retention) OVER (PARTITION BY rs7.report_date, rs7.Product_Name_Final, rs7.Country_Code, rs7.AFID_Suffix ORDER BY rs7.billing_cycle) * (1 - rs7.base_churn_rate) END
  ...
  SELECT rs8.*,
    CASE WHEN rs8.step_8_retention IS NOT NULL THEN rs8.step_8_retention
      WHEN rs8.billing_cycle = 9 THEN 
        CASE WHEN LAG(rs8.step_8_retention) OVER (PARTITION BY rs8.report_date, rs8.Product_Name_Final, rs8.Country_Code, rs8.AFID_Suffix ORDER BY rs8.billing_cycle) IS NULL THEN NULL
          WHEN rs8.clean_cb_user < rs8.minimum_rebill_users THEN 0.0
          ELSE LAG(rs8.step_8_retention) OVER (PARTITION BY rs8.report_date, rs8.Product_Name_Final, rs8.Country_Code, rs8.AFID_Suffix ORDER BY rs8.billing_cycle) * (1 - rs8.base_churn_rate) END
  ...
  SELECT rs9.*,
    CASE WHEN rs9.step_9_retention IS NOT NULL THEN rs9.step_9_retention
      WHEN rs9.billing_cycle = 10 THEN 
        CASE WHEN LAG(rs9.step_9_retention) OVER (PARTITION BY rs9.report_date, rs9.Product_Name_Final, rs9.Country_Code, rs9.AFID_Suffix ORDER BY rs9.billing_cycle) IS NULL THEN NULL
          WHEN rs9.clean_cb_user < rs9.minimum_rebill_users THEN 0.0
          ELSE LAG(rs9.step_9_retention) OVER (PARTITION BY rs9.report_date, rs9.Product_Name_Final, rs9.Country_Code, rs9.AFID_Suffix ORDER BY rs9.billing_cycle) * (1 - rs9.base_churn_rate) END
  ...
  SELECT rs10.*,
    CASE WHEN rs10.step_10_retention IS NOT NULL THEN rs10.step_10_retention
      WHEN rs10.billing_cycle = 11 THEN 
        CASE WHEN LAG(rs10.step_10_retention) OVER (PARTITION BY rs10.report_date, rs10.Product_Name_Final, rs10.Country_Code, rs10.AFID_Suffix ORDER BY rs10.billing_cycle) IS NULL THEN NULL
          WHEN rs10.clean_cb_user < rs10.minimum_rebill_users THEN 0.0
          ELSE LAG(rs10.step_10_retention) OVER (PARTITION BY rs10.report_date, rs10.Product_Name_Final, rs10.Country_Code, rs10.AFID_Suffix ORDER BY rs10.billing_cycle) * (1 - rs10.base_churn_rate) END
      ...
      WHEN rs12.billing_cycle = 13 THEN 
        CASE
          WHEN LAG(rs12.step_12_retention) OVER (
            PARTITION BY rs12.report_date, rs12.Product_Name_Final, rs12.Country_Code, rs12.AFID_Suffix
            ORDER BY rs12.billing_cycle
          ) IS NULL THEN NULL
          WHEN rs12.clean_cb_user < rs12.minimum_rebill_users THEN 0.0
          ELSE LAG(rs12.step_12_retention) OVER (
            PARTITION BY rs12.report_date, rs12.Product_Name_Final, rs12.Country_Code, rs12.AFID_Suffix
            ORDER BY rs12.billing_cycle
          ) * (1 - rs12.base_churn_rate)
        END
      ELSE NULL
      ...
      WHEN rs13.billing_cycle = 14 THEN 
        CASE
          WHEN LAG(rs13.step_13_retention) OVER (
            PARTITION BY rs13.report_date, rs13.Product_Name_Final, rs13.Country_Code, rs13.AFID_Suffix
            ORDER BY rs13.billing_cycle
          ) IS NULL THEN NULL
          WHEN rs13.clean_cb_user < rs13.minimum_rebill_users THEN 0.0
          ELSE LAG(rs13.step_13_retention) OVER (
            PARTITION BY rs13.report_date, rs13.Product_Name_Final, rs13.Country_Code, rs13.AFID_Suffix
            ORDER BY rs13.billing_cycle
          ) * (1 - rs13.base_churn_rate)
        END
      ELSE NULL
      ...
      WHEN rs14.billing_cycle = 15 THEN 
        CASE
          WHEN LAG(rs14.step_14_retention) OVER (
            PARTITION BY rs14.report_date, rs14.Product_Name_Final, rs14.Country_Code, rs14.AFID_Suffix
            ORDER BY rs14.billing_cycle
          ) IS NULL THEN NULL
          WHEN rs14.clean_cb_user < rs14.minimum_rebill_users THEN 0.0
          ELSE LAG(rs14.step_14_retention) OVER (
            PARTITION BY rs14.report_date, rs14.Product_Name_Final, rs14.Country_Code, rs14.AFID_Suffix
            ORDER BY rs14.billing_cycle
          ) * (1 - rs14.base_churn_rate)
        END
      ELSE NULL
      ...
      WHEN rs15.billing_cycle = 16 THEN 
        CASE
          WHEN LAG(rs15.step_15_retention) OVER (
            PARTITION BY rs15.report_date, rs15.Product_Name_Final, rs15.Country_Code, rs15.AFID_Suffix
            ORDER BY rs15.billing_cycle
          ) IS NULL THEN NULL
          WHEN rs15.clean_cb_user < rs15.minimum_rebill_users THEN 0.0
          ELSE LAG(rs15.step_15_retention) OVER (
            PARTITION BY rs15.report_date, rs15.Product_Name_Final, rs15.Country_Code, rs15.AFID_Suffix
            ORDER BY rs15.billing_cycle
          ) * (1 - rs15.base_churn_rate)
        END
      ELSE NULL
      ...
      WHEN rs16.billing_cycle = 17 THEN 
        CASE
          WHEN LAG(rs16.step_16_retention) OVER (
            PARTITION BY rs16.report_date, rs16.Product_Name_Final, rs16.Country_Code, rs16.AFID_Suffix
            ORDER BY rs16.billing_cycle
          ) IS NULL THEN NULL
          WHEN rs16.clean_cb_user < rs16.minimum_rebill_users THEN 0.0
          ELSE LAG(rs16.step_16_retention) OVER (
            PARTITION BY rs16.report_date, rs16.Product_Name_Final, rs16.Country_Code, rs16.AFID_Suffix
            ORDER BY rs16.billing_cycle
          ) * (1 - rs16.base_churn_rate)
        END
      ELSE NULL
      ...
      WHEN rs17.billing_cycle = 18 THEN 
        CASE
          WHEN LAG(rs17.step_17_retention) OVER (
            PARTITION BY rs17.report_date, rs17.Product_Name_Final, rs17.Country_Code, rs17.AFID_Suffix
            ORDER BY rs17.billing_cycle
          ) IS NULL THEN NULL
          WHEN rs17.clean_cb_user < rs17.minimum_rebill_users THEN 0.0
          ELSE LAG(rs17.step_17_retention) OVER (
            PARTITION BY rs17.report_date, rs17.Product_Name_Final, rs17.Country_Code, rs17.AFID_Suffix
            ORDER BY rs17.billing_cycle
          ) * (1 - rs17.base_churn_rate)
        END
      ELSE NULL
      ...
      WHEN rs18.billing_cycle = 19 THEN 
        CASE
          WHEN LAG(rs18.step_18_retention) OVER (
            PARTITION BY rs18.report_date, rs18.Product_Name_Final, rs18.Country_Code, rs18.AFID_Suffix
            ORDER BY rs18.billing_cycle
          ) IS NULL THEN NULL
          WHEN rs18.clean_cb_user < rs18.minimum_rebill_users THEN 0.0
          ELSE LAG(rs18.step_18_retention) OVER (
            PARTITION BY rs18.report_date, rs18.Product_Name_Final, rs18.Country_Code, rs18.AFID_Suffix
            ORDER BY rs18.billing_cycle
          ) * (1 - rs18.base_churn_rate)
        END
      ELSE NULL
      ...
      WHEN rs19.billing_cycle = 20 THEN 
        CASE
          WHEN LAG(rs19.step_19_retention) OVER (
            PARTITION BY rs19.report_date, rs19.Product_Name_Final, rs19.Country_Code, rs19.AFID_Suffix
            ORDER BY rs19.billing_cycle
          ) IS NULL THEN NULL
          WHEN rs19.clean_cb_user < rs19.minimum_rebill_users THEN 0.0
          ELSE LAG(rs19.step_19_retention) OVER (
            PARTITION BY rs19.report_date, rs19.Product_Name_Final, rs19.Country_Code, rs19.AFID_Suffix
            ORDER BY rs19.billing_cycle
          ) * (1 - rs19.base_churn_rate)
        END
      ELSE NULL
      ...
      WHEN rs20.billing_cycle = 21 THEN 
        CASE
          WHEN LAG(rs20.step_20_retention) OVER (
            PARTITION BY rs20.report_date, rs20.Product_Name_Final, rs20.Country_Code, rs20.AFID_Suffix
            ORDER BY rs20.billing_cycle
          ) IS NULL THEN NULL
          WHEN rs20.clean_cb_user < rs20.minimum_rebill_users THEN 0.0
          ELSE LAG(rs20.step_20_retention) OVER (
            PARTITION BY rs20.report_date, rs20.Product_Name_Final, rs20.Country_Code, rs20.AFID_Suffix
            ORDER BY rs20.billing_cycle
          ) * (1 - rs20.base_churn_rate)
        END
      ELSE NULL
      ...
      WHEN rs21.billing_cycle = 22 THEN 
        CASE
          WHEN LAG(rs21.step_21_retention) OVER (
            PARTITION BY rs21.report_date, rs21.Product_Name_Final, rs21.Country_Code, rs21.AFID_Suffix
            ORDER BY rs21.billing_cycle
          ) IS NULL THEN NULL
          WHEN rs21.clean_cb_user < rs21.minimum_rebill_users THEN 0.0
          ELSE LAG(rs21.step_21_retention) OVER (
            PARTITION BY rs21.report_date, rs21.Product_Name_Final, rs21.Country_Code, rs21.AFID_Suffix
            ORDER BY rs21.billing_cycle
          ) * (1 - rs21.base_churn_rate)
        END
      ELSE NULL
      ...
      WHEN rs22.billing_cycle = 23 THEN 
        CASE
          WHEN LAG(rs22.step_22_retention) OVER (
            PARTITION BY rs22.report_date, rs22.Product_Name_Final, rs22.Country_Code, rs22.AFID_Suffix
            ORDER BY rs22.billing_cycle
          ) IS NULL THEN NULL
          WHEN rs22.clean_cb_user < rs22.minimum_rebill_users THEN 0.0
          ELSE LAG(rs22.step_22_retention) OVER (
            PARTITION BY rs22.report_date, rs22.Product_Name_Final, rs22.Country_Code, rs22.AFID_Suffix
            ORDER BY rs22.billing_cycle
          ) * (1 - rs22.base_churn_rate)
        END
      ELSE NULL
      ...
      WHEN rs23.billing_cycle = 24 THEN 
        CASE
          WHEN LAG(rs23.step_23_retention) OVER (
            PARTITION BY rs23.report_date, rs23.Product_Name_Final, rs23.Country_Code, rs23.AFID_Suffix
            ORDER BY rs23.billing_cycle
          ) IS NULL THEN NULL
          WHEN rs23.clean_cb_user < rs23.minimum_rebill_users THEN 0.0
          ELSE LAG(rs23.step_23_retention) OVER (
            PARTITION BY rs23.report_date, rs23.Product_Name_Final, rs23.Country_Code, rs23.AFID_Suffix
            ORDER BY rs23.billing_cycle
          ) * (1 - rs23.base_churn_rate)
        END
...
final_calculations AS (
  SELECT rs24.*,
    ...
    rs24.base_refund_ratio as final_refund_ratio,
    CASE WHEN rs24.final_retention_rate IS NULL THEN NULL ELSE rs24.base_recent_cac END as final_recent_cac,
    CASE WHEN rs24.final_retention_rate IS NULL THEN NULL WHEN rs24.final_retention_rate = 0 THEN 0.00
      ELSE rs24.final_retention_rate - rs24.base_refund_ratio END as final_net_retention_rate,
    CASE WHEN rs24.final_retention_rate IS NULL THEN NULL WHEN rs24.final_retention_rate = 0 THEN 0.0
      ELSE COALESCE(SAFE_DIVIDE(rs24.clean_cb_value, NULLIF(rs24.clean_cb_user, 0)) * rs24.final_retention_rate, 0.0)
        + COALESCE(SAFE_DIVIDE(rs24.clean_ss_value, NULLIF(rs24.clean_subscription_users, 0)), 0) END as final_arpu_discounted,
    CASE WHEN rs24.final_retention_rate IS NULL THEN NULL WHEN rs24.final_retention_rate = 0 THEN 0.0
      ELSE COALESCE(SAFE_DIVIDE(rs24.clean_cb_value, NULLIF(rs24.clean_cb_user, 0)) * (rs24.final_retention_rate - rs24.base_refund_ratio), 0.0)
        + COALESCE(SAFE_DIVIDE(rs24.clean_ss_value, NULLIF(rs24.clean_subscription_users, 0)), 0) END as final_net_arpu_discounted
...
-- =====================================================
-- *** ROLLING WINDOW — THE ONE REAL BEHAVIOUR CHANGE ***
-- =====================================================
-- The rebuild's config was:
--     DATE_SUB(CURRENT_DATE(), INTERVAL 100 DAY) AS report_start_date
-- i.e. a ROLLING window. Every run rebuilt the last 100 days and, because
-- CREATE OR REPLACE TABLE replaced the whole table, anything older than 100
-- days was DISCARDED. The table has therefore never held more than 100 days.
--
-- This append only ever adds days and deletes the 8-day restatement window.
-- Nothing trims the tail. So the table will now GROW without bound.
--
-- Decide which you want:
--
--   (a) Keep the 100-day rolling behaviour — add a trim after the INSERT:
--         DELETE FROM <table>
--         WHERE Report_date < DATE_SUB(CURRENT_DATE(), INTERVAL 100 DAY);
--
--   (b) Let it accumulate — this is probably why you are moving to append,
--       but confirm nothing downstream assumes a 100-day table (a dashboard
--       doing "all time" over what used to be 100 days will change).
--
-- Cold start: fallback_date is CURRENT_DATE()-101 so the first run on an
-- empty table reproduces exactly the rebuild's 100-day window. It is NOT
-- 2025-01-01 like the non-AFID refund procs — copying that value here would
-- make the first run build ~600 days.
--
--
-- =====================================================
-- TODAY vs YESTERDAY
-- =====================================================
-- The rebuild used report_end_date = CURRENT_DATE(), writing a partial row
-- for TODAY on every run. This append uses CURRENT_DATE() - 1 so the last
-- row is always a complete day. To restore the old behaviour:
--     SET new_end_date = CURRENT_DATE();
--
--
-- =====================================================
-- WHY ONLY THE CONFIG CTE NEEDED CHANGING
-- =====================================================
-- Every lookback reaches into a RAW source table, never back into an earlier
-- date of this query's own output:
--   raw_eligible_transactions -> Sticky raw, Date_of_Sale <= bc_start_date
--   exchange_rate_lookup      -> Sticky_Dim_Exchnage_Rate
--   ad_spend / recent_spend   -> Merged_Spend_Split_TBL
--   recent_users / t30d       -> Sticky raw (7-day and 30-day windows)
--   single_sale_metrics       -> Sales_SS raw
-- So narrowing report_dates cannot truncate a lookback. In particular
-- raw_eligible_transactions must keep full history — at BC24 its Date_of_Sale
-- cutoff is far before the append window.
--
-- The retention cascade is safe too: every LAG is
--   PARTITION BY report_date, Product_Name_Final, Country_Code, AFID_Suffix
-- so no billing cycle reads across report dates.
--
--
-- =====================================================
-- RUN ORDER
-- =====================================================
--   proc_IC_Refund_Table_AFID_Append        <- must go FIRST
--   <whatever populates IC_7K_SOT_Ratio_AFID>    (Crystal Ball only)
--   proc_IC_7K_Main_Table_AFID_Append
--   proc_IC_7K_30D_Main_Table_AFID_Append
--   proc_IC_7K_Crystal_Ball_AFID_Append
--   proc_IC_7K_30D_Crystal_Ball_AFID_Append
--
-- calculated_ratios joins IC_Refund_Table_AFID on report_date. If it
-- lags a day, base_refund_ratio falls back to COALESCE(...,0.00) and every
-- Refund_ratio / NET_Retention_rate / Net_ARPU_Discounted for the new day is
-- silently wrong rather than missing.
--
-- Keep lookback_days the same across all five, or restated days on the
-- refund table never propagate into the product-level tables.
--
--
-- =====================================================
...
--        COUNT(DISTINCT AFID_Suffix) AS afid_suffixes,
--        MAX(Billing_Cycle) AS max_bc
-- FROM `variant-finance-data-project.Icarus_Cohort.IC_7K_Crystal_Ball_AFID`;
--
-- last_loaded should be yesterday and max_bc should be 24. Watch `days` —
-- if you did not add the trim from the rolling-window note above, it will
-- climb past 100 and keep going.
--
-- Do NOT gap-check per plan: master_combinations filters
-- report_date >= First_Date_of_Sale, so plans legitimately have no rows
-- before launch. Only table-wide MAX(Report_date) is a valid freshness check.
