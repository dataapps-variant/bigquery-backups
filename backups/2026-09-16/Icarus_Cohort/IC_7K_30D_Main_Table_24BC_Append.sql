-- =====================================================
-- IC_7K_30D_Main_Table_24BC — INCREMENTAL APPEND
-- Converted from proc_IC_7K_30D_Main_Table_24BC (full rebuild).
--
...
--      *** NO CROSSOVER HERE *** — unlike the refund tables, this query
--      names its bounds correctly (report_start_date IS the earlier one),
--      so start maps to start and end maps to end. See the DATE NAMING
--      note at the bottom before copying anything from those files.
--   3. CREATE OR REPLACE TABLE  ->  INSERT INTO (explicit 42-column list).
--   4. Final ORDER BY dropped.
--
-- Every CTE, CASE branch, join and window below is otherwise identical to
-- your rebuild — including the three cascade quirks flagged in the notes.
-- =====================================================

CREATE OR REPLACE PROCEDURE `variant-finance-data-project.Icarus_Cohort.proc_IC_7K_30D_Main_Table_24BC_Append`()
BEGIN
  ...
  -- =====================================================
  DELETE FROM `variant-finance-data-project.Icarus_Cohort.IC_7K_30D_Main_Table_24BC`
  WHERE Report_date >= DATE_SUB(
    (SELECT MAX(Report_date) FROM `variant-finance-data-project.Icarus_Cohort.IC_7K_30D_Main_Table_24BC`),
  ...
  SET last_processed_date = (
    ...
    FROM `variant-finance-data-project.Icarus_Cohort.IC_7K_30D_Main_Table_24BC`
  );
  ...
  IF days_to_process > 0 THEN
    ...
    -- =====================================================
    -- INSERT NEW DATA (42 columns)
    -- =====================================================
    INSERT INTO `variant-finance-data-project.Icarus_Cohort.IC_7K_30D_Main_Table_24BC` (
      Report_date, Product_Name_Final, Billing_Cycle,
      Cohort_Size, Limiting_days, Minimum_User_count, Retry_engine_Period,
      ...
      cohort_source,
      Subscription_users, Subscription_value, SS_Users, SS_Value,
      Rebill_users, Rebill_value,
...
-- =====================================================
-- AFID × PRODUCT × COUNTRY COMBINATIONS FROM AGGREGATED PLAN_LIST
-- =====================================================
...
-- =====================================================
-- RECENT SPEND CALCULATION (report_date - 6 days to report_date)
-- =====================================================
...
-- =====================================================
-- RECENT USERS CALCULATION (report_date - 6 days to report_date)
-- =====================================================
    ...
    CASE 
      ...
      WHEN rs12.billing_cycle = 13 THEN 
        CASE
          ...
          WHEN rs12.clean_rebill_users < rs12.minimum_rebill_users THEN 0.0
    ...
    CASE 
      ...
      WHEN rs13.billing_cycle = 14 THEN 
        CASE
          ...
          WHEN rs13.clean_rebill_users < rs13.minimum_rebill_users THEN 0.0
    ...
    CASE 
      ...
      WHEN rs14.billing_cycle = 15 THEN 
        CASE
          ...
          WHEN rs14.clean_rebill_users < rs14.minimum_rebill_users THEN 0.0
    ...
    CASE 
      ...
      WHEN rs15.billing_cycle = 16 THEN 
        CASE
          ...
          WHEN rs15.clean_rebill_users < rs15.minimum_rebill_users THEN 0.0
    ...
    CASE 
      ...
      WHEN rs16.billing_cycle = 17 THEN 
        CASE
          ...
          WHEN rs16.clean_rebill_users < rs16.minimum_rebill_users THEN 0.0
    ...
    CASE 
      ...
      WHEN rs17.billing_cycle = 18 THEN 
        CASE
          ...
          WHEN rs17.clean_rebill_users < rs17.minimum_rebill_users THEN 0.0
    ...
    CASE 
      ...
      WHEN rs18.billing_cycle = 19 THEN 
        CASE
          ...
          WHEN rs18.clean_rebill_users < rs18.minimum_rebill_users THEN 0.0
    ...
    CASE 
      ...
      WHEN rs19.billing_cycle = 20 THEN 
        CASE
          ...
          WHEN rs19.clean_rebill_users < rs19.minimum_rebill_users THEN 0.0
    ...
    CASE 
      ...
      WHEN rs20.billing_cycle = 21 THEN 
        CASE
          ...
          WHEN rs20.clean_rebill_users < rs20.minimum_rebill_users THEN 0.0
    ...
    CASE 
      ...
      WHEN rs21.billing_cycle = 22 THEN 
        CASE
          ...
          WHEN rs21.clean_rebill_users < rs21.minimum_rebill_users THEN 0.0
    ...
    CASE 
      ...
      WHEN rs22.billing_cycle = 23 THEN 
        CASE
          ...
          WHEN rs22.clean_rebill_users < rs22.minimum_rebill_users THEN 0.0
    ...
    CASE 
      ...
      WHEN rs23.billing_cycle = 24 THEN 
        CASE
          ...
          WHEN rs23.clean_rebill_users < rs23.minimum_rebill_users THEN 0.0
...
final_calculations AS (
  SELECT 
    ...
    CASE 
      ...
      WHEN rs24.final_retention_rate = 0 THEN 0.0
      ELSE COALESCE(
        SAFE_DIVIDE(rs24.clean_rebill_value + COALESCE(rs24.clean_ss_value, 0), NULLIF(rs24.clean_rebill_users, 0)) * (rs24.final_retention_rate - rs24.base_refund_ratio),
        0.0
      )
...
-- =====================================================
-- FINAL PREP - Add computed app name
-- =====================================================
...
-- =====================================================
-- DATE NAMING — READ THIS IF YOU COPY BETWEEN FILES
-- =====================================================
-- This query names its bounds CORRECTLY:
--   GENERATE_DATE_ARRAY(report_start_date, report_end_date)
--   report_start_date = earlier,  report_end_date = later
-- so the append mapping is the obvious one:
--   report_start_date := new_start_date
--   report_end_date   := new_end_date
--
-- The IC_Refund_Table / Refund_Table files are the OPPOSITE: there,
-- report_end_date is the EARLIER bound and the mapping has to cross over.
-- Copying that crossover into this file would build a DESCENDING date array,
-- which returns zero rows with NO error. Check the argument order in
-- GENERATE_DATE_ARRAY before assuming either mapping.
--
--
-- =====================================================
-- TODAY vs YESTERDAY — ONE DELIBERATE BEHAVIOUR CHANGE
-- =====================================================
-- The rebuild used report_end_date = CURRENT_DATE(), so it produced a row
-- for TODAY on every run — a partial day, since today's sales and spend are
-- still landing.
--
-- This append uses new_end_date = CURRENT_DATE() - 1, matching every other
-- append proc in the set, so the last row written is always a complete day.
--
-- If you want the old behaviour back, change the SET in STEP 2 to:
--   SET new_end_date = CURRENT_DATE();
-- It stays idempotent either way — the DELETE window re-covers today on the
-- next run — but you would be storing a partial day that changes shape until
-- midnight, which downstream dashboards tend to read as a real dip.
--
--
-- =====================================================
-- WHY ONLY THE CONFIG CTE NEEDED CHANGING
-- =====================================================
-- Every lookback in this query reaches into a RAW source table, never back
-- into an earlier date of this query's own output:
--
...
--   recent_users_metrics       -> Sticky raw, report_date-6 .. report_date
--   single_sale_metrics        -> Sales_SS raw
--
-- So narrowing report_dates cannot truncate a lookback. In particular
-- raw_eligible_transactions must keep seeing full history — at BC24 the
-- Date_of_Sale cutoff is far before the append window, and filtering it
-- would empty the cohort for every high BC.
--
-- The retention cascade is also safe: every LAG is
--   PARTITION BY report_date, Product_Name_Final, Country_Code, AFID
-- so no billing cycle reads across report dates.
--
--
-- =====================================================
-- THREE CASCADE QUIRKS CARRIED OVER VERBATIM
-- =====================================================
-- These are in your rebuild and are reproduced exactly. None is a bug, but
-- do not "tidy" them without re-baselining:
--
-- 1. retention_step_4 ends with  ELSE rs3.step_3_retention  where every
--    other step uses ELSE NULL. Reachable only when step_3_retention IS
--    NULL, so it evaluates to NULL — functionally identical, cosmetically
--    inconsistent.
--
-- 2. retention_step_24 likewise ends with ELSE rs23.step_23_retention.
--
-- 3. The rebill gate sets retention to 0.0, not NULL:
--      WHEN clean_rebill_users < minimum_rebill_users THEN 0.0
--    The App_level procs use NULL here. 0.0 propagates through the cascade
--    as a real zero and downstream final_calculations has explicit
--    "= 0 THEN 0.0" branches that depend on it, so this difference is
--    load-bearing.
--
--
-- =====================================================
...
-- =====================================================
-- 1. IC_7K_30D_Main_Table_24BC must already exist — run the rebuild once.
--
-- 2. IC_Refund_Table_24BC must be appended FIRST. calculated_ratios joins
--    it on report_date, so if it is a day behind, base_refund_ratio falls
--    back to COALESCE(...,0.00) and every Refund_ratio, NET_Retention_rate
--    and Net_ARPU_Discounted for the new day is silently wrong rather than
--    missing.
--
--      proc_IC_Refund_Table_24BC_Append
--      proc_IC_7K_30D_Main_Table_24BC_Append   <- this one
--
-- 3. Keep lookback_days aligned with IC_Refund_Table_24BC's.
--
--
-- =====================================================
-- limiting_days = 30, NOT 300
-- =====================================================
-- This is the ONLY logic difference between this procedure and
-- proc_IC_7K_30D_Main_Table_24BC's sibling IC_7K_Main_Table_24BC, which
-- uses 300. It is the "30D" in the table name.
--
-- limiting_days drives bc_end_date_calculation: if the cohort's oldest
-- transaction is older than bc_start_date - limiting_days, the row switches
-- from 'use_selected_cohort' to 'use_fresh_join' and the cohort is rebuilt
-- from a 30-day window instead of the top-7000 selection.
--
-- Practical effect: this table flips to fresh_join far more often than the
-- 300-day sibling, so cohort_source is worth watching after each run:
--   SELECT cohort_source, COUNT(*)
--   FROM `variant-finance-data-project.Icarus_Cohort.IC_7K_30D_Main_Table_24BC`
--   WHERE Report_date = DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY)
--   GROUP BY 1;
-- A sudden shift in that mix between runs usually means the source fact
-- table moved, not that the logic changed.
--
--
-- =====================================================
...
--        COUNTIF(Refund_ratio IS NULL) AS null_refunds
-- FROM `variant-finance-data-project.Icarus_Cohort.IC_7K_30D_Main_Table_24BC`;
--
-- last_loaded should be yesterday and max_bc should be 24.
--
-- Do NOT gap-check per plan: master_combinations filters
-- report_date >= First_Date_of_Sale, so a plan legitimately has no rows
-- before its launch. Only table-wide MAX(Report_date) is a valid freshness
-- signal — and it is what drives the append window.
