CREATE OR REPLACE VIEW `variant-finance-data-project.Tiktok_ad_spend_data.Tiktok_final_spend_DT` AS
SELECT 
...
FROM `variant-finance-data-project.Tiktok_ad_spend_data.TikTok_DT_AdsDailyReport` base

-- Join with campaigns to get campaign type (latest record only)
LEFT JOIN (
    SELECT Id, Type, _skyvia_sync
    FROM `variant-finance-data-project.Tiktok_ad_spend_data.TikTok_DT_Campaigns`
...
LEFT JOIN (
    ...
    FROM `variant-finance-data-project.Tiktok_ad_spend_data.TikTok_DT_AdGroups`
    QUALIFY ROW_NUMBER() OVER (PARTITION BY Id ORDER BY _skyvia_sync DESC) = 1
) adgroups
...
LEFT JOIN (
    SELECT Id, Name
    FROM `variant-finance-data-project.Tiktok_ad_spend_data.TikTok_DT_Locations`
