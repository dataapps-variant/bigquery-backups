...
CREATE OR REPLACE PROCEDURE `variant-finance-data-project.ICARUS_Multi.proc_7K_30D_Crystal_Ball_24BC`()
BEGIN
...
CREATE OR REPLACE TABLE `variant-finance-data-project.ICARUS_Multi.7K_30D_Crystal_Ball_24BC` AS
...
sot_ratio_lookup AS (
  ...
  FROM bc_end_date_calculation bedc
  LEFT JOIN `variant-finance-data-project.ICARUS_Multi.7k_SOT_Ratio_24BC` sot  -- CHANGED
...
calculated_ratios AS (
  ...
  LEFT JOIN `variant-finance-data-project.ICARUS_Multi.Refund_Table_24BC` rt
    ...
    CASE 
      ...
      WHEN rs11.billing_cycle = 12 THEN 
        CASE
          ...
          -- Check if clean_cb_user < minimum_rebill_users → set to NULL
...
final_calculations AS (
  SELECT 
    ...
    CASE 
      WHEN rs24.Country_Code IS NOT NULL AND rs24.Country_Code != '' 
      THEN CONCAT(rs24.App_Name, '-', rs24.Country_Code)
      ELSE rs24.App_Name
    ...
    CASE 
      ...
      WHEN rs24.billing_cycle = 0 THEN 
        (rs24.final_retention_rate - rs24.base_refund_ratio) * rs24.Trial_Price * rs24.final_exchange_rate + 
        COALESCE(SAFE_DIVIDE(rs24.clean_single_sale_value, NULLIF(rs24.clean_subscription_users, 0)), 0) - rs24.base_recent_cac
      ELSE 
        (rs24.final_retention_rate - rs24.base_refund_ratio) * rs24.Regular_Price * rs24.final_exchange_rate + 
        COALESCE(SAFE_DIVIDE(rs24.clean_single_sale_value, NULLIF(rs24.clean_subscription_users, 0)), 0) - rs24.base_recent_cac
  ...
  LEFT JOIN (
    ...
    ON CASE 
         ...
         ELSE rs24.App_Name
       END = ap.App_Name 
    AND rs24.Product_Name_Final = ap.Product_Name_Final
)
-- =====================================================
