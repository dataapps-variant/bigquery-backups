CREATE OR REPLACE PROCEDURE `variant-finance-data-project.Utilities.proc_Daily_Discounts_Plan`()
BEGIN
...
SELECT
  ...
  SAFE_DIVIDE(
    SUM(SUM(Plan_Price_Net_of_Tax_USD) - SUM(Order_Price_Net_of_Tax_USD))
      OVER (
        PARTITION BY Date, Country, App_Name, Plan_Name
  ...
  )                                                                 AS Cumulative_Discount_Percentage
