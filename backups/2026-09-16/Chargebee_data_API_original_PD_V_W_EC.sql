-- ============================================================================
-- Chargebee equivalent of: Sticky_Data.Sticky_data_API_original_PD_V_W_EC
-- Source: variant-finance-data-project.chargebee.fct_orders_full
-- v2 - updated after reviewing actual fct_orders_full sample data
--
-- Output: same 124 columns, same names, same order as the Sticky view.
--
-- Confirmed from real data:
--   * All timestamps are Unix epoch seconds -> converted with TIMESTAMP_SECONDS()
--   * All money is in cents -> divided by 100
--   * recurring = 'true'/'false' -> converted to '1'/'0'
--   * Tracking fields (afid, sid, c1-c3, aid, opt) exist as flattened columns
--     -> coalesced across the 3 sources (top-level, cf_, meta_data_, customer_cf_)
--   * Real card expiry exists (card_expiry_month/year) -> built as MMYY like Sticky
--   * Billing_Cycle is DERIVED (invoice sequence number per subscription, 0-based)
--     because Chargebee's billing_period means plan length, not rebill count
-- ============================================================================

CREATE OR REPLACE VIEW `variant-finance-data-project.chargebee.Chargebee_data_API_original_PD_V_W_EC` AS
WITH
...
-- No test-order flag in Chargebee. Note: the current sample data is all
-- internal test traffic (@variant.net emails). If you want to exclude those
-- in production, add:
--   WHERE B.customer_email NOT LIKE '%@variant.net%'
;
