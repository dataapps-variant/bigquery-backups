...
-- =====================================================
-- PLAN-SPECIFIC START DATE OVERRIDES
-- Add new plans here as UNION ALL rows when needed
-- =====================================================
plan_start_dates AS (
...
-- UPDATED: Filters report_date using plan-specific start date when available
-- =====================================================
...
final_calculations AS (
  SELECT 
    rs12.*,    -- ✅ Already correct
    
    -- Adjusted ratios (NULL when Retention_rate IS NULL)
    CASE 
      WHEN rs12.final_retention_rate IS NULL THEN NULL    -- ✅ Changed
      ELSE rs12.base_churn_rate    -- ✅ Changed
    END as final_churn_rate,
    
    -- Refund_ratio
   rs12.base_refund_ratio as final_refund_ratio,    -- ✅ Changed
    
    CASE 
      WHEN rs12.final_retention_rate IS NULL THEN NULL    -- ✅ Changed
      ELSE rs12.base_recent_cac    -- ✅ Changed
    END as final_recent_cac,
    
    -- NET_Retention_rate
    CASE 
      WHEN rs12.final_retention_rate IS NULL THEN NULL    -- ✅ Changed
      WHEN rs12.final_retention_rate = 0 THEN 0.00    -- ✅ Changed
      ELSE rs12.final_retention_rate - rs12.base_refund_ratio    -- ✅ Changed
    END as final_net_retention_rate,
    
    -- ARPU_Discounted (NEW)
    CASE 
      WHEN rs12.final_retention_rate IS NULL THEN NULL    -- ✅ Changed
      WHEN rs12.final_retention_rate = 0 THEN 0.0    -- ✅ Changed
      ELSE COALESCE(
        SAFE_DIVIDE(rs12.clean_rebill_value, NULLIF(rs12.clean_rebill_users, 0)) * rs12.final_retention_rate,    -- ✅ Changed
        0.0
      ) + COALESCE(SAFE_DIVIDE(rs12.clean_ss_value, NULLIF(rs12.clean_subscription_users, 0)), 0)    -- ✅ Changed
    END as final_arpu_discounted,
    
    -- Net_ARPU_Discounted (UPDATED: Now includes SS_Value)
    CASE 
      WHEN rs12.final_retention_rate IS NULL THEN NULL    -- ✅ Changed
      WHEN rs12.final_retention_rate = 0 THEN 0.0    -- ✅ Changed
      ELSE COALESCE(
        SAFE_DIVIDE(rs12.clean_rebill_value + COALESCE(rs12.clean_ss_value, 0), NULLIF(rs12.clean_rebill_users, 0)) * (rs12.final_retention_rate - rs12.base_refund_ratio),    -- ✅ Changed
        0.0
      )
    END as final_net_arpu_discounted
    
  FROM retention_step_12 rs12    -- ✅ Changed
),
-- =====================================================
...
-- =====================================================
-- SCRIPT COMPLETE - CWC MAIN TABLE
-- 
-- CHANGE 6: Removed Entity Level (EL) columns
-- CHANGE 7: Added Net_LTV_Discounted column
-- CHANGE 8: Added Active_Inactive column from Active_Plans_6M lookup
-- CHANGE 9: Added plan_start_dates CTE for per-plan start date overrides
--           - PD1000AE now starts from 2026-03-13 instead of 2025-01-01
-- 
-- CTEs REMOVED:
-- - entity_level_prep
-- - entity_level_final
--
-- COLUMNS REMOVED:
-- - Key_EL, BASE_EL, WA_RATIO_EL
-- - T7D_Spend_EL, T7D_Users_EL
-- - Recent_CAC_EL
-- - Net_ARPU_Discounted_EL, Net_BC4_ARPU_EL
--
-- COLUMNS ADDED:
-- - Net_LTV_Discounted (Net_ARPU_Discounted - Recent_CAC)
-- - Active_Inactive (Active if in Active_Plans_6M, else Inactive)
-- =====================================================
