CREATE or REPLACE VIEW `variant-finance-data-project.MS_ad_spend_data.MS_campaigns_final_spend_JF` AS
...
campaign_performance_with_countries AS (
  ...
  FROM `variant-finance-data-project.MS_ad_spend_data.MS_Ads_JF_CampaignPerformanceDailyReport` cp
  LEFT JOIN geographic_country_spend geo
    ON cp.CampaignId = geo.CampaignId 
    AND cp.Date = geo.Date
  WHERE NOT EXISTS (
    SELECT 1 
    FROM `variant-finance-data-project.MS_ad_spend_data.MS_Ads_JF_AdPerformanceDailyReport` ap
