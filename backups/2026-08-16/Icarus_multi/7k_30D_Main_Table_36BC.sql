...
CREATE OR REPLACE PROCEDURE `variant-finance-data-project.ICARUS_Multi.proc_7K_30D_Main_Table`()
BEGIN


-- =====================================================
-- 7K MAIN TABLE CREATION SCRIPT - UPDATED
-- With minimum_rebill_users NULL cascade logic (rebill < 25 = NULL)
-- 
-- UPDATES APPLIED:
-- 1. Plan_List source: VPU.Plan_List → ICARUS_Multi.Plan_List
-- 2. Added Plan_List aggregation logic (4 CTEs)
-- 3. Column names: Product_Name_Final → Product_Name_Final_Merged for transaction tables
-- 4. NEW: Rebill < 25 at ANY BC → Retention_rate = NULL → All metrics = NULL
-- 5. NEW: Added Active/Inactive column based on Active_Plans_6M table
-- =====================================================

CREATE OR REPLACE TABLE `variant-finance-data-project.ICARUS_Multi.7K_30D_Main_Table` AS
    ...
    CASE 
      ...
      WHEN rs24.billing_cycle = 25 THEN 
        CASE
          ...
          -- Check if Rebill_users < minimum_rebill_users → set to NULL
          WHEN rs24.clean_rebill_users < rs24.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
    ...
    CASE 
      ...
      WHEN rs25.billing_cycle = 26 THEN 
        CASE
          ...
          -- Check if Rebill_users < minimum_rebill_users → set to NULL
          WHEN rs25.clean_rebill_users < rs25.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
    ...
    CASE 
      ...
      WHEN rs26.billing_cycle = 27 THEN 
        CASE
          ...
          -- Check if Rebill_users < minimum_rebill_users → set to NULL
          WHEN rs26.clean_rebill_users < rs26.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
    ...
    CASE 
      ...
      WHEN rs27.billing_cycle = 28 THEN 
        CASE
          ...
          -- Check if Rebill_users < minimum_rebill_users → set to NULL
          WHEN rs27.clean_rebill_users < rs27.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
    ...
    CASE 
      ...
      WHEN rs28.billing_cycle = 29 THEN 
        CASE
          ...
          -- Check if Rebill_users < minimum_rebill_users → set to NULL
          WHEN rs28.clean_rebill_users < rs28.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
    ...
    CASE 
      ...
      WHEN rs29.billing_cycle = 30 THEN 
        CASE
          ...
          -- Check if Rebill_users < minimum_rebill_users → set to NULL
          WHEN rs29.clean_rebill_users < rs29.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
    ...
    CASE 
      ...
      WHEN rs30.billing_cycle = 31 THEN 
        CASE
          ...
          -- Check if Rebill_users < minimum_rebill_users → set to NULL
          WHEN rs30.clean_rebill_users < rs30.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
    ...
    CASE 
      ...
      WHEN rs31.billing_cycle = 32 THEN 
        CASE
          ...
          -- Check if Rebill_users < minimum_rebill_users → set to NULL
          WHEN rs31.clean_rebill_users < rs31.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
    ...
    CASE 
      ...
      WHEN rs32.billing_cycle = 33 THEN 
        CASE
          ...
          -- Check if Rebill_users < minimum_rebill_users → set to NULL
          WHEN rs32.clean_rebill_users < rs32.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
    ...
    CASE 
      ...
      WHEN rs33.billing_cycle = 34 THEN 
        CASE
          ...
          -- Check if Rebill_users < minimum_rebill_users → set to NULL
          WHEN rs33.clean_rebill_users < rs33.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
    ...
    CASE 
      ...
      WHEN rs34.billing_cycle = 35 THEN 
        CASE
          ...
          -- Check if Rebill_users < minimum_rebill_users → set to NULL
          WHEN rs34.clean_rebill_users < rs34.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
    ...
    CASE 
      ...
      WHEN rs35.billing_cycle = 36 THEN 
        CASE
          ...
          -- Check if Rebill_users < minimum_rebill_users → set to NULL
          WHEN rs35.clean_rebill_users < rs35.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
...
final_calculations AS (
  SELECT 
    ...
    CASE 
      WHEN rs36.final_retention_rate IS NULL THEN NULL
      ELSE COALESCE(
        SAFE_DIVIDE(rs36.clean_rebill_value, NULLIF(rs36.clean_rebill_users, 0)) * rs36.final_retention_rate,
        0.0
      ) + COALESCE(SAFE_DIVIDE(rs36.clean_single_sale_value, NULLIF(rs36.clean_subscription_users, 0)), 0)
    ...
    CASE 
      WHEN rs36.final_retention_rate IS NULL THEN NULL
      ELSE COALESCE(
        SAFE_DIVIDE(rs36.clean_rebill_value, NULLIF(rs36.clean_rebill_users, 0)) * (rs36.final_retention_rate - rs36.base_refund_ratio),
        0.0
      ) + COALESCE(SAFE_DIVIDE(rs36.clean_single_sale_value, NULLIF(rs36.clean_subscription_users, 0)), 0)
...
-- =====================================================
-- SCRIPT COMPLETE - 7K MAIN TABLE - UPDATED
-- Table: variant-finance-data-project.ICARUS_Multi.7K_30D_Main_Table
-- 
-- KEY CHANGES APPLIED:
-- ✅ Rebill_users < 25 at ANY BC → Retention_rate = NULL
-- ✅ When Retention_rate = NULL → ALL 12 metrics = NULL
-- ✅ NULL cascade from trigger BCs AND non-trigger BCs
-- ✅ Removed all "WHEN final_retention_rate = 0" special cases
-- ✅ Added Active/Inactive column based on Active_Plans_6M table
-- 
-- OUTPUT: 50 columns (was 49, now 50 with Active_Inactive)
-- =====================================================
