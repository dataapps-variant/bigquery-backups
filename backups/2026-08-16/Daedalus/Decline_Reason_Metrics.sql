...
WITH base_declines AS (
  ...
  FROM `variant-finance-data-project.Sticky_Data.Sticky_data_API_Original_V_W_EC_Merged` t
  LEFT JOIN `variant-finance-data-project.Daedalus.Dim_Decline_Reasons` d
...
WITH base_declines AS (
  SELECT 
    ...
    t.AFID,
    CASE 
      WHEN afid.Traffic_Channel = 99 THEN 'Organic'
