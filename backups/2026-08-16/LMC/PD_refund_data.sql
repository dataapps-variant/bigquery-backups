CREATE OR REPLACE TABLE `variant-finance-data-project.LMC.PD_refund_data` AS
WITH 
...
-- Step 2: Find products from other apps that map to PD
cross_app_products AS (
    ...
    WHERE Updated_App_Name != App_Name
        AND Updated_App_Name = 'PD'
),

base_data AS (
    -- Step 1: Direct PD refund data
    SELECT 
...
-- Step 5: Filter to keep only records where first 2 letters of Cohort_Concat = "PD"
WHERE SUBSTRING(COALESCE(upd.Updated_Concat, base.Cohort_Concat), 1, 2) = 'PD'
