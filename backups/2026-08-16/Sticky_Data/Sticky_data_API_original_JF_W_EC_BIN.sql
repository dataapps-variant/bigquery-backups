CREATE VIEW `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_JF_W_EC_BIN`
AS WITH 
...
IP_Parse AS (
  SELECT 
    JF.*,
    'JF' AS Entity_Name,
    LOWER(TRIM(SPLIT(JF.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(JF.IP_Address_Lookup, ' ')) - 1)])) AS last_1_word,
    LOWER(TRIM(
      SPLIT(JF.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(JF.IP_Address_Lookup, ' ')) - 2)] || ' ' ||
      SPLIT(JF.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(JF.IP_Address_Lookup, ' ')) - 1)]
    )) AS last_2_words,
    LOWER(TRIM(
      SPLIT(JF.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(JF.IP_Address_Lookup, ' ')) - 3)] || ' ' ||
      SPLIT(JF.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(JF.IP_Address_Lookup, ' ')) - 2)] || ' ' ||
      SPLIT(JF.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(JF.IP_Address_Lookup, ' ')) - 1)]
    )) AS last_3_words
    
  FROM variant-finance-data-project.Sticky_Data.Sticky_data_API_original_JF_BIN as JF
),
...
Country_Lookup_Base AS (
  SELECT 
    ...
    -- NEW: Two-tier App_Name lookup strategy
    COALESCE(
      CASE 
        WHEN AF.JF_App_Name IS NOT NULL 
         AND TRIM(AF.JF_App_Name) != '' 
         AND UPPER(TRIM(AF.JF_App_Name)) != 'COMBINED' 
        THEN AF.JF_App_Name
        ELSE NULL
      END,
      CAM.App_Name,
...
---------------------------------------------------------
  ...
  -- Tier 1: AFID Lookup - Get JF_App_Name from AFID dimension
  LEFT JOIN Sticky_Data.Sticky_Dim_AFID AF 
    ON ip.AFID = AF.AFID  
  
  -- Tier 2: Campaign Lookup - Get App_Name from Campaign dimension
  LEFT JOIN `Sticky_Data.Sticky_Dim_Campaign` CAM
    ON IP.Entity_Name = CAM.Entity
   AND IP.Campaign_Id = CAM.Campaign_Id
...
SELECT 
  ...
  CONCAT('JF_', CL.Customer_Number,CL.Acquisition_Date_Time) AS Updated_Cust_ID,
  CONCAT('JF_', CL.Order_Id ) AS Updated_Order_ID,
...
LEFT JOIN Sticky_Data.Sticky_Dim_Product PROD 
  ON CONCAT('JF_', CL.Product_Id) = CONCAT(PROD.ENTITY, '_', PROD.Product_ID)
...
LEFT JOIN Sticky_Data.Sticky_Dim_Offer OFR 
  ON CONCAT('JF_', CL.Offer_Id) = CONCAT(OFR.Entity, '_', OFR.ID)
