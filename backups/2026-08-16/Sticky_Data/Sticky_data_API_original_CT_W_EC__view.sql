CREATE VIEW `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_CT_W_EC`
AS WITH 
...
IP_Parse AS (
  SELECT 
    CT.*,
    'CT' AS Entity_Name,
    LOWER(TRIM(SPLIT(CT.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(CT.IP_Address_Lookup, ' ')) - 1)])) AS last_1_word,
    LOWER(TRIM(
      SPLIT(CT.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(CT.IP_Address_Lookup, ' ')) - 2)] || ' ' ||
      SPLIT(CT.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(CT.IP_Address_Lookup, ' ')) - 1)]
    )) AS last_2_words,
    LOWER(TRIM(
      SPLIT(CT.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(CT.IP_Address_Lookup, ' ')) - 3)] || ' ' ||
      SPLIT(CT.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(CT.IP_Address_Lookup, ' ')) - 2)] || ' ' ||
      SPLIT(CT.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(CT.IP_Address_Lookup, ' ')) - 1)]
    )) AS last_3_words
  FROM variant-finance-data-project.Sticky_Data.Sticky_data_API_original_CT as CT
),
...
---------------------------------------------------------
  ...
  LEFT JOIN `Sticky_Data.Sticky_Dim_Campaign` CAM
    ON CONCAT('CT_', ip.Campaign_Id) = CONCAT(CAM.Entity,'_',CAM.Campaign_Id)
----------------------------------------------------------
...
SELECT 
  ...
  CONCAT('CT_', CL.Customer_Number,CL.Acquisition_Date_Time) AS Updated_Cust_ID,
  CONCAT('CT_', CL.Order_Id ) AS Updated_Order_ID,
  ...
  CASE 
    WHEN CL.Campaign_Id IN ('61', '62') THEN 'JP'
    ELSE COALESCE(
      NULLIF(CL.Country_Code, 'OR'),
      CASE WHEN CL.IP_Country_Code IN (SELECT DISTINCT Country_Code FROM Sticky_Data.Sticky_Dim_AFID WHERE Country_Code IS NOT NULL AND TRIM(Country_Code) != '') THEN CL.IP_Country_Code END,
      CASE WHEN CL.Ship_Country IN (SELECT DISTINCT Country_Code FROM Sticky_Data.Sticky_Dim_AFID WHERE Country_Code IS NOT NULL AND TRIM(Country_Code) != '') THEN CL.Ship_Country END,
  ...
  END AS Spend_Country_Code,
  
  CASE 
    WHEN CL.Campaign_Id IN ('61', '62') THEN 'JP'
    ELSE COALESCE(CL.Bill_Country, CL.Ship_Country, CL.IP_Country_Code, CL.Country_Code, 'OR')
...
LEFT JOIN Sticky_Data.Sticky_Dim_Product PROD 
  ON CONCAT('CT_', CL.Product_Id) = CONCAT(PROD.ENTITY, '_', PROD.Product_ID)
...
LEFT JOIN Sticky_Data.Sticky_Dim_Offer OFR 
  ON CONCAT('CT_', CL.Offer_Id) = CONCAT(OFR.Entity, '_', OFR.ID)
