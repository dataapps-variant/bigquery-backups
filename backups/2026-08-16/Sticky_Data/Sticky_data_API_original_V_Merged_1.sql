CREATE VIEW `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_V_Merged_1`
AS WITH Prev_bc AS (
  SELECT *,
         ROW_NUMBER() OVER (
  ...
  FROM Sticky_Data.Sticky_data_API_Original_V_W_EC_Merged_TBL_1 cl
),
Next_bc AS (
  SELECT *,
         ROW_NUMBER() OVER (
...
CL AS (
  SELECT * EXCEPT(Refund_Amount, Refund_Date),
         SAFE_CAST(Refund_Amount AS FLOAT64) AS Refund_Amount,
         SAFE_CAST(Refund_Date AS DATE) AS Refund_Date
  FROM Sticky_Data.Sticky_data_API_Original_V_W_EC_Merged_TBL_1
),
CL_With_Joins AS (
  SELECT 
    CL.*,
