CREATE OR REPLACE VIEW Sticky_Data.Sticky_data_API_original_PD_V_W_EC AS
WITH 
...
IP_Parse AS (
  SELECT 
    PD_V.*,
    'PD' AS Entity_Name,
    LOWER(TRIM(SPLIT(PD_V.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(PD_V.IP_Address_Lookup, ' ')) - 1)])) AS last_1_word,
    LOWER(TRIM(
      SPLIT(PD_V.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(PD_V.IP_Address_Lookup, ' ')) - 2)] || ' ' ||
      SPLIT(PD_V.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(PD_V.IP_Address_Lookup, ' ')) - 1)]
    )) AS last_2_words,
    LOWER(TRIM(
      SPLIT(PD_V.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(PD_V.IP_Address_Lookup, ' ')) - 3)] || ' ' ||
      SPLIT(PD_V.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(PD_V.IP_Address_Lookup, ' ')) - 2)] || ' ' ||
      SPLIT(PD_V.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(PD_V.IP_Address_Lookup, ' ')) - 1)]
    )) AS last_3_words
  FROM variant-finance-data-project.Sticky_Data.Sticky_data_API_original_PD_V as PD_V
),
...
SELECT 
  ...
  COALESCE(CL.Country_Code, CL.IP_Country_Code, CL.Ship_Country, CL.Bill_Country, 'OR') AS Spend_Country,
  COALESCE(CL.Bill_Country, CL.Ship_Country, CL.IP_Country_Code, CL.Country_Code, 'OR') AS Tax_Country,
    OFR_DEF.Trial_Period,
    COALESCE(OFR_CUR.Trial_Price, OFR_DEF.Trial_Price) AS Trial_Price,
  OFR_DEF.Trial_Type,
  PROD_DEF.Product_Name_updated,
  CONCAT(CL.App_Name,PROD_DEF.Product_Name_updated,OFR_DEF.Trial_Type) AS Product_Name_Final,

  CASE 
    WHEN OFR_DEF.Trial_Type = 'NT' 
      THEN SAFE_CAST(CL.Billing_Cycle AS INT64) + 1
    ELSE SAFE_CAST(CL.Billing_Cycle AS INT64)
  END AS Billing_Cycle_Updated,

  COALESCE(CL.Country_Code, CL.IP_Country_Code, CL.Ship_Country, CL.Bill_Country, 'OR') AS Spend_Country_Code,
  ...
  CASE
    WHEN (
      CASE 
        WHEN OFR_DEF.Trial_Type = 'NT' THEN SAFE_CAST(CL.Billing_Cycle AS INT64) + 1
        ELSE SAFE_CAST(CL.Billing_Cycle AS INT64)
      END
    ) = 0 THEN
      CASE 
        WHEN OFR_DEF.Trial_Type = 'SS' THEN SAFE_MULTIPLY(COALESCE(PROD_CUR.Product_Price, PROD_DEF.Product_Price), Cast(CL.Quantity as float64))
        ELSE SAFE_MULTIPLY(COALESCE(OFR_CUR.Trial_Price, OFR_DEF.Trial_Price), Cast(CL.Quantity as float64))
      END
    ELSE SAFE_MULTIPLY(COALESCE(PROD_CUR.Product_Price, PROD_DEF.Product_Price), Cast(CL.Quantity as float64))
...
LEFT JOIN Sticky_Data.Sticky_Dim_Product PROD_CUR
  ON CONCAT('PD_', CL.Product_Id) = CONCAT(PROD_CUR.ENTITY, '_', PROD_CUR.Product_ID)
  AND PROD_CUR.Currency = CL.Currency

LEFT JOIN Sticky_Data.Sticky_Dim_Product PROD_DEF
  ON CONCAT('PD_', CL.Product_Id) = CONCAT(PROD_DEF.ENTITY, '_', PROD_DEF.Product_ID)
  AND (PROD_DEF.Currency = '' OR PROD_DEF.Currency IS NULL)
...
LEFT JOIN Sticky_Data.Sticky_Dim_Offer OFR_CUR
  ON CONCAT('PD_', CL.Offer_Id) = CONCAT(OFR_CUR.Entity, '_', OFR_CUR.ID)
  AND OFR_CUR.Currency = CL.Currency

LEFT JOIN Sticky_Data.Sticky_Dim_Offer OFR_DEF
  ON CONCAT('PD_', CL.Offer_Id) = CONCAT(OFR_DEF.Entity, '_', OFR_DEF.ID)
  AND (OFR_DEF.Currency = '' OR OFR_DEF.Currency IS NULL)
