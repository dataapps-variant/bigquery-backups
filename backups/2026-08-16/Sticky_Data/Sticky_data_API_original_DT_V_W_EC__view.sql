CREATE VIEW `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_DT_V_W_EC`
AS WITH 
...
IP_Parse AS (
  SELECT 
    DT_V.*,
    'DT' AS Entity_Name,
    LOWER(TRIM(SPLIT(DT_V.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(DT_V.IP_Address_Lookup, ' ')) - 1)])) AS last_1_word,
    LOWER(TRIM(
      SPLIT(DT_V.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(DT_V.IP_Address_Lookup, ' ')) - 2)] || ' ' ||
      SPLIT(DT_V.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(DT_V.IP_Address_Lookup, ' ')) - 1)]
    )) AS last_2_words,
    LOWER(TRIM(
      SPLIT(DT_V.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(DT_V.IP_Address_Lookup, ' ')) - 3)] || ' ' ||
      SPLIT(DT_V.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(DT_V.IP_Address_Lookup, ' ')) - 2)] || ' ' ||
      SPLIT(DT_V.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(DT_V.IP_Address_Lookup, ' ')) - 1)]
    )) AS last_3_words
  FROM variant-finance-data-project.Sticky_Data.Sticky_data_API_original_DT_V as DT_V
),
...
---------------------------------------------------------
  ...
  LEFT JOIN `Sticky_Data.Sticky_Dim_Campaign` CAM
    ON CONCAT('DT_', ip.Campaign_Id) = CONCAT(CAM.Entity,'_',CAM.Campaign_Id)
----------------------------------------------------------
...
SELECT 
  ...
  CONCAT('DT_', CL.Customer_Number,CL.Acquisition_Date_Time) AS Updated_Cust_ID,
  CONCAT('DT_', CL.Order_Id ) AS Updated_Order_ID,
...
LEFT JOIN Sticky_Data.Sticky_Dim_Product PROD 
  ON CONCAT('DT_', CL.Product_Id) = CONCAT(PROD.ENTITY, '_', PROD.Product_ID)
...
LEFT JOIN Sticky_Data.Sticky_Dim_Offer OFR 
  ON CONCAT('DT_', CL.Offer_Id) = CONCAT(OFR.Entity, '_', OFR.ID)
