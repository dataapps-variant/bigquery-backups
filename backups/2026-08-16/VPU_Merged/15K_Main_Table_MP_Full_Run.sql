-- =====================================================
-- 15K MAIN TABLE CREATION SCRIPT - MODIFIED VERSION
-- WITH RECENT_CAC (WA Columns Removed)
-- 
-- MODIFICATIONS APPLIED:
-- 1. Dataset migration: VPU → VPU_Merged (5 references)
-- 2. Plan_List aggregation with Country_Code grouping (7 changes)
-- 3. Column name updates for transaction tables (8 changes)
-- 4. Removed all WA columns and weighted_avg_final CTE (14 changes)
-- 
-- UPDATES IN THIS VERSION:
-- 1. Added Recent_Spend calculation (last 7 days before last spend date)
-- 2. Added Recent_Users calculation (last 7 days before last spend date)
...
CREATE OR REPLACE TABLE `variant-finance-data-project.VPU_Merged.15K_Main_Table_MP` AS
...
SELECT 
  ...
  lp.minimum_user_count as Minimum_User_count,
...
-- Table: variant-finance-data-project.VPU_Merged.15K_Main_Table_MP
-- 
-- MODIFICATIONS APPLIED:
-- =====================================================
-- 
-- MODIFICATION 1: DATASET MIGRATION (5 changes)
-- 1. Output table: VPU → VPU_Merged
-- 2-3. Plan_List references (2): VPU → VPU_Merged
-- 4. Refund_Table: VPU → VPU_Merged
-- 5. Comment reference: VPU → VPU_Merged
--
-- MODIFICATION 2: PLAN_LIST AGGREGATION (7 changes)
-- 1-4. Added 4 new CTEs for aggregation
-- 5. Updated product_list to use aggregated_plan_list
-- 6. Updated enriched_master with NULL-safe joins
-- 7. Explicit column list in enriched_master
--
-- MODIFICATION 3: COLUMN NAME UPDATES (8 changes)
-- Sticky Data (4): raw_eligible_transactions, final_cohort, rebill_metrics, recent_users_metrics
-- Single Sales (1): single_sales_metrics
-- Ad Spend (3): ad_spend_metrics, recent_spend_date_window, recent_spend_metrics
-- All changed: Product_Name_Final → Product_Name_Final_Merged
--
-- MODIFICATION 4: REMOVED WA COLUMNS (14 changes)
-- 1. Removed weighted_avg_final CTE completely
-- 2. Renamed weighted_avg_prep to ltv_prep (removed WA calculations)
-- 3. Changed final SELECT FROM clause: waf → lp
-- 4. Updated all column references: waf.* → lp.*
-- 5. Removed 11 WA output columns:
--    - Key, BASE, WA_RATIO
--    - CAC_WA, Recent_CAC_WA
--    - ARPU_WA, ARPU_Discounted_WA
--    - Net_ARPU_WA, Net_ARPU_Discounted_WA
--    - Net_LTV_WA, Net_LTV_Discounted_WA
--
-- TOTAL CHANGES: 34
-- 
-- OUTPUT COLUMNS: 41 (down from 52)
-- =====================================================
