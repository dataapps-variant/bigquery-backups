...
SET new_start_date = COALESCE(
  (SELECT MAX(Report_date) FROM `variant-finance-data-project.VPU.15K_Crystal_Ball`),
  DATE('2025-01-01')
);
SET new_start_date = DATE_SUB(new_start_date, INTERVAL lookback_days DAY);

-- =====================================================
-- DELETE EXISTING DATA FOR LOOKBACK PERIOD
-- =====================================================
DELETE FROM `variant-finance-data-project.VPU.15K_Crystal_Ball`
WHERE Report_date >= new_start_date;
...
-- =====================================================
-- APPEND SCRIPT COMPLETE
-- Table: VPU.15K_Crystal_Ball
-- Methodology: Crystal Ball (15K cohort, retry=1, CB metrics)
-- =====================================================
