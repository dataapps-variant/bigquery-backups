CREATE OR REPLACE PROCEDURE `variant-finance-data-project.ICARUS_Multi.proc_7K_Main_Table_App_level_24BC` ()
BEGIN

CREATE OR REPLACE TABLE `variant-finance-data-project.ICARUS_Multi.7K_Main_Table_App_level_24BC` AS
...
app_base_aggregated AS (
  ...
  FROM `variant-finance-data-project.ICARUS_Multi.7K_Main_Table_24BC`
  WHERE App_Name IN ('CT-JP','CT-Non-JP','CL','EN','FS','JF','PD')
...
vg_last_spend_dates AS (
  ...
  LEFT JOIN `variant-finance-data-project.Ad_spend_data.Merged_Spend_Split_TBL` ads
    ...
    AND ads.allocated_spend > 0
    AND ads.App_Name IN ('CT-JP','CT-Non-JP','CL','EN','FS','JF','PD')
...
vg_recent_users_calc AS (
  ...
  LEFT JOIN `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_V_Merged_TBL` base
    ...
    AND LEFT(base.App_Name, 2) IN ('CT','CL','EN','FS','JF','PD')
  GROUP BY sdw.Report_date, sdw.window_start_date
),
...
vg_refund_from_lookup AS (
  ...
  WHERE aba.App_Name IN ('CT-JP','CT-Non-JP','CL','EN','FS','JF','PD')
