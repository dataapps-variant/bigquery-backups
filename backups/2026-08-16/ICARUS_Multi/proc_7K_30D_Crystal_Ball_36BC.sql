CREATE PROCEDURE `variant-finance-data-project`.ICARUS_Multi.proc_7K_30D_Crystal_Ball_36BC()
BEGIN

-- =====================================================
-- 7K CRYSTAL BALL TABLE CREATION SCRIPT - 30D - UPDATED
-- Cohort Analysis with Crystal Ball Projections
-- 
-- UPDATES IN THIS VERSION:
-- 1. Plan_List source: VPU.Plan_List → ICARUS_Multi.Plan_List
-- 2. Added Plan_List aggregation logic (4 CTEs)
-- 3. Product column: Product_Name_Final → Product_Name_Final_Merged
-- 4. SOT_Ratio source: VPU.15K_SOT_Ratio → ICARUS_Multi.7k_SOT_Ratio
-- 5. Refund source: Already uses ICARUS_Multi.Refund_Table
-- 6. Column positions: SS_Users and T30D_New_Users repositioned
-- 7. Retention logic: CB_User < 25 now sets NULL instead of 0.0
-- 8. Removed "retention = 0" special cases from 6 metrics
-- 9. NT BC0: Always NULL (BC0 not meaningful for NT plans)
-- 10. NEW: Added Active/Inactive column based on Active_Plans_6M table
-- 
-- OUTPUT: 47 columns (Crystal Ball methodology with ICARUS_Multi sources)
-- =====================================================

CREATE OR REPLACE TABLE `variant-finance-data-project.ICARUS_Multi.7K_30D_Crystal_Ball_36BC` AS
...
-- =====================================================
-- SOT_RATIO LOOKUP
-- UPDATED: ICARUS_Multi.7k_SOT_Ratio_36BC
-- =====================================================
sot_ratio_lookup AS (
  SELECT 
    bedc.report_date,
  ...
  FROM bc_end_date_calculation bedc
  LEFT JOIN `variant-finance-data-project.ICARUS_Multi.7k_SOT_Ratio_36BC` sot  -- CHANGED
      ...
      WHEN rs12.billing_cycle = 13 THEN 
        CASE
          ...
          WHEN LAG(rs12.step_12_retention) OVER (
            PARTITION BY rs12.report_date, rs12.Product_Name_Final, rs12.Country_Code
            ORDER BY rs12.billing_cycle
          ) IS NULL THEN NULL
          -- Check if current BC has CB_User < minimum_rebill_users → set to NULL
          WHEN rs12.clean_cb_user < rs12.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
      ...
      WHEN rs13.billing_cycle = 14 THEN 
        CASE
          ...
          WHEN LAG(rs13.step_13_retention) OVER (
            PARTITION BY rs13.report_date, rs13.Product_Name_Final, rs13.Country_Code
            ORDER BY rs13.billing_cycle
          ) IS NULL THEN NULL
          -- Check if current BC has CB_User < minimum_rebill_users → set to NULL
          WHEN rs13.clean_cb_user < rs13.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
      ...
      WHEN rs14.billing_cycle = 15 THEN 
        CASE
          ...
          WHEN LAG(rs14.step_14_retention) OVER (
            PARTITION BY rs14.report_date, rs14.Product_Name_Final, rs14.Country_Code
            ORDER BY rs14.billing_cycle
          ) IS NULL THEN NULL
          -- Check if current BC has CB_User < minimum_rebill_users → set to NULL
          WHEN rs14.clean_cb_user < rs14.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
      ...
      WHEN rs15.billing_cycle = 16 THEN 
        CASE
          ...
          WHEN LAG(rs15.step_15_retention) OVER (
            PARTITION BY rs15.report_date, rs15.Product_Name_Final, rs15.Country_Code
            ORDER BY rs15.billing_cycle
          ) IS NULL THEN NULL
          -- Check if current BC has CB_User < minimum_rebill_users → set to NULL
          WHEN rs15.clean_cb_user < rs15.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
      ...
      WHEN rs16.billing_cycle = 17 THEN 
        CASE
          ...
          WHEN LAG(rs16.step_16_retention) OVER (
            PARTITION BY rs16.report_date, rs16.Product_Name_Final, rs16.Country_Code
            ORDER BY rs16.billing_cycle
          ) IS NULL THEN NULL
          -- Check if current BC has CB_User < minimum_rebill_users → set to NULL
          WHEN rs16.clean_cb_user < rs16.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
      ...
      WHEN rs17.billing_cycle = 18 THEN 
        CASE
          ...
          WHEN LAG(rs17.step_17_retention) OVER (
            PARTITION BY rs17.report_date, rs17.Product_Name_Final, rs17.Country_Code
            ORDER BY rs17.billing_cycle
          ) IS NULL THEN NULL
          -- Check if current BC has CB_User < minimum_rebill_users → set to NULL
          WHEN rs17.clean_cb_user < rs17.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
      ...
      WHEN rs18.billing_cycle = 19 THEN 
        CASE
          ...
          WHEN LAG(rs18.step_18_retention) OVER (
            PARTITION BY rs18.report_date, rs18.Product_Name_Final, rs18.Country_Code
            ORDER BY rs18.billing_cycle
          ) IS NULL THEN NULL
          -- Check if current BC has CB_User < minimum_rebill_users → set to NULL
          WHEN rs18.clean_cb_user < rs18.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
      ...
      WHEN rs19.billing_cycle = 20 THEN 
        CASE
          ...
          WHEN LAG(rs19.step_19_retention) OVER (
            PARTITION BY rs19.report_date, rs19.Product_Name_Final, rs19.Country_Code
            ORDER BY rs19.billing_cycle
          ) IS NULL THEN NULL
          -- Check if current BC has CB_User < minimum_rebill_users → set to NULL
          WHEN rs19.clean_cb_user < rs19.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
      ...
      WHEN rs20.billing_cycle = 21 THEN 
        CASE
          ...
          WHEN LAG(rs20.step_20_retention) OVER (
            PARTITION BY rs20.report_date, rs20.Product_Name_Final, rs20.Country_Code
            ORDER BY rs20.billing_cycle
          ) IS NULL THEN NULL
          -- Check if current BC has CB_User < minimum_rebill_users → set to NULL
          WHEN rs20.clean_cb_user < rs20.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
      ...
      WHEN rs21.billing_cycle = 22 THEN 
        CASE
          ...
          WHEN LAG(rs21.step_21_retention) OVER (
            PARTITION BY rs21.report_date, rs21.Product_Name_Final, rs21.Country_Code
            ORDER BY rs21.billing_cycle
          ) IS NULL THEN NULL
          -- Check if current BC has CB_User < minimum_rebill_users → set to NULL
          WHEN rs21.clean_cb_user < rs21.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
      ...
      WHEN rs22.billing_cycle = 23 THEN 
        CASE
          ...
          WHEN LAG(rs22.step_22_retention) OVER (
            PARTITION BY rs22.report_date, rs22.Product_Name_Final, rs22.Country_Code
            ORDER BY rs22.billing_cycle
          ) IS NULL THEN NULL
          -- Check if current BC has CB_User < minimum_rebill_users → set to NULL
          WHEN rs22.clean_cb_user < rs22.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
      ...
      WHEN rs23.billing_cycle = 24 THEN 
        CASE
          ...
          WHEN LAG(rs23.step_23_retention) OVER (
            PARTITION BY rs23.report_date, rs23.Product_Name_Final, rs23.Country_Code
            ORDER BY rs23.billing_cycle
          ) IS NULL THEN NULL
          -- Check if current BC has CB_User < minimum_rebill_users → set to NULL
          WHEN rs23.clean_cb_user < rs23.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
      ...
      WHEN rs24.billing_cycle = 25 THEN 
        CASE
          ...
          WHEN LAG(rs24.step_24_retention) OVER (
            PARTITION BY rs24.report_date, rs24.Product_Name_Final, rs24.Country_Code
            ORDER BY rs24.billing_cycle
          ) IS NULL THEN NULL
          -- Check if current BC has CB_User < minimum_rebill_users → set to NULL
          WHEN rs24.clean_cb_user < rs24.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
          ELSE LAG(rs24.step_24_retention) OVER (
            PARTITION BY rs24.report_date, rs24.Product_Name_Final, rs24.Country_Code
            ORDER BY rs24.billing_cycle
          ) * (1 - rs24.base_churn_rate)
        END
      ELSE NULL
      ...
      WHEN rs25.billing_cycle = 26 THEN 
        CASE
          ...
          WHEN LAG(rs25.step_25_retention) OVER (
            PARTITION BY rs25.report_date, rs25.Product_Name_Final, rs25.Country_Code
            ORDER BY rs25.billing_cycle
          ) IS NULL THEN NULL
          -- Check if current BC has CB_User < minimum_rebill_users → set to NULL
          WHEN rs25.clean_cb_user < rs25.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
          ELSE LAG(rs25.step_25_retention) OVER (
            PARTITION BY rs25.report_date, rs25.Product_Name_Final, rs25.Country_Code
            ORDER BY rs25.billing_cycle
          ) * (1 - rs25.base_churn_rate)
        END
      ELSE NULL
      ...
      WHEN rs26.billing_cycle = 27 THEN 
        CASE
          ...
          WHEN LAG(rs26.step_26_retention) OVER (
            PARTITION BY rs26.report_date, rs26.Product_Name_Final, rs26.Country_Code
            ORDER BY rs26.billing_cycle
          ) IS NULL THEN NULL
          -- Check if current BC has CB_User < minimum_rebill_users → set to NULL
          WHEN rs26.clean_cb_user < rs26.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
          ELSE LAG(rs26.step_26_retention) OVER (
            PARTITION BY rs26.report_date, rs26.Product_Name_Final, rs26.Country_Code
            ORDER BY rs26.billing_cycle
          ) * (1 - rs26.base_churn_rate)
        END
      ELSE NULL
      ...
      WHEN rs27.billing_cycle = 28 THEN 
        CASE
          ...
          WHEN LAG(rs27.step_27_retention) OVER (
            PARTITION BY rs27.report_date, rs27.Product_Name_Final, rs27.Country_Code
            ORDER BY rs27.billing_cycle
          ) IS NULL THEN NULL
          -- Check if current BC has CB_User < minimum_rebill_users → set to NULL
          WHEN rs27.clean_cb_user < rs27.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
          ELSE LAG(rs27.step_27_retention) OVER (
            PARTITION BY rs27.report_date, rs27.Product_Name_Final, rs27.Country_Code
            ORDER BY rs27.billing_cycle
          ) * (1 - rs27.base_churn_rate)
        END
      ELSE NULL
      ...
      WHEN rs28.billing_cycle = 29 THEN 
        CASE
          ...
          WHEN LAG(rs28.step_28_retention) OVER (
            PARTITION BY rs28.report_date, rs28.Product_Name_Final, rs28.Country_Code
            ORDER BY rs28.billing_cycle
          ) IS NULL THEN NULL
          -- Check if current BC has CB_User < minimum_rebill_users → set to NULL
          WHEN rs28.clean_cb_user < rs28.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
          ELSE LAG(rs28.step_28_retention) OVER (
            PARTITION BY rs28.report_date, rs28.Product_Name_Final, rs28.Country_Code
            ORDER BY rs28.billing_cycle
          ) * (1 - rs28.base_churn_rate)
        END
      ELSE NULL
      ...
      WHEN rs29.billing_cycle = 30 THEN 
        CASE
          ...
          WHEN LAG(rs29.step_29_retention) OVER (
            PARTITION BY rs29.report_date, rs29.Product_Name_Final, rs29.Country_Code
            ORDER BY rs29.billing_cycle
          ) IS NULL THEN NULL
          -- Check if current BC has CB_User < minimum_rebill_users → set to NULL
          WHEN rs29.clean_cb_user < rs29.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
          ELSE LAG(rs29.step_29_retention) OVER (
            PARTITION BY rs29.report_date, rs29.Product_Name_Final, rs29.Country_Code
            ORDER BY rs29.billing_cycle
          ) * (1 - rs29.base_churn_rate)
        END
      ELSE NULL
      ...
      WHEN rs30.billing_cycle = 31 THEN 
        CASE
          ...
          WHEN LAG(rs30.step_30_retention) OVER (
            PARTITION BY rs30.report_date, rs30.Product_Name_Final, rs30.Country_Code
            ORDER BY rs30.billing_cycle
          ) IS NULL THEN NULL
          -- Check if current BC has CB_User < minimum_rebill_users → set to NULL
          WHEN rs30.clean_cb_user < rs30.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
          ELSE LAG(rs30.step_30_retention) OVER (
            PARTITION BY rs30.report_date, rs30.Product_Name_Final, rs30.Country_Code
            ORDER BY rs30.billing_cycle
          ) * (1 - rs30.base_churn_rate)
        END
      ELSE NULL
      ...
      WHEN rs31.billing_cycle = 32 THEN 
        CASE
          ...
          WHEN LAG(rs31.step_31_retention) OVER (
            PARTITION BY rs31.report_date, rs31.Product_Name_Final, rs31.Country_Code
            ORDER BY rs31.billing_cycle
          ) IS NULL THEN NULL
          -- Check if current BC has CB_User < minimum_rebill_users → set to NULL
          WHEN rs31.clean_cb_user < rs31.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
          ELSE LAG(rs31.step_31_retention) OVER (
            PARTITION BY rs31.report_date, rs31.Product_Name_Final, rs31.Country_Code
            ORDER BY rs31.billing_cycle
          ) * (1 - rs31.base_churn_rate)
        END
      ELSE NULL
      ...
      WHEN rs32.billing_cycle = 33 THEN 
        CASE
          ...
          WHEN LAG(rs32.step_32_retention) OVER (
            PARTITION BY rs32.report_date, rs32.Product_Name_Final, rs32.Country_Code
            ORDER BY rs32.billing_cycle
          ) IS NULL THEN NULL
          -- Check if current BC has CB_User < minimum_rebill_users → set to NULL
          WHEN rs32.clean_cb_user < rs32.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
          ELSE LAG(rs32.step_32_retention) OVER (
            PARTITION BY rs32.report_date, rs32.Product_Name_Final, rs32.Country_Code
            ORDER BY rs32.billing_cycle
          ) * (1 - rs32.base_churn_rate)
        END
      ELSE NULL
      ...
      WHEN rs33.billing_cycle = 34 THEN 
        CASE
          ...
          WHEN LAG(rs33.step_33_retention) OVER (
            PARTITION BY rs33.report_date, rs33.Product_Name_Final, rs33.Country_Code
            ORDER BY rs33.billing_cycle
          ) IS NULL THEN NULL
          -- Check if current BC has CB_User < minimum_rebill_users → set to NULL
          WHEN rs33.clean_cb_user < rs33.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
          ELSE LAG(rs33.step_33_retention) OVER (
            PARTITION BY rs33.report_date, rs33.Product_Name_Final, rs33.Country_Code
            ORDER BY rs33.billing_cycle
          ) * (1 - rs33.base_churn_rate)
        END
      ELSE NULL
      ...
      WHEN rs34.billing_cycle = 35 THEN 
        CASE
          ...
          WHEN LAG(rs34.step_34_retention) OVER (
            PARTITION BY rs34.report_date, rs34.Product_Name_Final, rs34.Country_Code
            ORDER BY rs34.billing_cycle
          ) IS NULL THEN NULL
          -- Check if current BC has CB_User < minimum_rebill_users → set to NULL
          WHEN rs34.clean_cb_user < rs34.minimum_rebill_users THEN NULL
          -- Otherwise calculate normally
          ELSE LAG(rs34.step_34_retention) OVER (
            PARTITION BY rs34.report_date, rs34.Product_Name_Final, rs34.Country_Code
            ORDER BY rs34.billing_cycle
          ) * (1 - rs34.base_churn_rate)
        END
      ELSE NULL
      ...
      WHEN rs35.billing_cycle = 36 THEN 
        CASE
          WHEN LAG(rs35.step_35_retention) OVER (
            PARTITION BY rs35.report_date, rs35.Product_Name_Final, rs35.Country_Code
            ORDER BY rs35.billing_cycle
          ) IS NULL THEN NULL
          WHEN rs35.clean_cb_user < rs35.minimum_rebill_users THEN NULL
          ELSE LAG(rs35.step_35_retention) OVER (
            PARTITION BY rs35.report_date, rs35.Product_Name_Final, rs35.Country_Code
            ORDER BY rs35.billing_cycle
          ) * (1 - rs35.base_churn_rate)
        END
...
final_calculations AS (
  SELECT 
    ...
    CASE 
      WHEN rs36.final_retention_rate IS NULL THEN NULL
      ELSE rs36.base_churn_rate
    END as final_churn_rate,
    
    -- Refund_ratio: NULL when Retention IS NULL
    CASE 
      WHEN rs36.final_retention_rate IS NULL THEN NULL
      ELSE rs36.base_refund_ratio
    ...
    CASE 
      WHEN rs36.final_retention_rate IS NULL THEN NULL
      ELSE rs36.base_recent_cac
    END as final_recent_cac,
    
    -- NET_Retention_rate
    CASE 
      WHEN rs36.final_retention_rate IS NULL THEN NULL
      ELSE rs36.final_retention_rate - rs36.base_refund_ratio
    END as final_net_retention_rate,
    
    -- ARPU
    CASE 
      WHEN rs36.final_retention_rate IS NULL THEN NULL
      WHEN rs36.billing_cycle = 0 THEN 
        rs36.final_retention_rate * rs36.Trial_Price * rs36.final_exchange_rate + 
        COALESCE(SAFE_DIVIDE(rs36.clean_single_sale_value, NULLIF(rs36.clean_subscription_users, 0)), 0)
      ELSE 
        rs36.final_retention_rate * rs36.Regular_Price * rs36.final_exchange_rate + 
        COALESCE(SAFE_DIVIDE(rs36.clean_single_sale_value, NULLIF(rs36.clean_subscription_users, 0)), 0)
    END as final_arpu,
    ...
    CASE 
      WHEN rs36.final_retention_rate IS NULL THEN NULL
      ELSE COALESCE(
        SAFE_DIVIDE(rs36.clean_cb_value, NULLIF(rs36.clean_cb_user, 0)) * rs36.final_retention_rate,
        0.0
      ) + COALESCE(SAFE_DIVIDE(rs36.clean_single_sale_value, NULLIF(rs36.clean_subscription_users, 0)), 0)
    ...
    CASE 
      WHEN rs36.final_retention_rate IS NULL THEN NULL
      WHEN rs36.billing_cycle = 0 THEN 
        (rs36.final_retention_rate - rs36.base_refund_ratio) * rs36.Trial_Price * rs36.final_exchange_rate + 
        COALESCE(SAFE_DIVIDE(rs36.clean_single_sale_value, NULLIF(rs36.clean_subscription_users, 0)), 0)
      ELSE 
        (rs36.final_retention_rate - rs36.base_refund_ratio) * rs36.Regular_Price * rs36.final_exchange_rate + 
        COALESCE(SAFE_DIVIDE(rs36.clean_single_sale_value, NULLIF(rs36.clean_subscription_users, 0)), 0)
    ...
    CASE 
      WHEN rs36.final_retention_rate IS NULL THEN NULL
      ELSE COALESCE(
        SAFE_DIVIDE(rs36.clean_cb_value, NULLIF(rs36.clean_cb_user, 0)) * (rs36.final_retention_rate - rs36.base_refund_ratio),
        0.0
      ) + COALESCE(SAFE_DIVIDE(rs36.clean_single_sale_value, NULLIF(rs36.clean_subscription_users, 0)), 0)
    ...
    CASE 
      WHEN rs36.Country_Code IS NOT NULL AND rs36.Country_Code != '' 
      THEN CONCAT(rs36.App_Name, '-', rs36.Country_Code)
      ELSE rs36.App_Name
    ...
    CASE 
      ...
      WHEN rs36.billing_cycle = 0 THEN 
        (rs36.final_retention_rate - rs36.base_refund_ratio) * rs36.Trial_Price * rs36.final_exchange_rate + 
        COALESCE(SAFE_DIVIDE(rs36.clean_single_sale_value, NULLIF(rs36.clean_subscription_users, 0)), 0) - rs36.base_recent_cac
      ELSE 
        (rs36.final_retention_rate - rs36.base_refund_ratio) * rs36.Regular_Price * rs36.final_exchange_rate + 
        COALESCE(SAFE_DIVIDE(rs36.clean_single_sale_value, NULLIF(rs36.clean_subscription_users, 0)), 0) - rs36.base_recent_cac
  ...
  LEFT JOIN (
    ...
    ON CASE 
         ...
         ELSE rs36.App_Name
       END = ap.App_Name 
    AND rs36.Product_Name_Final = ap.Product_Name_Final
)
-- =====================================================
