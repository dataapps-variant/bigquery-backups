CREATE OR REPLACE TABLE `variant-finance-data-project.LMC.FS_refund_data` AS
WITH 
...
-- Step 2: Find products from other apps that map to FS
cross_app_products AS (
    ...
    WHERE Updated_App_Name != App_Name
        AND Updated_App_Name = 'FS'
),

base_data AS (
    -- Step 1: Direct FS refund data
    SELECT 
...
-- Step 5: Filter to keep only records where first 2 letters of Cohort_Concat = "FS"
WHERE SUBSTRING(COALESCE(upd.Updated_Concat, base.Cohort_Concat), 1, 2) = 'FS'
