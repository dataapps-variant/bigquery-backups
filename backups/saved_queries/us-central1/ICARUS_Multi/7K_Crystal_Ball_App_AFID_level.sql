CREATE OR REPLACE TABLE `variant-finance-data-project.ICARUS_Multi.7K_Crystal_Ball_App_AFID_level` AS

WITH
-- =====================================================
-- CONFIGURATION
-- =====================================================
config AS (
  SELECT 7 AS recent_cac_days
),

-- =====================================================
-- CTE 1: AGGREGATE BASE METRICS FROM PLAN-LEVEL TABLE
-- Grain: Report_date × App_Name × Country_Code × AFID × Billing_Cycle
-- SUM all additive base metrics
-- Weighted avg components for Refund_ratio (by CB_Value)
-- =====================================================
app_afid_base_aggregated AS (
  SELECT 
    Report_date,
    App_Name,
    Country_Code,
    AFID,
    Billing_Cycle,
    
    -- Additive base metrics
    SUM(Subscription_users) AS Subscription_users,
    SUM(Subscription_value) AS Subscription_value,
    SUM(Day_0_user) AS Day_0_user,
    SUM(Day_0_values) AS Day_0_values,
    SUM(SS_Users) AS SS_Users,
    SUM(SS_Value) AS SS_Value,
    
    -- Refund weighted average components (NULL Refund_ratio treated as 0%)
    SUM(COALESCE(Refund_ratio, 0) * CB_Value) AS weighted_refund_numerator,
    SUM(CB_Value) AS weighted_refund_denominator
    
  FROM `variant-finance-data-project.Icarus_Cohort.IC_7K_Crystal_Ball`
  GROUP BY Report_date, App_Name, Country_Code, AFID, Billing_Cycle
),

-- =====================================================
-- CTE 2: SOT RATIO LOOKUP (App × AFID level table)
-- NOTE: Table to be created later. Will error until then.
-- =====================================================
app_afid_sot_lookup AS (
  SELECT 
    aba.Report_date,
    aba.App_Name,
    aba.Country_Code,
    aba.AFID,
    aba.Billing_Cycle,
    COALESCE(sot.SOT_Ratio, 0) AS SOT_Ratio
  FROM app_afid_base_aggregated aba
  LEFT JOIN `variant-finance-data-project.ICARUS_Multi.7k_SOT_Ratio_App_AFID` sot
    ON aba.Report_date = sot.Report_date
    AND aba.App_Name = sot.App_Name
    AND aba.Country_Code = sot.Country_Code
    AND aba.AFID = sot.AFID
    AND aba.Billing_Cycle = sot.Billing_Cycle
),

-- =====================================================
-- CTE 3: CB METRICS (Crystal Ball User & Value)
-- CB_User = Day_0_user / SOT_Ratio
-- CB_Value = Day_0_values / SOT_Ratio
-- =====================================================
app_afid_cb_metrics AS (
  SELECT 
    aba.*,
    asl.SOT_Ratio,
    
    -- CB_User = Day_0_user / SOT_Ratio
    COALESCE(
      SAFE_DIVIDE(aba.Day_0_user, NULLIF(asl.SOT_Ratio, 0)),
      0
    ) AS CB_User,
    
    -- CB_Value = Day_0_values / SOT_Ratio
    COALESCE(
      SAFE_DIVIDE(aba.Day_0_values, NULLIF(asl.SOT_Ratio, 0)),
      0
    ) AS CB_Value
    
  FROM app_afid_base_aggregated aba
  INNER JOIN app_afid_sot_lookup asl
    ON aba.Report_date = asl.Report_date
    AND aba.App_Name = asl.App_Name
    AND aba.Country_Code = asl.Country_Code
    AND aba.AFID = asl.AFID
    AND aba.Billing_Cycle = asl.Billing_Cycle
),

-- =====================================================
-- CTE 4: LAST SPEND DATE PER APP + COUNTRY + AFID
-- Find the most recent date with actual ad spend
-- =====================================================
last_spend_dates AS (
  SELECT 
    acm.Report_date,
    acm.App_Name,
    acm.Country_Code,
    acm.AFID,
    MAX(ads.Date) AS last_spend_date
  FROM (
    SELECT DISTINCT Report_date, App_Name, Country_Code, AFID
    FROM app_afid_cb_metrics
    WHERE Billing_Cycle = 0
  ) acm
  LEFT JOIN `variant-finance-data-project.Ad_spend_data.Merged_Spend_Split_TBL` ads
    ON acm.App_Name = ads.App_Name
    AND COALESCE(ads.Channel_code, 80) = acm.AFID
    AND ads.Date <= acm.Report_date
    AND ads.allocated_spend > 0
    AND (
      (acm.Country_Code = 'JP' AND ads.Country = 'JP')
      OR (acm.Country_Code = 'Non-JP' AND (ads.Country != 'JP' OR ads.Country IS NULL))
      OR (acm.Country_Code IS NULL OR acm.Country_Code = '')
    )
  GROUP BY acm.Report_date, acm.App_Name, acm.Country_Code, acm.AFID
),

-- =====================================================
-- CTE 5: SPEND DATE WINDOWS (7-day window from last_spend_date)
-- =====================================================
spend_date_windows AS (
  SELECT 
    Report_date,
    App_Name,
    Country_Code,
    AFID,
    last_spend_date,
    CASE 
      WHEN last_spend_date IS NOT NULL 
      THEN DATE_SUB(last_spend_date, INTERVAL (SELECT recent_cac_days FROM config) - 1 DAY)
      ELSE NULL
    END AS window_start_date,
    last_spend_date AS window_end_date
  FROM last_spend_dates
),

-- =====================================================
-- CTE 6: RECENT SPEND (from source table)
-- Sum ad spend within 7-day window per App + Country + AFID
-- =====================================================
recent_spend_calc AS (
  SELECT 
    sdw.Report_date,
    sdw.App_Name,
    sdw.Country_Code,
    sdw.AFID,
    CASE 
      WHEN sdw.window_start_date IS NOT NULL THEN
        COALESCE(SUM(
          CASE 
            WHEN sdw.Country_Code = 'JP' AND ads.Country = 'JP' 
              THEN ads.allocated_spend
            WHEN sdw.Country_Code = 'Non-JP' AND (ads.Country != 'JP' OR ads.Country IS NULL) 
              THEN ads.allocated_spend
            WHEN sdw.Country_Code IS NULL OR sdw.Country_Code = '' 
              THEN ads.allocated_spend
            ELSE 0
          END
        ), 0)
      ELSE 0
    END AS Recent_Spend
  FROM spend_date_windows sdw
  LEFT JOIN `variant-finance-data-project.Ad_spend_data.Merged_Spend_Split_TBL` ads
    ON sdw.App_Name = ads.App_Name
    AND COALESCE(ads.Channel_code, 80) = sdw.AFID
    AND ads.Date BETWEEN sdw.window_start_date AND sdw.window_end_date
  GROUP BY sdw.Report_date, sdw.App_Name, sdw.Country_Code, sdw.AFID, sdw.window_start_date
),

-- =====================================================
-- CTE 7: RECENT USERS (from source table)
-- COUNT DISTINCT users within 7-day window
-- Uses LEFT(App_Name, 2) prefix match (VPU pattern)
-- AFID matched via COALESCE(AFID_CHANNEL, 80)
-- =====================================================
recent_users_calc AS (
  SELECT 
    sdw.Report_date,
    sdw.App_Name,
    sdw.Country_Code,
    sdw.AFID,
    CASE 
      WHEN sdw.window_start_date IS NOT NULL THEN
        COUNT(DISTINCT base.Updated_Cust_ID)
      ELSE 0
    END AS Recent_Users
  FROM spend_date_windows sdw
  LEFT JOIN `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_V_Merged_TBL` base
    ON LEFT(base.App_Name, 2) = LEFT(sdw.App_Name, 2)
    AND COALESCE(base.AFID_CHANNEL, 80) = sdw.AFID
    AND base.Date_of_Sale BETWEEN sdw.window_start_date AND sdw.window_end_date
    AND base.Trial_Type IS NOT NULL
    AND base.Trial_Type != 'SS'
    -- Billing Cycle filter based on Trial_Type
    AND (
      (base.Trial_Type = 'NT' AND base.Billing_Cycle_Updated = 1)
      OR (base.Trial_Type != 'NT' AND base.Billing_Cycle_Updated = 0)
    )
    -- Country logic (VPU CT-pattern)
    AND (
      -- CT-JP: Only JP country codes
      (LEFT(sdw.App_Name, 2) = 'CT' AND sdw.App_Name NOT LIKE '%Non-JP%' 
        AND sdw.Country_Code = 'JP' AND base.Spend_Country_Code_AFID = 'JP')
      OR
      -- CT-Non-JP: Only non-JP country codes
      (LEFT(sdw.App_Name, 2) = 'CT' AND sdw.App_Name LIKE '%Non-JP%' 
        AND sdw.Country_Code = 'Non-JP' AND base.Spend_Country_Code_AFID != 'JP')
      OR
      -- Non-CT apps with JP
      (LEFT(sdw.App_Name, 2) != 'CT' AND sdw.Country_Code = 'JP' 
        AND base.Spend_Country_Code_AFID = 'JP')
      OR
      -- Non-CT apps with Non-JP
      (LEFT(sdw.App_Name, 2) != 'CT' AND sdw.Country_Code = 'Non-JP' 
        AND (base.Spend_Country_Code_AFID != 'JP' OR base.Spend_Country_Code_AFID IS NULL))
      OR
      -- No country split
      (sdw.Country_Code IS NULL OR sdw.Country_Code = '')
    )
  GROUP BY sdw.Report_date, sdw.App_Name, sdw.Country_Code, sdw.AFID, sdw.window_start_date
),

-- =====================================================
-- CTE 8: T30D NEW USERS (from source table)
-- COUNT DISTINCT users in 30-day window (report_date - 29 to report_date)
-- Same join and country logic as Recent_Users
-- AFID matched via COALESCE(AFID_CHANNEL, 80)
-- =====================================================
t30d_new_users_calc AS (
  SELECT 
    acm.Report_date,
    acm.App_Name,
    acm.Country_Code,
    acm.AFID,
    COUNT(DISTINCT base.Updated_Cust_ID) AS T30D_New_Users
  FROM (
    SELECT DISTINCT Report_date, App_Name, Country_Code, AFID
    FROM app_afid_cb_metrics
  ) acm
  LEFT JOIN `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_V_Merged_TBL` base
    ON LEFT(base.App_Name, 2) = LEFT(acm.App_Name, 2)
    AND COALESCE(base.AFID_CHANNEL, 80) = acm.AFID
    AND base.Date_of_Sale BETWEEN DATE_SUB(acm.Report_date, INTERVAL 29 DAY) AND acm.Report_date
    AND base.Trial_Type IS NOT NULL
    AND base.Trial_Type != 'SS'
    -- Billing Cycle filter based on Trial_Type
    AND (
      (base.Trial_Type = 'NT' AND base.Billing_Cycle_Updated = 1)
      OR (base.Trial_Type != 'NT' AND base.Billing_Cycle_Updated = 0)
    )
    -- Country logic (VPU CT-pattern)
    AND (
      (LEFT(acm.App_Name, 2) = 'CT' AND acm.App_Name NOT LIKE '%Non-JP%' 
        AND acm.Country_Code = 'JP' AND base.Spend_Country_Code_AFID = 'JP')
      OR
      (LEFT(acm.App_Name, 2) = 'CT' AND acm.App_Name LIKE '%Non-JP%' 
        AND acm.Country_Code = 'Non-JP' AND base.Spend_Country_Code_AFID != 'JP')
      OR
      (LEFT(acm.App_Name, 2) != 'CT' AND acm.Country_Code = 'JP' 
        AND base.Spend_Country_Code_AFID = 'JP')
      OR
      (LEFT(acm.App_Name, 2) != 'CT' AND acm.Country_Code = 'Non-JP' 
        AND (base.Spend_Country_Code_AFID != 'JP' OR base.Spend_Country_Code_AFID IS NULL))
      OR
      (acm.Country_Code IS NULL OR acm.Country_Code = '')
    )
  GROUP BY acm.Report_date, acm.App_Name, acm.Country_Code, acm.AFID
),

-- =====================================================
-- CTE 9: ASSEMBLE ALL BASE DATA + CHURN RATE
-- =====================================================
with_churn AS (
  SELECT 
    acm.Report_date,
    acm.App_Name,
    acm.Country_Code,
    acm.AFID,
    acm.Billing_Cycle,
    
    -- Base metrics (from plan table aggregation)
    acm.Subscription_users,
    acm.Subscription_value,
    acm.Day_0_user,
    acm.Day_0_values,
    acm.SOT_Ratio,
    acm.CB_User,
    acm.CB_Value,
    acm.SS_Users,
    acm.SS_Value,
    
    -- Refund components (carry forward for final calc)
    acm.weighted_refund_numerator,
    acm.weighted_refund_denominator,
    
    -- Recent metrics (from source tables, BC0 only)
    CASE 
      WHEN acm.Billing_Cycle = 0 THEN COALESCE(rsc.Recent_Spend, 0)
      ELSE 0
    END AS Recent_Spend,
    CASE 
      WHEN acm.Billing_Cycle = 0 THEN COALESCE(ruc.Recent_Users, 0)
      ELSE 0
    END AS Recent_Users,
    
    -- T30D New Users (from source table)
    COALESCE(t30d.T30D_New_Users, 0) AS T30D_New_Users,
    
    -- Refund ratio (weighted avg by CB_Value)
    COALESCE(
      SAFE_DIVIDE(acm.weighted_refund_numerator, NULLIF(acm.weighted_refund_denominator, 0)),
      0
    ) AS Refund_ratio,
    
    -- Churn rate: BC0 = 0.0, else = 1 - CB_User/Subscription_users
    CASE 
      WHEN acm.Billing_Cycle = 0 THEN 0.00
      ELSE 1 - COALESCE(SAFE_DIVIDE(acm.CB_User, NULLIF(acm.Subscription_users, 0)), 0)
    END AS Churn_rate
    
  FROM app_afid_cb_metrics acm
  LEFT JOIN recent_spend_calc rsc
    ON acm.Report_date = rsc.Report_date
    AND acm.App_Name = rsc.App_Name
    AND acm.Country_Code = rsc.Country_Code
    AND acm.AFID = rsc.AFID
    AND acm.Billing_Cycle = 0
  LEFT JOIN recent_users_calc ruc
    ON acm.Report_date = ruc.Report_date
    AND acm.App_Name = ruc.App_Name
    AND acm.Country_Code = ruc.Country_Code
    AND acm.AFID = ruc.AFID
    AND acm.Billing_Cycle = 0
  LEFT JOIN t30d_new_users_calc t30d
    ON acm.Report_date = t30d.Report_date
    AND acm.App_Name = t30d.App_Name
    AND acm.Country_Code = t30d.Country_Code
    AND acm.AFID = t30d.AFID
),

-- =====================================================
-- RETENTION CASCADE
-- BC0 = 1.0 ALWAYS (no threshold check — Fix #4)
-- CB_User < 25 threshold at BC1-BC12 only (Fix #5)
-- NULL cascade: if previous BC is NULL or CB_User < 25 (Fix #6)
-- Partition by Report_date, App_Name, Country_Code, AFID
-- =====================================================
retention_step_0 AS (
  SELECT 
    wc.*,
    CASE 
      WHEN wc.Billing_Cycle = 0 THEN 1.0
      ELSE NULL
    END AS step_0_retention
  FROM with_churn wc
),

retention_step_1 AS (
  SELECT 
    rs0.*,
    CASE 
      WHEN rs0.step_0_retention IS NOT NULL THEN rs0.step_0_retention
      WHEN rs0.Billing_Cycle = 1 THEN 
        CASE
          WHEN LAG(rs0.step_0_retention) OVER (
            PARTITION BY rs0.Report_date, rs0.App_Name, rs0.Country_Code, rs0.AFID
            ORDER BY rs0.Billing_Cycle
          ) IS NULL THEN NULL
          WHEN rs0.CB_User < 25 THEN NULL
          ELSE LAG(rs0.step_0_retention) OVER (
            PARTITION BY rs0.Report_date, rs0.App_Name, rs0.Country_Code, rs0.AFID
            ORDER BY rs0.Billing_Cycle
          ) * (1 - rs0.Churn_rate)
        END
      ELSE NULL
    END AS step_1_retention
  FROM retention_step_0 rs0
),

retention_step_2 AS (
  SELECT 
    rs1.*,
    CASE 
      WHEN rs1.step_1_retention IS NOT NULL THEN rs1.step_1_retention
      WHEN rs1.Billing_Cycle = 2 THEN 
        CASE
          WHEN LAG(rs1.step_1_retention) OVER (
            PARTITION BY rs1.Report_date, rs1.App_Name, rs1.Country_Code, rs1.AFID
            ORDER BY rs1.Billing_Cycle
          ) IS NULL THEN NULL
          WHEN rs1.CB_User < 25 THEN NULL
          ELSE LAG(rs1.step_1_retention) OVER (
            PARTITION BY rs1.Report_date, rs1.App_Name, rs1.Country_Code, rs1.AFID
            ORDER BY rs1.Billing_Cycle
          ) * (1 - rs1.Churn_rate)
        END
      ELSE NULL
    END AS step_2_retention
  FROM retention_step_1 rs1
),

retention_step_3 AS (
  SELECT 
    rs2.*,
    CASE 
      WHEN rs2.step_2_retention IS NOT NULL THEN rs2.step_2_retention
      WHEN rs2.Billing_Cycle = 3 THEN 
        CASE
          WHEN LAG(rs2.step_2_retention) OVER (
            PARTITION BY rs2.Report_date, rs2.App_Name, rs2.Country_Code, rs2.AFID
            ORDER BY rs2.Billing_Cycle
          ) IS NULL THEN NULL
          WHEN rs2.CB_User < 25 THEN NULL
          ELSE LAG(rs2.step_2_retention) OVER (
            PARTITION BY rs2.Report_date, rs2.App_Name, rs2.Country_Code, rs2.AFID
            ORDER BY rs2.Billing_Cycle
          ) * (1 - rs2.Churn_rate)
        END
      ELSE NULL
    END AS step_3_retention
  FROM retention_step_2 rs2
),

retention_step_4 AS (
  SELECT 
    rs3.*,
    CASE 
      WHEN rs3.step_3_retention IS NOT NULL THEN rs3.step_3_retention
      WHEN rs3.Billing_Cycle = 4 THEN 
        CASE
          WHEN LAG(rs3.step_3_retention) OVER (
            PARTITION BY rs3.Report_date, rs3.App_Name, rs3.Country_Code, rs3.AFID
            ORDER BY rs3.Billing_Cycle
          ) IS NULL THEN NULL
          WHEN rs3.CB_User < 25 THEN NULL
          ELSE LAG(rs3.step_3_retention) OVER (
            PARTITION BY rs3.Report_date, rs3.App_Name, rs3.Country_Code, rs3.AFID
            ORDER BY rs3.Billing_Cycle
          ) * (1 - rs3.Churn_rate)
        END
      ELSE NULL
    END AS step_4_retention
  FROM retention_step_3 rs3
),

retention_step_5 AS (
  SELECT 
    rs4.*,
    CASE 
      WHEN rs4.step_4_retention IS NOT NULL THEN rs4.step_4_retention
      WHEN rs4.Billing_Cycle = 5 THEN 
        CASE
          WHEN LAG(rs4.step_4_retention) OVER (
            PARTITION BY rs4.Report_date, rs4.App_Name, rs4.Country_Code, rs4.AFID
            ORDER BY rs4.Billing_Cycle
          ) IS NULL THEN NULL
          WHEN rs4.CB_User < 25 THEN NULL
          ELSE LAG(rs4.step_4_retention) OVER (
            PARTITION BY rs4.Report_date, rs4.App_Name, rs4.Country_Code, rs4.AFID
            ORDER BY rs4.Billing_Cycle
          ) * (1 - rs4.Churn_rate)
        END
      ELSE NULL
    END AS step_5_retention
  FROM retention_step_4 rs4
),

retention_step_6 AS (
  SELECT 
    rs5.*,
    CASE 
      WHEN rs5.step_5_retention IS NOT NULL THEN rs5.step_5_retention
      WHEN rs5.Billing_Cycle = 6 THEN 
        CASE
          WHEN LAG(rs5.step_5_retention) OVER (
            PARTITION BY rs5.Report_date, rs5.App_Name, rs5.Country_Code, rs5.AFID
            ORDER BY rs5.Billing_Cycle
          ) IS NULL THEN NULL
          WHEN rs5.CB_User < 25 THEN NULL
          ELSE LAG(rs5.step_5_retention) OVER (
            PARTITION BY rs5.Report_date, rs5.App_Name, rs5.Country_Code, rs5.AFID
            ORDER BY rs5.Billing_Cycle
          ) * (1 - rs5.Churn_rate)
        END
      ELSE NULL
    END AS step_6_retention
  FROM retention_step_5 rs5
),

retention_step_7 AS (
  SELECT 
    rs6.*,
    CASE 
      WHEN rs6.step_6_retention IS NOT NULL THEN rs6.step_6_retention
      WHEN rs6.Billing_Cycle = 7 THEN 
        CASE
          WHEN LAG(rs6.step_6_retention) OVER (
            PARTITION BY rs6.Report_date, rs6.App_Name, rs6.Country_Code, rs6.AFID
            ORDER BY rs6.Billing_Cycle
          ) IS NULL THEN NULL
          WHEN rs6.CB_User < 25 THEN NULL
          ELSE LAG(rs6.step_6_retention) OVER (
            PARTITION BY rs6.Report_date, rs6.App_Name, rs6.Country_Code, rs6.AFID
            ORDER BY rs6.Billing_Cycle
          ) * (1 - rs6.Churn_rate)
        END
      ELSE NULL
    END AS step_7_retention
  FROM retention_step_6 rs6
),

retention_step_8 AS (
  SELECT 
    rs7.*,
    CASE 
      WHEN rs7.step_7_retention IS NOT NULL THEN rs7.step_7_retention
      WHEN rs7.Billing_Cycle = 8 THEN 
        CASE
          WHEN LAG(rs7.step_7_retention) OVER (
            PARTITION BY rs7.Report_date, rs7.App_Name, rs7.Country_Code, rs7.AFID
            ORDER BY rs7.Billing_Cycle
          ) IS NULL THEN NULL
          WHEN rs7.CB_User < 25 THEN NULL
          ELSE LAG(rs7.step_7_retention) OVER (
            PARTITION BY rs7.Report_date, rs7.App_Name, rs7.Country_Code, rs7.AFID
            ORDER BY rs7.Billing_Cycle
          ) * (1 - rs7.Churn_rate)
        END
      ELSE NULL
    END AS step_8_retention
  FROM retention_step_7 rs7
),

retention_step_9 AS (
  SELECT 
    rs8.*,
    CASE 
      WHEN rs8.step_8_retention IS NOT NULL THEN rs8.step_8_retention
      WHEN rs8.Billing_Cycle = 9 THEN 
        CASE
          WHEN LAG(rs8.step_8_retention) OVER (
            PARTITION BY rs8.Report_date, rs8.App_Name, rs8.Country_Code, rs8.AFID
            ORDER BY rs8.Billing_Cycle
          ) IS NULL THEN NULL
          WHEN rs8.CB_User < 25 THEN NULL
          ELSE LAG(rs8.step_8_retention) OVER (
            PARTITION BY rs8.Report_date, rs8.App_Name, rs8.Country_Code, rs8.AFID
            ORDER BY rs8.Billing_Cycle
          ) * (1 - rs8.Churn_rate)
        END
      ELSE NULL
    END AS step_9_retention
  FROM retention_step_8 rs8
),

retention_step_10 AS (
  SELECT 
    rs9.*,
    CASE 
      WHEN rs9.step_9_retention IS NOT NULL THEN rs9.step_9_retention
      WHEN rs9.Billing_Cycle = 10 THEN 
        CASE
          WHEN LAG(rs9.step_9_retention) OVER (
            PARTITION BY rs9.Report_date, rs9.App_Name, rs9.Country_Code, rs9.AFID
            ORDER BY rs9.Billing_Cycle
          ) IS NULL THEN NULL
          WHEN rs9.CB_User < 25 THEN NULL
          ELSE LAG(rs9.step_9_retention) OVER (
            PARTITION BY rs9.Report_date, rs9.App_Name, rs9.Country_Code, rs9.AFID
            ORDER BY rs9.Billing_Cycle
          ) * (1 - rs9.Churn_rate)
        END
      ELSE NULL
    END AS step_10_retention
  FROM retention_step_9 rs9
),

retention_step_11 AS (
  SELECT 
    rs10.*,
    CASE 
      WHEN rs10.step_10_retention IS NOT NULL THEN rs10.step_10_retention
      WHEN rs10.Billing_Cycle = 11 THEN 
        CASE
          WHEN LAG(rs10.step_10_retention) OVER (
            PARTITION BY rs10.Report_date, rs10.App_Name, rs10.Country_Code, rs10.AFID
            ORDER BY rs10.Billing_Cycle
          ) IS NULL THEN NULL
          WHEN rs10.CB_User < 25 THEN NULL
          ELSE LAG(rs10.step_10_retention) OVER (
            PARTITION BY rs10.Report_date, rs10.App_Name, rs10.Country_Code, rs10.AFID
            ORDER BY rs10.Billing_Cycle
          ) * (1 - rs10.Churn_rate)
        END
      ELSE NULL
    END AS step_11_retention
  FROM retention_step_10 rs10
),

retention_step_12 AS (
  SELECT 
    rs11.*,
    CASE 
      WHEN rs11.step_11_retention IS NOT NULL THEN rs11.step_11_retention
      WHEN rs11.Billing_Cycle = 12 THEN 
        CASE
          WHEN LAG(rs11.step_11_retention) OVER (
            PARTITION BY rs11.Report_date, rs11.App_Name, rs11.Country_Code, rs11.AFID
            ORDER BY rs11.Billing_Cycle
          ) IS NULL THEN NULL
          WHEN rs11.CB_User < 25 THEN NULL
          ELSE LAG(rs11.step_11_retention) OVER (
            PARTITION BY rs11.Report_date, rs11.App_Name, rs11.Country_Code, rs11.AFID
            ORDER BY rs11.Billing_Cycle
          ) * (1 - rs11.Churn_rate)
        END
      ELSE rs11.step_11_retention
    END AS Retention_rate
  FROM retention_step_11 rs11
),

-- =====================================================
-- ACTIVE/INACTIVE STATUS
-- Active if ANY product under this App exists in Active_Plans_6M
-- =====================================================
active_apps AS (
  SELECT DISTINCT App_Name
  FROM `variant-finance-data-project.ICARUS_Multi.Active_Plans_6M`
),

-- =====================================================
-- FINAL CALCULATIONS
-- NULL cascade: all downstream metrics NULL when Retention_rate IS NULL
-- =====================================================
final_calculations AS (
  SELECT 
    rs12.*,
    
    -- Active/Inactive
    CASE 
      WHEN aa.App_Name IS NOT NULL THEN 'Active'
      ELSE 'Inactive'
    END AS Active_Inactive,

    -- FIX 6: Churn_rate — NULL when Retention IS NULL
    CASE 
      WHEN rs12.Retention_rate IS NULL THEN NULL
      ELSE rs12.Churn_rate
    END AS final_Churn_rate,

    -- FIX 6: Refund_ratio — NULL when Retention IS NULL
    CASE 
      WHEN rs12.Retention_rate IS NULL THEN NULL
      ELSE rs12.Refund_ratio
    END AS final_Refund_ratio,
    
    -- NET Retention rate
    CASE 
      WHEN rs12.Retention_rate IS NULL THEN NULL
      ELSE rs12.Retention_rate - rs12.Refund_ratio
    END AS NET_Retention_rate,
    
    -- Recent CAC (BC0 only, NULL cascade)
    CASE 
      WHEN rs12.Retention_rate IS NULL THEN NULL
      ELSE COALESCE(
        CASE 
          WHEN rs12.Billing_Cycle = 0 
          THEN SAFE_DIVIDE(rs12.Recent_Spend, NULLIF(rs12.Recent_Users, 0))
          ELSE 0.0
        END,
        0.0
      )
    END AS Recent_CAC,
    
    -- ARPU_Discounted = (CB_Value/CB_User) x Retention + SS/Subscription_users
    CASE 
      WHEN rs12.Retention_rate IS NULL THEN NULL
      ELSE COALESCE(
        SAFE_DIVIDE(rs12.CB_Value, NULLIF(rs12.CB_User, 0)) * rs12.Retention_rate,
        0.0
      ) + COALESCE(
        SAFE_DIVIDE(rs12.SS_Value, NULLIF(rs12.Subscription_users, 0)),
        0.0
      )
    END AS ARPU_Discounted,
    
    -- Net_ARPU_Discounted = (CB_Value/CB_User) x (Retention - Refund) + SS/Subscription_users
    CASE 
      WHEN rs12.Retention_rate IS NULL THEN NULL
      ELSE COALESCE(
        SAFE_DIVIDE(rs12.CB_Value, NULLIF(rs12.CB_User, 0)) * (rs12.Retention_rate - rs12.Refund_ratio),
        0.0
      ) + COALESCE(
        SAFE_DIVIDE(rs12.SS_Value, NULLIF(rs12.Subscription_users, 0)),
        0.0
      )
    END AS Net_ARPU_Discounted
    
  FROM retention_step_12 rs12
  LEFT JOIN active_apps aa
    ON rs12.App_Name = aa.App_Name
)

-- =====================================================
-- FINAL OUTPUT
-- =====================================================
SELECT 
  -- Dimensions (5)
  fc.Report_date,
  fc.App_Name,
  fc.Country_Code,
  fc.AFID,
  fc.Billing_Cycle,
  
  -- Status (1)
  fc.Active_Inactive,
  
  -- Base Metrics - from plan table SUM (6)
  fc.Subscription_users,
  fc.Subscription_value,
  fc.Day_0_user,
  fc.Day_0_values,
  fc.SS_Users,
  fc.SS_Value,
  
  -- Base Metrics - from source tables (3)
  fc.Recent_Spend,
  fc.Recent_Users,
  fc.T30D_New_Users,
  
  -- SOT & Crystal Ball (3)
  fc.SOT_Ratio,
  fc.CB_User,
  fc.CB_Value,
  
  -- Rates (4) - all NULL when Retention IS NULL
  fc.final_Churn_rate AS Churn_rate,
  fc.final_Refund_ratio AS Refund_ratio,
  fc.Retention_rate,
  fc.NET_Retention_rate,
  
  -- CAC (1)
  fc.Recent_CAC,
  
  -- Revenue Metrics - Discounted only (3)
  fc.ARPU_Discounted,
  fc.Net_ARPU_Discounted,
  CASE 
    WHEN fc.Net_ARPU_Discounted IS NULL THEN NULL
    ELSE fc.Net_ARPU_Discounted - fc.Recent_CAC
  END AS Net_LTV_Discounted

FROM final_calculations fc
ORDER BY fc.Report_date DESC, fc.App_Name, fc.Country_Code, fc.AFID, fc.Billing_Cycle;

-- =====================================================
-- SCRIPT COMPLETE - 7K CRYSTAL BALL APP × AFID LEVEL (300D)
-- Table: variant-finance-data-project.ICARUS_Multi.7K_Crystal_Ball_App_AFID_level
--
-- GRAIN: Report_date × App_Name × Country_Code × AFID × Billing_Cycle
--
-- SOURCE TABLE:
-- variant-finance-data-project.Icarus_Cohort.IC_7K_Crystal_Ball (300D, limiting_days=300)
--
-- SOT LOOKUP:
-- variant-finance-data-project.ICARUS_Multi.7k_SOT_Ratio_App_AFID
--
-- SOURCE TABLE JOINS:
-- Spend: COALESCE(ads.Channel_code, 80) = AFID
-- Sticky: COALESCE(base.AFID_CHANNEL, 80) = AFID
--
-- ALL 7 FIXES APPLIED:
-- 1. Refund denominator: SUM(COALESCE(Refund_ratio,0) × CB_Value) / SUM(CB_Value) — ALL rows
-- 2. No Spend_amount in output
-- 3. No CAC (only Recent_CAC)
-- 4. BC0 = 1.0 ALWAYS (no threshold check)
-- 5. CB_User < 25 threshold at BC1-BC12 only → NULL
-- 6. Full NULL cascade: Churn, Refund, NET_Ret, CAC, ARPU, Net_ARPU, Net_LTV
-- 7. Net_LTV NULL check via Net_ARPU_Discounted IS NULL
--
-- IDENTICAL TO 30D VERSION EXCEPT SOURCE TABLE:
-- 30D reads from IC_7K_30D_Crystal_Ball (limiting_days=30)
-- 300D reads from IC_7K_Crystal_Ball (limiting_days=300)
--
-- OUTPUT COLUMNS: 26
-- Dimensions: Report_date, App_Name, Country_Code, AFID, Billing_Cycle
-- Status: Active_Inactive
-- Base: Subscription_users, Subscription_value, Day_0_user, Day_0_values, SS_Users, SS_Value
-- Source: Recent_Spend, Recent_Users, T30D_New_Users
-- CB: SOT_Ratio, CB_User, CB_Value
-- Rates: Churn_rate, Refund_ratio, Retention_rate, NET_Retention_rate
-- CAC: Recent_CAC
-- Revenue: ARPU_Discounted, Net_ARPU_Discounted, Net_LTV_Discounted
-- =====================================================
