-- =============================================================================
-- RETENTION ANALYSIS V2.4: BC0 through BC12 with Rolling Cohorts
-- =============================================================================
--
-- CHANGES FROM V2.3 TO V2.4:
--   1. Removed NULL cascade logic (first_invalid_bc, first_zero_gross_bc,
--      first_zero_net_bc window columns removed)
--   2. Simplified cumulative retention to straight EXP(SUM(LN)) product
--   3. Final WHERE filters Cohort_Size >= min_cohort_size (50)
--   4. Anchor BC net_survival uses Approved_Users as denominator
--      (NT BC1 and Non-NT BC0)
--   5. Added Churn_Rate column (per-BC, independent, same denominator logic)
--
-- =============================================================================
...
cumulative_retention AS (
  SELECT
    *,
    ...
    EXP(SUM(LN(NULLIF(gross_survival, 0))) OVER (
