...
-- Step 1: Dynamically build the list of months
SET months_list = (
  SELECT STRING_AGG(month_quoted ORDER BY month_quoted)
  FROM (
    SELECT DISTINCT
      FORMAT("'%s'",
        FORMAT('%d-%d-%d', 1,
...
SET query_str = FORMAT("""
  ...
  filtered AS (
    SELECT
      COALESCE(pm.Updated_Concat, m.Product_Name_Final) AS Product_Group,
      FORMAT('%%d-%%d-%%d', 1,
