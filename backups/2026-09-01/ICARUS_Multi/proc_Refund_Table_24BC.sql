...
BEGIN
  ...
  -- metadata operations.
  CREATE OR REPLACE TABLE `variant-finance-data-project.ICARUS_Multi.Refund_Table_24BC_staging`
  PARTITION BY Report_date
  CLUSTER BY Product_Name_Final, Country_Code, Billing_Cycle
  AS
  WITH
  ...
  ------------------------------------------------------------------
  DROP TABLE IF EXISTS `variant-finance-data-project.ICARUS_Multi.Refund_Table_24BC`;

  ALTER TABLE `variant-finance-data-project.ICARUS_Multi.Refund_Table_24BC_staging`
    RENAME TO Refund_Table_24BC;
