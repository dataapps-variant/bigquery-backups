-- =====================================================
-- 7K_30D_Main_Table_App_AFID_level_24BC — INCREMENTAL APPEND
-- Converted from proc_7K_30D_Main_Table_App_AFID_level_24BC (full rebuild).
--
-- WHAT CHANGED (4 things, nothing else):
--   1. DECLARE block + DELETE-last-N-days + window calculation.
--   2. ONE date filter on app_afid_base_aggregated. (Only one, because this
--      rollup has no VG UNION branch — see note at the bottom.)
...
-- your rebuild, including the retention cascade 0..24.
-- =====================================================

CREATE OR REPLACE PROCEDURE `variant-finance-data-project.ICARUS_Multi.proc_7K_30D_Main_Table_App_AFID_level_24BC_Append`()
BEGIN
  ...
  -- =====================================================
  DELETE FROM `variant-finance-data-project.ICARUS_Multi.7K_30D_Main_Table_App_AFID_level_24BC`
  WHERE Report_date >= DATE_SUB(
    (SELECT MAX(Report_date) FROM `variant-finance-data-project.ICARUS_Multi.7K_30D_Main_Table_App_AFID_level_24BC`),
  ...
  SET last_processed_date = (
    ...
    FROM `variant-finance-data-project.ICARUS_Multi.7K_30D_Main_Table_App_AFID_level_24BC`
  );
  ...
  IF days_to_process > 0 THEN
    ...
    -- =====================================================
    INSERT INTO `variant-finance-data-project.ICARUS_Multi.7K_30D_Main_Table_App_AFID_level_24BC` (
      Report_date, App_Name, Country_Code, AFID, Billing_Cycle,
      Active_Inactive,
      Subscription_users, Subscription_value, Rebill_users, Rebill_value,
      SS_Users, SS_Value,
    ...
    CASE WHEN rs12.step_12_retention IS NOT NULL THEN rs12.step_12_retention
      WHEN rs12.Billing_Cycle = 13 THEN 
        CASE WHEN LAG(rs12.step_12_retention) OVER (PARTITION BY rs12.Report_date, rs12.App_Name, rs12.Country_Code, rs12.AFID ORDER BY rs12.Billing_Cycle) IS NULL THEN NULL
          WHEN rs12.Rebill_users < 25 THEN NULL
    ...
    CASE WHEN rs13.step_13_retention IS NOT NULL THEN rs13.step_13_retention
      WHEN rs13.Billing_Cycle = 14 THEN 
        CASE WHEN LAG(rs13.step_13_retention) OVER (PARTITION BY rs13.Report_date, rs13.App_Name, rs13.Country_Code, rs13.AFID ORDER BY rs13.Billing_Cycle) IS NULL THEN NULL
          WHEN rs13.Rebill_users < 25 THEN NULL
    ...
    CASE WHEN rs14.step_14_retention IS NOT NULL THEN rs14.step_14_retention
      WHEN rs14.Billing_Cycle = 15 THEN 
        CASE WHEN LAG(rs14.step_14_retention) OVER (PARTITION BY rs14.Report_date, rs14.App_Name, rs14.Country_Code, rs14.AFID ORDER BY rs14.Billing_Cycle) IS NULL THEN NULL
          WHEN rs14.Rebill_users < 25 THEN NULL
    ...
    CASE WHEN rs15.step_15_retention IS NOT NULL THEN rs15.step_15_retention
      WHEN rs15.Billing_Cycle = 16 THEN 
        CASE WHEN LAG(rs15.step_15_retention) OVER (PARTITION BY rs15.Report_date, rs15.App_Name, rs15.Country_Code, rs15.AFID ORDER BY rs15.Billing_Cycle) IS NULL THEN NULL
          WHEN rs15.Rebill_users < 25 THEN NULL
    ...
    CASE WHEN rs16.step_16_retention IS NOT NULL THEN rs16.step_16_retention
      WHEN rs16.Billing_Cycle = 17 THEN 
        CASE WHEN LAG(rs16.step_16_retention) OVER (PARTITION BY rs16.Report_date, rs16.App_Name, rs16.Country_Code, rs16.AFID ORDER BY rs16.Billing_Cycle) IS NULL THEN NULL
          WHEN rs16.Rebill_users < 25 THEN NULL
    ...
    CASE WHEN rs17.step_17_retention IS NOT NULL THEN rs17.step_17_retention
      WHEN rs17.Billing_Cycle = 18 THEN 
        CASE WHEN LAG(rs17.step_17_retention) OVER (PARTITION BY rs17.Report_date, rs17.App_Name, rs17.Country_Code, rs17.AFID ORDER BY rs17.Billing_Cycle) IS NULL THEN NULL
          WHEN rs17.Rebill_users < 25 THEN NULL
    ...
    CASE WHEN rs18.step_18_retention IS NOT NULL THEN rs18.step_18_retention
      WHEN rs18.Billing_Cycle = 19 THEN 
        CASE WHEN LAG(rs18.step_18_retention) OVER (PARTITION BY rs18.Report_date, rs18.App_Name, rs18.Country_Code, rs18.AFID ORDER BY rs18.Billing_Cycle) IS NULL THEN NULL
          WHEN rs18.Rebill_users < 25 THEN NULL
    ...
    CASE WHEN rs19.step_19_retention IS NOT NULL THEN rs19.step_19_retention
      WHEN rs19.Billing_Cycle = 20 THEN 
        CASE WHEN LAG(rs19.step_19_retention) OVER (PARTITION BY rs19.Report_date, rs19.App_Name, rs19.Country_Code, rs19.AFID ORDER BY rs19.Billing_Cycle) IS NULL THEN NULL
          WHEN rs19.Rebill_users < 25 THEN NULL
    ...
    CASE WHEN rs20.step_20_retention IS NOT NULL THEN rs20.step_20_retention
      WHEN rs20.Billing_Cycle = 21 THEN 
        CASE WHEN LAG(rs20.step_20_retention) OVER (PARTITION BY rs20.Report_date, rs20.App_Name, rs20.Country_Code, rs20.AFID ORDER BY rs20.Billing_Cycle) IS NULL THEN NULL
          WHEN rs20.Rebill_users < 25 THEN NULL
    ...
    CASE WHEN rs21.step_21_retention IS NOT NULL THEN rs21.step_21_retention
      WHEN rs21.Billing_Cycle = 22 THEN 
        CASE WHEN LAG(rs21.step_21_retention) OVER (PARTITION BY rs21.Report_date, rs21.App_Name, rs21.Country_Code, rs21.AFID ORDER BY rs21.Billing_Cycle) IS NULL THEN NULL
          WHEN rs21.Rebill_users < 25 THEN NULL
    ...
    CASE WHEN rs22.step_22_retention IS NOT NULL THEN rs22.step_22_retention
      WHEN rs22.Billing_Cycle = 23 THEN 
        CASE WHEN LAG(rs22.step_22_retention) OVER (PARTITION BY rs22.Report_date, rs22.App_Name, rs22.Country_Code, rs22.AFID ORDER BY rs22.Billing_Cycle) IS NULL THEN NULL
          WHEN rs22.Rebill_users < 25 THEN NULL
    ...
    CASE WHEN rs23.step_23_retention IS NOT NULL THEN rs23.step_23_retention
      WHEN rs23.Billing_Cycle = 24 THEN 
        CASE WHEN LAG(rs23.step_23_retention) OVER (PARTITION BY rs23.Report_date, rs23.App_Name, rs23.Country_Code, rs23.AFID ORDER BY rs23.Billing_Cycle) IS NULL THEN NULL
          WHEN rs23.Rebill_users < 25 THEN NULL
...
final_calculations AS (
  SELECT 
    ...
    CASE 
      WHEN rs24.Retention_rate IS NULL THEN NULL
      ELSE COALESCE(
        SAFE_DIVIDE(rs24.Rebill_value, NULLIF(rs24.Rebill_users, 0)) * rs24.Retention_rate,
        0.0
      ) + COALESCE(
    ...
    CASE 
      WHEN rs24.Retention_rate IS NULL THEN NULL
      ELSE COALESCE(
        SAFE_DIVIDE(rs24.Rebill_value, NULLIF(rs24.Rebill_users, 0)) * (rs24.Retention_rate - rs24.Refund_ratio),
        0.0
      ) + COALESCE(
        SAFE_DIVIDE(rs24.SS_Value, NULLIF(rs24.Subscription_users, 0)),
        0.0
...
-- =====================================================
-- Unlike 7K_Main_Table_App_level / 7K_Crystal_Ball_App_level, this rollup has
-- NO VG UNION branch — app_afid_base_aggregated is a single SELECT. So there
-- is one filter to add, not two. If you diff this against those files, the
-- missing second filter is correct, not an omission.
--
-- Every lookback reaches a RAW source, never back into
-- app_afid_base_aggregated:
...
--
-- The cascade LAG is
--   PARTITION BY Report_date, App_Name, Country_Code, AFID
-- so no billing cycle reads across report dates.
--
--
-- =====================================================
-- CASCADE DIFFERENCES FROM THE IC_ PRODUCT-LEVEL PROCS
-- =====================================================
-- Carried over verbatim; do not harmonise without re-baselining:
--
-- 1. The gate is a HARDCODED literal 25 (Rebill_users < 25), not a config
--    column threaded through master_combinations. There is no
--    minimum_rebill_users in this query's config at all — config holds only
--    recent_cac_days. If 25 ever needs to change it must be edited in 24
--    places, or lifted into config as its own change.
--
-- 2. The gate resolves to NULL, not 0.0. The IC_ product-level procs
--    (IC_7K_30D_Main_Table_24BC and siblings) use 0.0 there, and their final_calculations
--    have matching "= 0 THEN 0.0" branches. This query has no such branches —
--    it only tests "Retention_rate IS NULL" — so NULL is the correct pairing
--    here. Copying 0.0 in from those files would silently produce zero
--    ARPU/LTV rows instead of NULL ones.
--
-- 3. step_4 has a plain ELSE NULL here. The IC_ procs have an odd
--    ELSE rs3.step_3_retention at that step. Only step_24 carries the
--    ELSE rs23.step_23_retention tail in this file.
--
--
-- =====================================================
-- CROSS-DATASET — RUN ORDER MATTERS
-- =====================================================
-- This procedure lives in ICARUS_Multi but reads from Icarus_Cohort:
--
--   target : ICARUS_Multi.7K_30D_Main_Table_App_AFID_level_24BC
--   source : Icarus_Cohort.IC_7K_30D_Main_Table_24BC
...
--   proc_IC_7K_30D_Main_Table_24BC_Append
--   proc_7K_30D_Main_Table_App_AFID_level_24BC_Append   <- this one
--
...
-- Retention_rate and Net_ARPU_Discounted is overstated. It will not error.
--
-- Keep lookback_days aligned with IC_7K_30D_Main_Table_24BC's, or restated days
-- on the source never propagate into this rollup.
--
--
-- =====================================================
...
--        COUNTIF(Refund_ratio = 0) AS zero_refund
-- FROM `variant-finance-data-project.ICARUS_Multi.7K_30D_Main_Table_App_AFID_level_24BC`;
--
-- last_loaded should be yesterday and max_bc should be 24. A spike in
-- zero_refund usually means IC_Refund_Table_App_24BC is stale, not that
-- refunds actually stopped.
--
-- Cross-check the rollup against its source for the newest day:
-- SELECT s.Report_date,
--        SUM(s.Rebill_users) AS src_rebills,
--        MAX(t.tot)          AS rollup_rebills
-- FROM `variant-finance-data-project.Icarus_Cohort.IC_7K_30D_Main_Table_24BC` s
-- JOIN (SELECT Report_date, SUM(Rebill_users) AS tot
--       FROM `variant-finance-data-project.ICARUS_Multi.7K_30D_Main_Table_App_AFID_level_24BC`
--       GROUP BY 1) t
--   ON s.Report_date = t.Report_date
-- WHERE s.Report_date = DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY)
-- GROUP BY 1;
-- The two totals should match exactly — this rollup only SUMs its source.
