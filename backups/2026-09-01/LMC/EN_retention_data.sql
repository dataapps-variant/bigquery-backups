CREATE OR REPLACE TABLE `variant-finance-data-project.LMC.EN_retention_data` AS
WITH 
...
-- Step 2: Find products from other apps that map to EN
cross_app_products AS (
    ...
    WHERE Updated_App_Name != App_Name
        AND Updated_App_Name = 'EN'
),

base_data AS (
    -- Step 1: Direct EN retention data
    SELECT 
        ...
        COUNT(Updated_Cust_ID) as Updated_Cust_ID_count,
        CONCAT( 
            Product_Name_Final, 
    ...
    SELECT 
        ...
        COUNT(sticky.Updated_Cust_ID) as Updated_Cust_ID_count,
        CONCAT( 
...
-- Filter to keep only records where first 2 letters of Cohort_Concat = "EN"
WHERE SUBSTRING(COALESCE(upd.Updated_Concat, base.Cohort_Concat), 1, 2) = 'EN'
