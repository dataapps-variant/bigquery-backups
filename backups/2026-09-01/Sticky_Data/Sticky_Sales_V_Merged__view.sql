CREATE VIEW `variant-finance-data-project.Sticky_Data.Sticky_Sales_V_Merged`
AS WITH Prev_bc AS (
  SELECT *,
         ROW_NUMBER() OVER (
  ...
  FROM `Sticky_Data.Sticky_Sales_V_W_EC_Merged` cl
),
Next_bc AS (
  SELECT *,
         ROW_NUMBER() OVER (
...
CL AS (
  SELECT * FROM `Sticky_Data.Sticky_Sales_V_W_EC_Merged`
),
CL_With_Joins AS (
  SELECT 
    CL.*,
