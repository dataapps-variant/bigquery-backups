...
-- Cohort     = customer's SECOND churn within that app, bucketed
--              by the month of THAT churn
-- Repurchase = new chain in the SAME APP (any product, ANY CHANNEL)
--              after the second churn
-- CT is split into CT-JP / CT-Non-JP on Spend_Country_Code_AFID
-- Bands are NON-OVERLAPPING; censored cells returned as NULL
--
-- HOW THIS DIFFERS FROM THE FIRST-CANCELLATION TABLE
--   Only a customer who came back after churn 1 can reach churn 2,
--   so this cohort is a SUBSET of the first table's repurchasers.
--   Read the shares within this table across months, and compare
--   the curve as a whole against the first-cancellation curve to
--   answer "do win-backs get easier or harder the second time?"
--   Do NOT read these shares as a percentage of the original
--   first-cancellation population.
--
--   Sanity check: Second_Time_Cancelled here should be somewhat
--   BELOW Repurchased_Total_730d in the first-cancellation table.
...
--
-- CONCURRENCY
--   A customer running two overlapping chains in the same app will
--   churn both, producing a "second cancellation" with no return
--   in between. That is not a win-back cycle. The optional filter
--   in second_churn restricts to chains that genuinely STARTED
--   after the first churn.
--
-- CHANNEL SEMANTICS
--   AFID_Channel is the acquisition channel of the chain that
--   produced the SECOND churn -- i.e. the channel that RE-acquired
--   the customer, not their original acquisition channel. The
...
CREATE OR REPLACE TABLE
  `variant-finance-data-project.Sticky_Data.Second_Cancellation_Resubscribers`
PARTITION BY cohort_month
CLUSTER BY App_Name, channel_code
OPTIONS (
  description = "Post-cancellation repurchase curve by App x AFID_Channel x churn month, for each customer's SECOND inferred churn per app. Churn is inferred (expected next bill date passed with no further order in the chain), so this table RESTATES on every rebuild as chains in dunning recover. This cohort is a SUBSET of the first-cancellation table's repurchasers -- do not read its shares against the original first-cancellation population. AFID_Channel is the acquisition channel of the chain that produced the second churn, i.e. the re-acquiring channel. Censored bands are NULL, not zero."
)
AS
WITH base AS (
  SELECT
...
-- ***** SECOND churn per customer + app *****
-- (the first-cancellation table uses rn = 1 here)
...
-- earliest same-app repurchase after the second churn
repurchase AS (
  SELECT
...
SELECT
  ...
  channel_code,
  AFID_Channel,
  COUNT(DISTINCT Customer_Number)        AS Second_Time_Cancelled,
