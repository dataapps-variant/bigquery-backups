CREATE OR REPLACE VIEW `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_CN` AS
...
UNION ALL
  ...
  SELECT
    ...
    prd_offer_id AS Offer_Id,
    cc_first_6 AS BIN
  FROM `variant-finance-data-project.Sticky_Data.hold_orders_CN`
  UNION ALL
  SELECT
    FORMAT_TIMESTAMP('%Y-%m-%d %H:%M:%S', TIMESTAMP(synced_at)) AS last_updated,
    order_id AS Order_Id,
    ...
    prd_offer_id AS Offer_Id,
    cc_first_6 AS BIN
  FROM `variant-finance-data-project.Sticky_Data.hold_orders_2_CN`
),
ranked_data AS (
  SELECT *,
    ROW_NUMBER() OVER (
