CREATE OR REPLACE VIEW `variant-finance-data-project.Sticky_Data.Sticky_data_API_original_JF` AS
...
UNION ALL
  ...
  SELECT
    ...
    prd_hold_type AS Hold_Type,
    prd_offer_id AS Offer_Id,
    SAFE.PARSE_JSON(custom_fields) AS custom_fields,
    cc_first_6 AS BIN
  FROM `variant-finance-data-project.Sticky_Data.hold_orders_JF`
UNION ALL
  SELECT
    FORMAT_TIMESTAMP('%Y-%m-%d %H:%M:%S', TIMESTAMP(synced_at)) AS last_updated,
    order_id AS Order_Id,
    ...
    SAFE.PARSE_JSON(custom_fields) AS custom_fields,
    cc_first_6 AS BIN
  FROM `variant-finance-data-project.Sticky_Data.hold_orders_2_JF`
),
ranked_data AS (
  SELECT *,
    ROW_NUMBER() OVER (
