CREATE PROCEDURE `variant-finance-data-project`.VPU.proc_15K_Main_Table_MP()
BEGIN

-- =====================================================
-- 15K MAIN TABLE CREATION SCRIPT - WITH RECENT_CAC
-- Cohort Analysis for Subscription Business Metrics
-- 
-- UPDATES IN THIS VERSION:
-- 1. Added Recent_Spend CTE (last 7 days before report_date)
-- 2. Added Recent_Users CTE (last 7 days before report_date, first purchases only)
-- 3. Added Recent_CAC column (Recent_Spend / Recent_Users)
-- 4. Kept original CAC column
-- 5. Replaced CAC with Recent_CAC in Net_LTV formulas
-- 6. Added Recent_CAC_WA column
-- =====================================================

CREATE OR REPLACE TABLE `variant-finance-data-project.VPU.15K_Main_Table_MP` AS
...
config AS (
  SELECT 
    ...
    7 AS recent_cac_days  -- NEW: Days for Recent_CAC calculation (7 days ending on last spend date)
),
...
-- =====================================================
-- RECENT SPEND DATE WINDOW CALCULATION (NEW LOGIC)
-- Find the last date with spend > 0, then take recent_cac_days ending on that date
-- =====================================================
recent_spend_date_window AS (
  ...
  LEFT JOIN `variant-finance-data-project.Ad_spend_data.Merged_Spend_Split_TBL` ads
    ...
    AND ads.allocated_spend > 0  -- Only dates with actual spend
    AND (
      -- Country filtering
...
-- Calculate start and end dates for recent window
...
-- =====================================================
-- RECENT SPEND CALCULATION (UPDATED LOGIC)
-- Uses recent_cac_days ending on last spend date
-- =====================================================
...
-- =====================================================
-- RECENT USERS CALCULATION (UPDATED LOGIC)
-- Uses SAME date window as Recent_Spend
-- =====================================================
...
calculated_ratios AS (
  SELECT 
    ...
    -- Refund ratio from Refund_Table
    COALESCE(rt.Refund_Ratio, 0.00) as final_refund_ratio,
    
    -- CAC calculation (ORIGINAL - KEPT)
    COALESCE(
      CASE 
...
retention_step_1 AS (
  SELECT 
    ...
    CASE 
      ...
      WHEN rs0.billing_cycle = 1 AND rs0.Trial_Type != 'NT' THEN 1.0 * (1 - rs0.final_churn_rate)
      ELSE NULL
...
final_calculations AS (
  SELECT 
    ...
    -- Net retention rate
    rs4.final_retention_rate - rs4.final_refund_ratio as final_net_retention_rate,
    
    -- ARPU with exchange rate + single sales (already in USD)
    CASE 
    ...
    -- ARPU Discounted: Changed to use Rebill_value/Rebill_users + single sales
    COALESCE(
      SAFE_DIVIDE(rs4.clean_rebill_value, NULLIF(rs4.clean_rebill_users, 0)) * rs4.final_retention_rate,
      0.0
    ) + COALESCE(SAFE_DIVIDE(rs4.clean_single_sale_value, NULLIF(rs4.clean_subscription_users, 0)), 0) as final_arpu_discounted,
    
    -- Net ARPU with exchange rate + single sales (already in USD)
    CASE 
      WHEN rs4.billing_cycle = 0 THEN 
        (rs4.final_retention_rate - rs4.final_refund_ratio) * rs4.Trial_Price * rs4.final_exchange_rate + 
        COALESCE(SAFE_DIVIDE(rs4.clean_single_sale_value, NULLIF(rs4.clean_subscription_users, 0)), 0)
      ELSE 
        (rs4.final_retention_rate - rs4.final_refund_ratio) * rs4.Regular_Price * rs4.final_exchange_rate + 
        COALESCE(SAFE_DIVIDE(rs4.clean_single_sale_value, NULLIF(rs4.clean_subscription_users, 0)), 0)
    END as final_net_arpu,
    
    -- Net ARPU Discounted: Changed to use Rebill_value/Rebill_users + single sales
    COALESCE(
      SAFE_DIVIDE(rs4.clean_rebill_value, NULLIF(rs4.clean_rebill_users, 0)) * (rs4.final_retention_rate - rs4.final_refund_ratio),
      0.0
    ) + COALESCE(SAFE_DIVIDE(rs4.clean_single_sale_value, NULLIF(rs4.clean_subscription_users, 0)), 0) as final_net_arpu_discounted
...
weighted_avg_prep AS (
  SELECT 
    ...
    -- Compute App_Name with Country_Code for Key generation
    CASE 
    ...
    -- UPDATED: LTV calculations now use Recent_CAC instead of CAC
    (CASE 
      WHEN fc.billing_cycle = 0 THEN 
        (fc.final_retention_rate - fc.final_refund_ratio) * fc.Trial_Price * fc.final_exchange_rate + 
        COALESCE(SAFE_DIVIDE(fc.clean_single_sale_value, NULLIF(fc.clean_subscription_users, 0)), 0)
      ELSE 
        (fc.final_retention_rate - fc.final_refund_ratio) * fc.Regular_Price * fc.final_exchange_rate + 
        COALESCE(SAFE_DIVIDE(fc.clean_single_sale_value, NULLIF(fc.clean_subscription_users, 0)), 0)
    END) - fc.final_recent_cac as final_net_ltv,
    
    (COALESCE(
      SAFE_DIVIDE(fc.clean_rebill_value, NULLIF(fc.clean_rebill_users, 0)) * (fc.final_retention_rate - fc.final_refund_ratio),
      0.0
    ) + COALESCE(SAFE_DIVIDE(fc.clean_single_sale_value, NULLIF(fc.clean_subscription_users, 0)), 0)) - fc.final_recent_cac as final_net_ltv_discounted
...
weighted_avg_final AS (
  SELECT 
    ...
    -- KEY: Concatenate App_Name + Report_date + Billing_Cycle
    CONCAT(
    ...
    -- BASE: Sum of Rebill_value for the same Key
    SUM(wap.clean_rebill_value) OVER (
      PARTITION BY 
        wap.computed_app_name,
        wap.report_date, 
        wap.billing_cycle
    ...
    -- WA_RATIO: Rebill_value / BASE
    SAFE_DIVIDE(
      wap.clean_rebill_value,
      NULLIF(SUM(wap.clean_rebill_value) OVER (
        PARTITION BY 
          wap.computed_app_name,
          wap.report_date, 
          wap.billing_cycle
...
SELECT 
  ...
  waf.clean_rebill_users as Rebill_users,
  waf.clean_rebill_value as Rebill_value,
  ...
  waf.final_cac as CAC,  -- KEPT: Original CAC
  waf.final_recent_cac as Recent_CAC,  -- NEW: Recent CAC
  ...
  -- UPDATED: LTV Calculations (now use Recent_CAC)
  ...
  waf.final_cac * COALESCE(waf.final_wa_ratio, 0.0) as CAC_WA,  -- KEPT: Original CAC_WA
  waf.final_recent_cac * COALESCE(waf.final_wa_ratio, 0.0) as Recent_CAC_WA,  -- NEW: Recent CAC_WA
...
-- =====================================================
-- SCRIPT COMPLETE - WITH RECENT_CAC IMPLEMENTATION
-- Table: variant-finance-data-project.VPU.15K_Main_Table
-- 
-- SUMMARY OF CHANGES:
-- ✅ Added recent_cac_days config variable (default: 7 days)
-- ✅ Added recent_spend_date_window CTE (finds last date with spend > 0)
-- ✅ Added recent_date_range CTE (calculates recent_cac_days window)
-- ✅ Updated recent_spend_metrics (uses last spend date window)
-- ✅ Updated recent_users_metrics (uses same window as spend)
-- ✅ Added Recent_Spend column in output
-- ✅ Added Recent_Users column in output
-- ✅ Added Recent_CAC column in output (Recent_Spend / Recent_Users)
-- ✅ Kept original CAC column
-- ✅ Updated Net_LTV to use Recent_CAC instead of CAC
-- ✅ Updated Net_LTV_Discounted to use Recent_CAC instead of CAC
-- ✅ Kept original CAC_WA column
-- ✅ Added Recent_CAC_WA column (Recent_CAC * WA_RATIO)
-- 
-- RECENT_CAC LOGIC (UPDATED):
-- 1. Find last_spend_date: Most recent date where allocated_spend > 0
-- 2. Calculate window: [last_spend_date - (recent_cac_days - 1), last_spend_date]
-- 3. Recent_Spend: Sum of spend in this window
-- 4. Recent_Users: Count of first purchases in SAME window
-- 5. Recent_CAC: Recent_Spend / Recent_Users
-- 6. If no spend found: Recent_Spend = 0, Recent_Users = 0, Recent_CAC = 0
-- 
-- EXAMPLE:
-- report_date = June 9, 2025
-- last_spend_date = May 15, 2025 (last date with spend > 0)
-- recent_cac_days = 7
-- Window = [May 9, May 15] (7 days ending on last spend)
-- Recent_Spend = sum of spend from May 9-15
-- Recent_Users = count of first purchases from May 9-15
