...
CREATE OR REPLACE VIEW Sticky_Data.Sticky_Sales_V_CN_W_EC AS
WITH IP_Parse AS (
  SELECT 
    CN.*,
    'CN' AS Entity_Name,
    LOWER(TRIM(SPLIT(CN.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(CN.IP_Address_Lookup, ' ')) - 1)])) AS last_1_word,
    LOWER(TRIM(
      SPLIT(CN.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(CN.IP_Address_Lookup, ' ')) - 2)] || ' ' ||
      SPLIT(CN.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(CN.IP_Address_Lookup, ' ')) - 1)]
    )) AS last_2_words,
    LOWER(TRIM(
      SPLIT(CN.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(CN.IP_Address_Lookup, ' ')) - 3)] || ' ' ||
      SPLIT(CN.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(CN.IP_Address_Lookup, ' ')) - 2)] || ' ' ||
      SPLIT(CN.IP_Address_Lookup, ' ')[SAFE_OFFSET(ARRAY_LENGTH(SPLIT(CN
      .IP_Address_Lookup, ' ')) - 1)]
    )) AS last_3_words
  FROM Sticky_Data.Sticky_CN  CN
),
...
Country_Lookup_Base AS (
  ...
  LEFT JOIN `Sticky_Data.Sticky_Dim_Campaign` CAM
    ON CONCAT('CN_', ip.Campaign_Id) = CONCAT(CAM.Entity,'_',CAM.Campaign_Id)
),
...
SELECT 
  ...
  CONCAT('CN_', CL.Customer_Number,CL.Acquisition_Date_Time) AS Updated_Cust_ID,
  CONCAT('CN_', CL.Order_Id ) AS Updated_Order_ID,
...
LEFT JOIN Sticky_Data.Sticky_Dim_AFID AF 
  ON CONCAT('CN_', CL.AFID) = CONCAT(AF.ENTITY, '_', AF.AFID)

LEFT JOIN Sticky_Data.Sticky_Dim_Product PROD 
  ON CONCAT('CN_', CL.Product_Id) = CONCAT(PROD.ENTITY, '_', PROD.Product_ID)
...
LEFT JOIN Sticky_Data.Sticky_Dim_Offer OFR 
  ON CONCAT('CN_', CL.Offer_Id) = CONCAT(OFR.Entity, '_', OFR.ID)

LEFT JOIN Sticky_Data.Sticky_Dim_Order_Status OS
  ON CONCAT('CN_', CL.Order_Status) = CONCAT('CN_', OS.Order_Status)
