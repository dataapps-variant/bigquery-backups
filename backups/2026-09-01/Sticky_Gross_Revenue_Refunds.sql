CREATE OR REPLACE TABLE `variant-finance-data-project.Utilities.Sticky_Gross_Revenue_Refunds` AS
WITH revenue AS (
  SELECT
...
FULL OUTER JOIN refunds f
  ON r.Month = f.Refund_Month AND r.Entity_Name = f.Entity_Name
ORDER BY 1, 2
