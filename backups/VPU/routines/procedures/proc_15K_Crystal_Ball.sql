CREATE PROCEDURE `variant-finance-data-project`.VPU.proc_15K_Crystal_Ball()
BEGIN

-- =====================================================
-- 15K CRYSTAL BALL TABLE - WITH ZERO SUBSCRIPTION LOGIC
-- Cohort Analysis with Crystal Ball Projections
-- 
-- NEW IN THIS VERSION:
-- 1. When Subscription_users = 0, set Retention_rate = 0
-- 2. When Retention_rate = 0, zero out all dependent metrics
-- 3. Exclude zero-retention rows from BASE and WA_RATIO
-- =====================================================

CREATE OR REPLACE TABLE `variant-finance-data-project.VPU.15K_Crystal_Ball` AS
...
-- =====================================================
-- SCRIPT COMPLETE - CRYSTAL BALL WITH ZERO SUBSCRIPTION LOGIC
-- 
-- CHANGES APPLIED:
-- 1. Subscription_users = 0 → Retention_rate = 0 (in retention steps)
-- 2. Automatic cascade through LAG() to subsequent billing cycles
-- 3. Created base ratios, then adjusted versions when Retention = 0:
--    - Churn_rate, Refund_ratio, CAC, Recent_CAC
-- 4. All ARPU and LTV metrics = 0 when Retention = 0
-- 5. BASE and WA_RATIO exclude rows with Retention_rate = 0
-- 6. Spend metrics (Spend_amount, Recent_Spend, Recent_Users) unchanged
-- 
-- KEY DIFFERENCE FROM MAIN TABLE:
-- Uses CB_User and CB_Value (projected metrics) instead of Rebill metrics
-- Logic is identical, just different source metrics
-- =====================================================
